...
const ADAPTATION_SCOPES_FALLBACK = [{
  ...
  label: 'Palette materica'
}, {
  key: 'imagery',
  label: 'Riferimenti visivi'
}, {
  key: 'cultural_refs',
  label: 'Riferimenti culturali'
}, {
  key: 'headlines',
  label: 'Titoli e cappelli'
}, {
  key: 'atmosphere',
  label: 'Atmosfera narrativa'
}];
...
const NARRATIVE_MODES_OPTS = [{
  key: 'strategic',
  label: 'Strategica · sintetica e progettuale'
}, {
  key: 'technical',
  label: 'Tecnica · architettonica, zero metafore'
}, {
  key: 'emotional',
  label: 'Emozionale · misurata, sensoriale'
}, {
  key: 'cinematic',
  label: 'Cinematografica · immersiva'
}, {
  key: 'hospitality',
  label: 'Ospitale · esperienziale'
}, {
  key: 'luxury_editorial',
  label: 'Editorial luxury · magazine alta gamma'
}, {
  key: 'commercial_soft',
  label: 'Commerciale morbida · rassicurante'
}, {
  key: 'cultural_analyst',
  label: 'Consulenziale · culturale internazionale'
}, {
...
const NARRATIVE_INTENSITY_OPTS = [{
  key: 'minimal',
  label: 'Minimal · essenziale'
}, {
  key: 'balanced',
  label: 'Bilanciata · misurata'
}, {
  key: 'editorial',
  label: 'Editoriale · densa'
}, {
  key: 'cinematic',
  label: 'Cinematica · narrativa'
}];
const labelForNarrativeMode = key => NARRATIVE_MODES_OPTS.find(o => o.key === key)?.label || key;
const labelForIntensity = key => NARRATIVE_INTENSITY_OPTS.find(o => o.key === key)?.label || key;
const CulturalEditionWizard = ({
  open,
  onClose,
  onCreated,
  ...
  useEffect(() => {
    ...
    api.get('/api/cultural-editions/markets').then(r => {
      setConfig(r.data);
  ...
  useEffect(() => {
    ...
    api.get(`/api/cultural-editions/sources?type=${sourceType}`).then(r => setSources(r.data?.items || [])).catch(() => setSources([])).finally(() => setLoadingSources(false));
  }, [open, sourceType]);
  ...
  const selectedMarket = useMemo(() => (config?.markets || []).find(m => m.code === market), [market, config]);
  const selectedProfile = selectedMarket?.narrative_profile || null;
  const isOverridden = useMemo(() => {
    if (!selectedProfile) return false;
    return narrativeMode && narrativeMode !== selectedProfile.suggested_narrative_mode || narrativeIntensity && narrativeIntensity !== selectedProfile.suggested_intensity;
  ...
  if (!open) return null;
  return <div className="cew-backdrop" data-testid="cultural-edition-wizard-backdrop" onClick={e => {
    if (e.target === e.currentTarget) onClose?.();
  }}>
      <div className="cew-modal" data-testid="cultural-edition-wizard">
        <button type="button" className="cew-close" onClick={onClose} aria-label={t("cultural.cultural_edition.chiudi")} data-testid="cew-close">
          <Icons.X size={16} />
        </button>
        ...
        <header className="cew-head">
          ...
          <p className="cew-lede">
            {t("cultural.cultural_edition.mood_prende_un_tuo_contenuto_e_ne_prepara_una_vers")}
          </p>
        </header>

        <nav className="cew-stepper" data-testid="cew-stepper">
          {STEPS.map((s, i) => <div key={s.key} className={`cew-step ${i === stepIdx ? 'cew-step--current' : ''} ${i < stepIdx ? 'cew-step--done' : ''}`} data-testid={`cew-step-${s.key}`}>
        ...
        <div className="cew-body">
          {/* STEP 1 — TIPO */}
          {stepIdx === 0 && <div className="cew-step-panel" data-testid="cew-panel-source-type">
              <p className="cew-question">Da quale contenuto vuoi partire?</p>
              <div className="cew-grid-2">
                {(config?.source_types || SOURCE_TYPES_FALLBACK).map(t => <button key={t.key} type="button" onClick={() => {
              setSourceType(t.key);
              setSourceId('');
            }} className={`cew-card ${sourceType === t.key ? 'cew-card--selected' : ''}`} data-testid={`cew-source-type-${t.key}`}>
          ...
          {/* STEP 2 — CONTENUTO */}
          {stepIdx === 1 && <div className="cew-step-panel" data-testid="cew-panel-source">
              <p className="cew-question">{t('cultural.cultural_edition.scegli_il_contenuto_da_adattare')}</p>
              {sourceType === 'showcase' || sourceType === 'material_selection' ? <div className="cew-field">
                  <label className="cew-label">
                    {sourceType === 'showcase' ? 'Riferimento dello showcase' : 'Riferimento della selezione materiali'}
                  </label>
                  <input type="text" className="cew-input" value={sourceId} placeholder="Incolla l'identificativo o uno slug" onChange={e => {
              setSourceId(e.target.value);
              setSourceTitle(e.target.value);
            }} data-testid="cew-source-id-input" />
                  <p className="cew-hint">
                    {t("cultural.cultural_edition.showcase_e_selezioni_materiali_sono_in_lavorazione")}
                  </p>
                </div> : loadingSources ? <div className="cew-loading">{t('cultural.cultural_edition.in_attesa_dei_contenuti_dello_studio')}</div> : sources.length === 0 ? <div className="cew-empty">
                  <Icons.Folder size={20} strokeWidth={1.2} />
                  <p>Nessun {sourceType === 'project' ? 'progetto' : 'moodboard'} ancora disponibile.</p>
                </div> : <ul className="cew-source-list" data-testid="cew-source-list">
                  {sources.map(s => <li key={s.id}>
                      <button type="button" onClick={() => {
                setSourceId(s.id);
                setSourceTitle(s.title);
              }} className={`cew-source-row ${sourceId === s.id ? 'cew-source-row--selected' : ''}`} data-testid={`cew-source-${s.id}`}>
                        <span className="cew-source-row__cover">
                          {s.cover_url ? <img src={s.cover_url} alt={s.title} loading="lazy" /> : <Icons.Image size={16} strokeWidth={1.2} />}
                        </span>
          ...
          {/* STEP 3 — MERCATO */}
          {stepIdx === 2 && <div className="cew-step-panel" data-testid="cew-panel-market">
              <p className="cew-question">Per quale mercato vuoi adattarlo?</p>
              <div className="cew-grid-3">
                {(config?.markets || []).map(m => <button key={m.code} type="button" onClick={() => setMarket(m.code)} className={`cew-market ${market === m.code ? 'cew-market--selected' : ''}`} data-testid={`cew-market-${m.code}`}>
                    ...
                    <div className="cew-market__chips">
                      {(m.descriptors || []).slice(0, 3).map((d, i) => <span key={i} className="cew-market__chip">{d}</span>)}
                    </div>
                  </button>)}
              </div>
              {selectedMarket && <div className="cew-field" style={{
            marginTop: 22
          }}>
                  <label className="cew-label">{t('cultural.cultural_edition.lingua_della_versione_mercato')}</label>
                  <select className="cew-input" value={locale} onChange={e => setLocale(e.target.value)} data-testid="cew-locale-select">
              ...
              {selectedMarket && selectedProfile && <div className="cew-narrative-profile" data-testid="cew-narrative-profile">
                  <div className="cew-narrative-profile__head">
                    <span className="cew-narrative-profile__badge">
                      <Icons.Compass size={11} strokeWidth={1.6} /> {t("cultural.cultural_edition.suggerito_dal_mercato")}
                    </span>
                  ...
                  {selectedProfile.narrative_direction?.length > 0 && <div className="cew-narrative-profile__chips">
                      {selectedProfile.narrative_direction.slice(0, 5).map((d, i) => <span key={i} className="cew-narrative-profile__chip">{d}</span>)}
                    </div>}
                  <div className="cew-narrative-profile__grid">
                    <div className="cew-field">
                      <label className="cew-label">Direzione narrativa</label>
                      <select className="cew-input" value={narrativeMode} onChange={e => {
                  setNarrativeMode(e.target.value);
                  setManualOverride(true);
                }} data-testid="cew-narrative-mode">
                        {NARRATIVE_MODES_OPTS.map(o => <option key={o.key} value={o.key}>{o.label}</option>)}
                      </select>
                    </div>
                    <div className="cew-field">
                      <label className="cew-label">Intensità narrativa</label>
                      <select className="cew-input" value={narrativeIntensity} onChange={e => {
                  setNarrativeIntensity(e.target.value);
                  setManualOverride(true);
                }} data-testid="cew-narrative-intensity">
                        {NARRATIVE_INTENSITY_OPTS.map(o => <option key={o.key} value={o.key}>{o.label}</option>)}
                      </select>
                    </div>
                  </div>
                  {isOverridden ? <div className="cew-narrative-profile__override" data-testid="cew-narrative-override">
                      <span>{t('cultural.cultural_edition.hai_personalizzato_la_direzione_del_mercato')}</span>
                      <button type="button" className="cew-narrative-profile__reset" onClick={resetToSuggested} data-testid="cew-narrative-reset">
                        <Icons.RotateCcw size={10} /> {t("cultural.cultural_edition.torna_ai_suggerimenti_del_mercato")}
                      </button>
                    </div> : <p className="cew-narrative-profile__hint">
                      {t("cultural.cultural_edition.mood_ha_pre_compilato_i_campi_con_la_cultura_narra")}
          ...
          {/* STEP 4 — AMBITO */}
          {stepIdx === 3 && <div className="cew-step-panel" data-testid="cew-panel-scope">
              <p className="cew-question">Cosa vuoi adattare?</p>
              <p className="cew-hint cew-hint--inline">
                {t("cultural.cultural_edition.puoi_selezionare_uno_o_piu_ambiti_la_redazione_di")}
              </p>
              <div className="cew-scope-list">
                {(config?.adaptation_scopes || ADAPTATION_SCOPES_FALLBACK).map(s => {
              const on = scope.includes(s.key);
              return <button key={s.key} type="button" onClick={() => toggleScope(s.key)} className={`cew-scope ${on ? 'cew-scope--on' : ''}`} data-testid={`cew-scope-${s.key}`}>
              ...
              <div className="cew-field" style={{
            marginTop: 22
          }}>
                <label className="cew-label">{t('cultural.cultural_edition.briefing_al_caporedattore_opzionale')}</label>
                <textarea className="cew-textarea" rows={3} placeholder="Tono che desideri, vincoli, riferimenti culturali da onorare…" value={note} onChange={e => setNote(e.target.value)} data-testid="cew-note" />
              </div>
            </div>}

          {/* STEP 5 — REVISIONE */}
          {stepIdx === 4 && <div className="cew-step-panel" data-testid="cew-panel-review">
              ...
              <div className="cew-review">
                ...
                {(narrativeMode || narrativeIntensity) && <div className="cew-review__row" data-testid="cew-review-narrative">
                    ...
                    <span className="cew-review__val">
                      ...
                      {isOverridden && <em className="cew-review__hint"> · personalizzata</em>}
                      {!isOverridden && selectedProfile && <em className="cew-review__hint"> {t("cultural.cultural_edition.suggerita_dal_mercato")}</em>}
                    </span>
                  </div>}
                <div className="cew-review__row">
                ...
                {note && <div className="cew-review__row">
                    <span className="cew-review__lbl">Briefing</span>
                    <span className="cew-review__val cew-review__val--soft">"{note}"</span>
                  </div>}
              </div>
              {submitting && <div className="cew-processing" data-testid="cew-processing">
                  <span className="cew-processing__pulse" />
                  {t("cultural.cultural_edition.la_redazione_internazionale_di_mood_sta_preparando")}
                </div>}
            </div>}
        </div>

        <footer className="cew-foot">
          <button type="button" className="cew-btn cew-btn--ghost" onClick={stepIdx === 0 ? onClose : prev} disabled={submitting} data-testid="cew-prev">
            {stepIdx === 0 ? 'Annulla' : 'Indietro'}
          </button>
          <button type="button" className="cew-btn cew-btn--primary" onClick={next} disabled={!canAdvance || submitting} data-testid="cew-next">
            {submitting ? 'Creazione…' : stepIdx === STEPS.length - 1 ? 'Crea versione mercato' : 'Avanti'}
...
const labelForSourceType = (config, key) => (config?.source_types || SOURCE_TYPES_FALLBACK).find(t => t.key === key)?.label || key;
