...
const BeforeAfter = ({
  ...
  return <div className="se-baf" ref={wrap} data-testid="se-baf">
      <img src={before} alt="Prima dell'intervento" className="se-baf__img" />
      <div className="se-baf__after-wrap" style={{
      width: `${pos}%`
    }}>
      ...
      <button type="button" className="se-baf__handle" style={{
      left: `${pos}%`
    }} onMouseDown={onMouseDown} onTouchMove={onTouchMove} aria-label="Trascina per confrontare prima e dopo" data-testid="se-baf-handle" />
    </div>;
};

// ── Single entry card ────────────────────────────────────────────
const SiteEvolutionEntry = ({
  entry
    ...
    <div className="se-entry__body">
      ...
      {entry.before_url && entry.after_url ? <BeforeAfter before={entry.before_url} after={entry.after_url} /> : entry.photos?.length > 0 ? <div className="se-photos" data-testid={`se-photos-${entry.id}`}>
          {entry.photos.map((p, i) => <figure key={i} className="se-photo" data-testid={`se-photo-${entry.id}-${i}`}>
              <img src={p.url} alt={p.caption || entry.title} loading="lazy" />
              {p.caption && <figcaption className="se-photo__caption">{p.caption}</figcaption>}
            </figure>)}
        </div> : null}
    </div>
  </article>;
...
const SiteEvolutionSection = ({
  ...
  const spaces = useMemo(() => {
    return [{
      key: 'all',
      label: t('client.site_evolution.all_spaces', null, 'All spaces')
    }, ...groups.map(g => ({
      key: g.space_key,
      label: g.space_label
    }))];
  ...
  const visible = useMemo(() => {
    const list = activeSpace === 'all' ? entries : entries.filter(e => e.space_key === activeSpace);
    return [...list].sort((a, b) => (b.occurred_at || '').localeCompare(a.occurred_at || ''));
  }, [entries, activeSpace]);
  if (!data?.available || entries.length === 0) {
    return <div className="se-empty" data-testid="se-empty">
  ...
  return <div data-testid="se-section">
      {/* Filtro per spazio */}
      <div className="se-spaces" data-testid="se-spaces">
        {spaces.map(s => <button key={s.key} type="button" onClick={() => setActiveSpace(s.key)} className={`se-space-chip ${activeSpace === s.key ? 'is-active' : ''}`} data-testid={`se-space-${s.key}`}>
            {s.label}
          </button>)}
      </div>
