...
import {
  Plus, X, Smartphone, Monitor, Eye, Edit3, Trash2, Save, Loader2,
  MapPin, Gem, BookOpen, Folder, Link2,
} from 'lucide-react';
...
const KINDS = [
  ...
  { id: 'material_note',     label: 'Material Note',     icon: Gem },
  { id: 'design_note',       label: 'Design Note',       icon: BookOpen },
  { id: 'discover_detail',   label: 'Discover Detail',   icon: Eye },
  { id: 'editorial_hotspot', label: 'Editorial Hotspot', icon: Link2 },
];
...
const HotspotEditor = ({
  ...
  const handleImageClick = useCallback((e) => {
    ...
    setDraft({ x_pct: parseFloat(x_pct.toFixed(2)), y_pct: parseFloat(y_pct.toFixed(2)), kind: 'detail_point', title: '', description: '' });
  ...
  const handlePinPointerDown = (e, h) => {
    if (readOnly) return;
    e.stopPropagation();
    setDragging({ id: h.id, startX: e.clientX, startY: e.clientY, originalX: h.x_pct, originalY: h.y_pct });
  };
  useEffect(() => {
    if (!dragging) return;
    const move = (e) => {
      ...
      setDragging({ ...dragging, currentX: nx, currentY: ny });
    };
    const up = async () => {
      if (dragging.currentX !== undefined && onUpdate) {
        ...
        } catch { toast.error('Posizione non salvata'); }
      }
      setDragging(null);
    };
  ...
  const active = activeId === '__draft__'
    ? draft
  ...
  const handleDelete = async (hid) => {
    ...
    try {
      ...
      setActiveId(null);
    } catch { toast.error('Rimozione fallita'); }
    finally { setBusy(false); }
  };

  return (
    <div className={`hot-wrap ${view === 'mobile' ? 'is-mobile' : ''}`} data-testid="hotspot-editor">
      {/* Top bar */}
        ...
        <div className="hot-toolbar__right">
          <div className="hot-view-toggle">
            <button type="button" onClick={() => setView('desktop')}
                    ...
                    data-testid="hot-view-desktop"
                    className={view === 'desktop' ? 'is-active' : ''}>
              <Monitor size={11} />
            </button>
            <button type="button" onClick={() => setView('mobile')}
                    aria-pressed={view === 'mobile'}
                    data-testid="hot-view-mobile"
                    className={view === 'mobile' ? 'is-active' : ''}>
              <Smartphone size={11} />
            </button>
          </div>
          {!readOnly && (
            <span className="hot-hint">
      ...
      <div className="hot-canvas-wrap">
        <div
          ref={imgRef}
          className="hot-canvas"
          onClick={handleImageClick}
          data-testid="hot-canvas"
        >
          ...
          {hotspots.map((h) => {
            ...
            return (
              <button
                key={h.id}
                type="button"
                className={`hot-pin ${isActive ? 'is-active' : ''}`}
                style={{ left: `${x}%`, top: `${y}%` }}
                onClick={(e) => { e.stopPropagation(); setActiveId(h.id); }}
                onPointerDown={(e) => handlePinPointerDown(e, h)}
                data-testid={`hot-pin-${h.id}`}
                title={h.title || kind.label}
              >
          ...
          {/* Draft pin (unsaved) */}
          {draft && (
            <span className="hot-pin is-draft" style={{ left: `${draft.x_pct}%`, top: `${draft.y_pct}%` }}>
        ...
        {active && (
          <aside className="hot-panel" data-testid="hot-panel">
            ...
            <div className="hot-panel__body">
                ...
                {KINDS.map((k) => {
                  ...
                  return (
                    <button
                      key={k.id}
                      type="button"
                      className={`hot-kind ${sel ? 'is-active' : ''}`}
                      data-testid={`hot-kind-${k.id}`}
                      onClick={() => {
                        if (activeId === '__draft__') setDraft({ ...draft, kind: k.id });
                        else onUpdate?.(active.id, { kind: k.id });
                      }}>
                      <Icon size={11} />
                      <span>{k.label}</span>
                    </button>
              ...
              <input
                value={activeId === '__draft__' ? draft.title : (active.title || '')}
                onChange={(e) => {
                  const v = e.target.value;
                  if (activeId === '__draft__') setDraft({ ...draft, title: v });
                  else onUpdate?.(active.id, { title: v });
                }}
                className="hot-input"
                placeholder="es. Pietra travertino · taglio orizzontale"
                data-testid="hot-input-title"
                autoFocus={activeId === '__draft__'}
              />

              <label className="hot-label">Descrizione</label>
              <textarea
                rows={3}
                value={activeId === '__draft__' ? draft.description : (active.description || '')}
                onChange={(e) => {
                  const v = e.target.value;
                  if (activeId === '__draft__') setDraft({ ...draft, description: v });
                  else onUpdate?.(active.id, { description: v });
                }}
                className="hot-input"
                placeholder="Breve nota editoriale sul dettaglio."
                data-testid="hot-input-description"
              />
              ...
              {activeId !== '__draft__' && (
                <>
                  ...
                  <div className="hot-links">
                    {active.linked_material_id && (
                      <span className="hot-chip"><Gem size={10} /> material · {String(active.linked_material_id).slice(0, 8)}</span>
                    )}
                    {active.linked_project_id && (
                      <span className="hot-chip"><Folder size={10} /> project · {String(active.linked_project_id).slice(0, 8)}</span>
                    )}
                    {active.linked_article_id && (
                      <span className="hot-chip"><BookOpen size={10} /> article · {String(active.linked_article_id).slice(0, 8)}</span>
                    )}
                    {active.cta_action && (
                      <span className="hot-chip"><Link2 size={10} /> CTA · {active.cta_action}</span>
                    )}
                    {!active.linked_material_id && !active.linked_project_id && !active.linked_article_id && !active.cta_action && (
                      <span className="hot-empty">Nessun collegamento — UI aggiunta in P1.</span>
                    )}
                  </div>
                </>
              )}
            ...
            <footer className="hot-panel__foot">
              {activeId === '__draft__' ? (
                ...
                <>
                  <button type="button" onClick={() => handleDelete(active.id)}
                          disabled={busy}
                          className="hot-btn hot-btn--danger" data-testid="hot-delete">
