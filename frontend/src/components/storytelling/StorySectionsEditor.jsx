...
import {
  GripVertical, Trash2, Plus, Type, Quote, Image as ImgIcon,
  Images as GalleryIcon, MapPin, Megaphone, ChevronDown, ChevronUp,
} from 'lucide-react';
...
const BLOCK_TYPES = [
  ...
  { id: 'pull_quote',    label: 'Pull Quote',     icon: Quote },
  { id: 'image',         label: 'Immagine',       icon: ImgIcon },
  { id: 'gallery',       label: 'Mini gallery',   icon: GalleryIcon },
  { id: 'hotspot_image', label: 'Immagine + Detail Points', icon: MapPin },
...
const StorySectionsEditor = ({
  ...
  const safeBlocks = React.useMemo(
    () => blocks.map((b, i) => b.id ? b : { ...b, id: `legacy_${i}_${Math.random().toString(36).slice(2, 8)}` }),
  ...
  const updateBlock = useCallback((id, patch) => {
    const next = safeBlocks.map((b) => (b.id === id ? { ...b, ...patch } : b));
    onChange?.(next);
  }, [safeBlocks, onChange]);
  ...
  const addBlock = (type) => {
    ...
    let block;
    switch (type) {
      case 'paragraph':     block = { ...base, text: '' }; break;
      case 'pull_quote':    block = { ...base, text: '', attribution: '' }; break;
      case 'image':         block = { ...base, url: '', asset_id: null, caption: '', alt_text: '' }; break;
      case 'gallery':       block = { ...base, items: [] }; break;
      case 'hotspot_image': block = { ...base, url: '', asset_id: null, caption: '', hotspots: [] }; break;
      case 'cta':           block = { ...base, label: '', action: 'save_reference', tier: 'soft' }; break;
      default:              block = { ...base, text: '' };
    }
  ...
  return (
    <section className="ss-wrap" data-testid={testId}>
      ...
      {!readOnly && (
        <div className="ss-addbar">
          {!showAdd ? (
            <button
              type="button"
              className="ss-add-trigger"
              data-testid="ss-add-trigger"
              onClick={() => setShowAdd(true)}
            >
              <Plus size={13} /> Aggiungi blocco
            </button>
          ) : (
            <div className="ss-add-menu" data-testid="ss-add-menu">
              {BLOCK_TYPES.map((t) => {
                const Icon = t.icon;
                return (
                  <button
                    key={t.id}
                    type="button"
                    className="ss-add-opt"
                    data-testid={`ss-add-${t.id}`}
      ...
      {hotspotBlockId && (() => {
        ...
        return (
          <HotspotImageOverlay
            open
            imageUrl={blk.url}
            imageCaption={blk.caption}
            hotspots={blk.hotspots || []}
            mode={isRemote ? 'remote' : 'memory'}
            remoteArticleId={remoteArticleId}
            remoteBlockId={blk.id}
            onChange={(nextHs) => updateBlock(blk.id, { hotspots: nextHs })}
...
// ─── Block Row ────────────────────────────────────────────────────
const BlockRow = ({
  block, index, total, readOnly, entityType, entityId,
  ...
  return (
    <article
      className={`ss-block ss-block--${block.type}`}
      data-testid={`ss-block-${index}`}
    ...
    >
      <header className="ss-block__head">
        ...
        {!readOnly && (
          <div className="ss-block__act">
            <button
              type="button" className="ss-mini" title="Sposta su"
              data-testid={`ss-up-${index}`}
              disabled={index === 0}
              onClick={onMoveUp}
            ><ChevronUp size={11} /></button>
            <button
              type="button" className="ss-mini" title="Sposta giù"
              data-testid={`ss-down-${index}`}
              disabled={index === total - 1}
              onClick={onMoveDown}
            ><ChevronDown size={11} /></button>
            <button
              type="button" className="ss-mini ss-mini--danger"
              data-testid={`ss-del-${index}`}
              title="Elimina blocco" onClick={onRemove}
      ...
      <div className="ss-block__body">
        {block.type === 'paragraph' && (
          <textarea
            ...
            rows={4}
            data-testid={`ss-text-${index}`}
            value={block.text || ''}
            onChange={(e) => onChange({ text: e.target.value })}
            placeholder="Paragrafo editoriale…"
          />
        )}
        {block.type === 'pull_quote' && (
          <>
            <textarea
              className="ss-textarea ss-textarea--quote"
              rows={2}
              data-testid={`ss-quote-${index}`}
              value={block.text || ''}
              onChange={(e) => onChange({ text: e.target.value })}
              placeholder="La frase che rimane."
            />
            <input
              className="ss-input ss-input--small"
              placeholder="— Attribuzione (opzionale)"
              data-testid={`ss-attribution-${index}`}
        ...
        {block.type === 'image' && (
          <>
            <EditorialMediaField
              ...
              onChange={(v) => onChange({
                url: v.url, asset_id: v.asset_id,
                alt_text: v.alt_text, caption: v.caption,
              })}
              preset="hero"
              entityType={entityType}
              entityId={entityId}
              role={`story_image_${index}`}
              testId={`ss-img-${index}`}
            />
          </>
        )}
        ...
        {block.type === 'hotspot_image' && (
          <>
            <EditorialMediaField
              ...
              entityType={entityType}
              entityId={entityId}
              role={`story_hotspot_${index}`}
              testId={`ss-himg-${index}`}
            />
            {block.url && (
        ...
        {block.type === 'cta' && (
          <div className="ss-cta-row">
            <select
              className="ss-select"
              data-testid={`ss-cta-tier-${index}`}
              value={block.tier || 'soft'}
            ...
            <input
              className="ss-input"
              placeholder="Copy editoriale CTA"
              data-testid={`ss-cta-label-${index}`}
              value={block.label || ''}
              onChange={(e) => onChange({ label: e.target.value })}
            />
            <select
              className="ss-select"
              data-testid={`ss-cta-action-${index}`}
              value={block.action || 'save_reference'}
...
const MiniGalleryEditor = ({ items, onChange, entityType, entityId, blockIndex }) => {
  const addItem = () => onChange([
    ...items,
    { id: `mg_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 5)}`,
      url: '', asset_id: null, caption: '', alt_text: '' },
  ]);
  ...
  return (
    <div className="ss-mg" data-testid={`ss-gallery-${blockIndex}`}>
      <div className="ss-mg-grid">
        {items.map((it, i) => (
          <div key={it.id} className="ss-mg-cell" data-testid={`ss-gallery-${blockIndex}-cell-${i}`}>
            <EditorialMediaField
              valueShape="object"
              value={{
                url: it.url || '',
              ...
              entityType={entityType}
              entityId={entityId}
              role={`gallery_block_${blockIndex}_${i}`}
            />
            <button
              type="button"
              className="ss-mg-rm"
              data-testid={`ss-gallery-${blockIndex}-rm-${i}`}
        ...
        <button
          type="button"
          className="ss-mg-add"
          data-testid={`ss-gallery-${blockIndex}-add`}
          onClick={addItem}
        ><Plus size={14} /> Aggiungi immagine</button>
      </div>
    </div>
  );
};
