...
const HotspotImageOverlay = ({
  ...
  hotspots = [],
  onChange,
  // (nextHotspots) => void  — in-memory mode
  mode = 'memory',
  // 'memory' | 'remote'
  remoteArticleId,
  remoteBlockId
  ...
  const memCreate = useCallback(async ({
    x_pct,
    y_pct,
    kind,
    title,
  ...
  const memUpdate = useCallback(async (id, patch) => {
    const next = hotspots.map(h => h.id === id ? {
      ...h,
      ...patch
    } : h);
    onChange?.(next);
  ...
  const remoteCreate = useCallback(async ({
    x_pct,
    y_pct,
    kind,
    title,
    ...
    setBusy(true);
    try {
      const r = await api.post(`/api/magazine/admin/articles/${remoteArticleId}/hotspots`, {
  ...
  if (!open) return null;
  return <div className="hsov-bg" data-testid="hotspot-overlay-bg" onClick={e => {
    if (e.target === e.currentTarget) onClose?.();
      ...
      <div className="hsov-modal" data-testid="hotspot-overlay" data-busy={busy}>
        <header className="hsov-modal__head">
          ...
          <button type="button" className="hsov-close" data-testid="hotspot-overlay-close" onClick={onClose}><X size={16} /></button>
        </header>
        <div className="hsov-modal__body">
          <HotspotEditor imageUrl={imageUrl} hotspots={hotspots} onCreate={isRemote ? remoteCreate : memCreate} onUpdate={isRemote ? remoteUpdate : memUpdate} onDelete={isRemote ? remoteDelete : memDelete} />
        </div>
        <footer className="hsov-modal__foot">
          ...
          <button type="button" className="hsov-btn" data-testid="hotspot-overlay-done" onClick={onClose}>{t('storytelling.hotspot_image_overlay.chiudi')}</button>
        </footer>
      </div>
    </div>;
};
