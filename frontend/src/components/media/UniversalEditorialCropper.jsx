...
export const EDITORIAL_FILTERS = [
  { key: '',                    label: 'Nessun filtro',           css: 'none' },
  { key: 'editorial_neutral',   label: 'Editorial Neutral',       css: 'contrast(1.04) saturate(0.95)' },
  { key: 'warm_residential',    label: 'Warm Residential',        css: 'sepia(0.10) saturate(1.10) brightness(1.03)' },
  { key: 'hospitality_glow',    label: 'Hospitality Glow',        css: 'sepia(0.18) saturate(1.18) brightness(1.06) contrast(1.02)' },
  { key: 'ad_contrast',         label: 'AD Contrast',             css: 'contrast(1.15) saturate(1.05) brightness(0.96)' },
  { key: 'soft_natural',        label: 'Soft Natural',            css: 'contrast(0.96) saturate(0.92) brightness(1.05)' },
  { key: 'material_focus',      label: 'Material Focus',          css: 'contrast(1.10) saturate(0.88) brightness(1.02)' },
  { key: 'cinematic_dark',      label: 'Cinematic Dark',          css: 'contrast(1.20) brightness(0.88) saturate(1.05)' },
];

// ── Anteprime safe-area (multi-device) ──────────────────────────────
export const SAFE_AREAS = [
  { key: 'hero',         label: 'Hero',           ratio: '16/9'  },
  { key: 'moodboard',    label: 'Moodboard',      ratio: '4/3'   },
  { key: 'card',         label: 'Editorial Card', ratio: '1/1'   },
  { key: 'mobile',       label: 'Mobile portrait', ratio: '9/16' },
  { key: 'cinematic',    label: 'Cinematic',      ratio: '21/9'  },
];

export const filterCssFor = (key) =>
  (EDITORIAL_FILTERS.find((f) => f.key === key) || EDITORIAL_FILTERS[0]).css;


const UniversalEditorialCropper = ({
  mediaId,
  imageUrl,
  initial,                  // { focal_x, focal_y, editorial_filter, crop_ratio, zoom }
  open,
  onClose,
  onSaved,                  // (display_meta) => void
}) => {
  const [focal, setFocal]     = useState({ x: initial?.focal_x ?? 0.5, y: initial?.focal_y ?? 0.5 });
  ...
  useEffect(() => {
    if (!open) return;
    setFocal({ x: initial?.focal_x ?? 0.5, y: initial?.focal_y ?? 0.5 });
  ...
  return (
    <div className="uec-backdrop" data-testid="universal-cropper"
      ...
      <div className="uec-modal">
        <header className="uec-head">
          ...
          <button type="button" className="uec-close" onClick={onClose}
                  data-testid="universal-cropper-close" aria-label="Chiudi">
            <Icons.X size={16} />
          </button>
        </header>
        ...
        <div className="uec-body">
          ...
          <section className="uec-stage-wrap">
            <p className="uec-section-label">
              <Icons.Target size={11} /> Trascina il punto focale sulla parte importante dell'immagine
            </p>
            <div
              ref={stageRef}
              className="uec-stage"
              data-testid="universal-cropper-stage"
            ...
            >
              ...
              <span
                className="uec-focal"
                data-testid="universal-cropper-focal"
          ...
          <aside className="uec-panel">
            <Section title="Filtro editoriale" icon={Icons.Wand2}>
              <div className="uec-filter-grid" data-testid="universal-cropper-filters">
                {EDITORIAL_FILTERS.map((f) => (
                  <button key={f.key || 'none'} type="button"
                          className={`uec-filter-tile ${filter === f.key ? 'is-on' : ''}`}
                          onClick={() => setFilter(f.key)}
                          data-testid={`universal-cropper-filter-${f.key || 'none'}`}>
                    <div className="uec-filter-tile__preview"
              ...
              <div className="uec-safe-toggle" data-testid="universal-cropper-safe-toggle">
                {SAFE_AREAS.map((s) => (
                  <button key={s.key} type="button"
                          className={`uec-safe-btn ${safe === s.key ? 'is-on' : ''}`}
                          onClick={() => setSafe(s.key)}>
                    {s.label}
                  </button>
                ))}
              </div>
              <div className="uec-safe-preview"
            ...
            <Section title="Proporzioni preferite" icon={Icons.Crop}>
              <div className="uec-ratio-row">
                {['auto', '16:9', '4:3', '1:1', '9:16', '21:9'].map((r) => (
                  <button key={r} type="button"
                          className={`uec-ratio-btn ${ratio === r ? 'is-on' : ''}`}
                          onClick={() => setRatio(r)}
                          data-testid={`universal-cropper-ratio-${r}`}>
                    {r}
                  </button>
                ))}
              </div>
              <p className="uec-hint">
                Questa è la proporzione che i consumer (Hero · Moodboard · Presentation)
                useranno quando possono scegliere.
              </p>
            </Section>
          </aside>
        </div>
        ...
        <footer className="uec-foot">
          <button type="button" className="uec-btn-soft" onClick={reset}
                  data-testid="universal-cropper-reset">
            <Icons.RotateCcw size={11} /> Ripristina
          </button>
          <span className="uec-foot__spacer" />
          <button type="button" className="uec-btn" onClick={save} disabled={saving}
                  data-testid="universal-cropper-save">
            {saving ? 'Salvataggio…' : <>Salva regia <Icons.Check size={12} /></>}
          </button>
        </footer>
      </div>
    </div>
