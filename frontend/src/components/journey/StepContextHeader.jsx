...
const StepContextHeader = ({
  ...
  const pct = progress ? Math.max(0, Math.min(100, (progress.current_step_index + 1) / Math.max(1, progress.total_steps) * 100)) : 0;
  ...
  return <header className="sw-context" data-testid="sw-context-header">
      ...
      <div className="sw-context__head">
        <div>
          ...
          {step.description && <p className="sw-context__lede">{step.description}</p>}

          <div className="sw-context__meta" style={{
          marginTop: 24
        }}>
            <span className={`sw-meta-pill`} data-testid="sw-meta-step-state">
              <span className={`sw-meta-pill__dot sw-meta-pill__dot--${stepTone === 'cool' ? 'cool' : stepTone === 'gold' ? 'gold' : stepTone === 'amber' ? 'amber' : 'muted'}`} />
              {step.status_label}
            </span>
            <span className="sw-meta-pill" data-testid="sw-meta-journey-state">
              <span className={`sw-meta-pill__dot sw-meta-pill__dot--${journeyTone === 'gold' ? 'gold' : journeyTone === 'amber' ? 'amber' : journeyTone === 'muted' ? 'muted' : 'cool'}`} />
              {journey?.lifecycle_label}
            </span>
          </div>
        </div>

        {progress && <div className="sw-context__progress" data-testid="sw-context-progress">
            <span className="sw-context__progress-label">Avanzamento</span>
            <div className="sw-context__progress-track">
              <div className="sw-context__progress-fill" style={{
            width: `${pct}%`
          }} />
            </div>
      ...
      {(prev_step || next_step) && <div className="sw-context__nav" data-testid="sw-context-nav">
          {prev_step ? <Link to={`/journey/${projectId}/step/${prev_step.milestone_type}`} className="sw-nav-chip" data-testid="sw-context-prev">
          ...
          {next_step && <Link to={`/journey/${projectId}/step/${next_step.milestone_type}`} className="sw-nav-chip" data-testid="sw-context-next">
              <span style={{
          textAlign: 'right'
        }}>
