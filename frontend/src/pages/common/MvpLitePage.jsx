...
const MvpLitePage = ({
  ...
  return <div data-testid={testid || 'mvp-lite-page'} className="min-h-[calc(100vh-3.5rem)] flex flex-col items-center justify-center bg-[var(--bp-bg)] px-10 py-16 text-center">
      ...
      {eyebrow && <p className="text-[10px] tracking-[0.3em] uppercase text-[var(--bp-primary)] font-body mb-3">
          {eyebrow}
        </p>}
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        {primaryCta && <Link to={primaryCta.to} data-testid={`${testid || 'mvp'}-primary-cta`} className="inline-flex items-center gap-2 px-5 py-2.5 bg-[var(--bp-primary)] hover:brightness-110
                       text-[var(--bp-primary-foreground,#0F0F10)] text-[10.5px] uppercase tracking-[0.22em]
                       transition-all">
            {primaryCta.label}
            <Icons.ArrowUpRight size={11} strokeWidth={1.6} />
          </Link>}
        {secondaryCta && <Link to={secondaryCta.to} data-testid={`${testid || 'mvp'}-secondary-cta`} className="inline-flex items-center gap-2 px-5 py-2.5 border border-[var(--bp-border)] hover:border-[var(--bp-border-strong)]
...
export const IntegrationsHub = () => <MvpLitePage testid="integrations-hub" icon="Plug" eyebrow="STUDIO · INTEGRAZIONI" title="Integrazioni esterne." body="Stripe (fatturazione), ElevenLabs (voce), Resend (email transazionali), Google Calendar: in arrivo come moduli opt-in. Le integrazioni AI editoriali (LLM) sono già attive." primaryCta={{
  to: '/settings',
