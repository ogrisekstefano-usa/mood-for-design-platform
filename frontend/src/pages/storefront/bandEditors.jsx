...
export const TraceabilityChip = ({ section_type }) => {
  ...
  return (
    <div className="ss-trace" data-testid={`ss-trace-${section_type}`}>
...
const TextField = ({ label, value, onChange, placeholder, testid, display }) => (
  <div className="ss-section">
    <p className="ss-section__label">{label}</p>
    <input
      className={`ss-input${display ? ' ss-input--display' : ''}`}
      data-testid={testid}
      value={value || ''}
...
const AreaField = ({ label, value, onChange, rows = 3, testid }) => (
  <div className="ss-section">
    <p className="ss-section__label">{label}</p>
    <textarea
...
// ───────────────────────────────────────────────────────────────────────
export const NavTopEditor = ({ section, localeTab, onPatchSetting }) => {
  const items = Array.isArray(section.settings?.links) ? section.settings.links : [];

  const update = (next) => onPatchSetting('links', next);

  const addLink = () => update([...items, { id: `link-${Date.now().toString(36)}`, href: '/', label_i18n: { [localeTab]: 'Nuova voce' }, visible: true }]);
  ...
  const patchLabel = (idx, locale, value) => {
    const copy = [...items];
    copy[idx] = { ...copy[idx], label_i18n: { ...(copy[idx].label_i18n || {}), [locale]: value } };
    update(copy);
  };

  return (
    <div className="ss-table" data-testid="ss-nav-top-editor">
      <div className="ss-table__head">
        <span style={{ flex: '0 0 28px' }}>#</span>
        <span style={{ flex: '0 0 130px' }}>ID</span>
        <span style={{ flex: 1 }}>Label ({localeTab})</span>
        <span style={{ flex: '0 0 180px' }}>Destination</span>
        <span style={{ flex: '0 0 70px' }}>Visible</span>
      ...
      {items.map((it, idx) => (
        <div className="ss-table__row" key={it.id || idx} data-testid={`ss-nav-row-${idx}`}>
          <span className="ss-table__order" style={{ flex: '0 0 28px' }}>{String(idx + 1).padStart(2, '0')}</span>
          <input className="ss-input ss-input--sm" style={{ flex: '0 0 130px' }}
            data-testid={`ss-nav-id-${idx}`}
            value={it.id || ''} onChange={(e) => patchField(idx, 'id', e.target.value)} placeholder="slug-id" />
          <input className="ss-input ss-input--sm" style={{ flex: 1 }}
            data-testid={`ss-nav-label-${idx}`}
            value={(it.label_i18n || {})[localeTab] || ''}
            onChange={(e) => patchLabel(idx, localeTab, e.target.value)} placeholder="Etichetta visibile" />
          <input className="ss-input ss-input--sm" style={{ flex: '0 0 180px' }}
            data-testid={`ss-nav-href-${idx}`}
            value={it.href || ''} onChange={(e) => patchField(idx, 'href', e.target.value)} placeholder="/projects" />
          <button type="button" className="ss-chip ss-chip--inline" style={{ flex: '0 0 70px' }}
            data-active={it.visible !== false}
            data-testid={`ss-nav-visible-${idx}`}
            onClick={() => patchField(idx, 'visible', it.visible === false)}>
            {it.visible !== false ? 'On' : 'Off'}
          </button>
          <div className="ss-table__actions" style={{ flex: '0 0 90px' }}>
      ...
      <button type="button" className="ss-btn ss-btn--ghost" onClick={addLink} data-testid="ss-nav-add">
        <Plus size={12} /> Aggiungi voce di menu
      </button>
    </div>
  );
};
...
// ───────────────────────────────────────────────────────────────────────
export const FooterColumnsEditor = ({ section, localeTab, onPatchSetting }) => {
  ...
  const addColumn = () => updateCols([...columns, { id: `col-${Date.now().toString(36)}`, title: { [localeTab]: 'Nuova colonna' }, links: [], visible: true }]);
  const removeColumn = (idx) => updateCols(columns.filter((_, i) => i !== idx));
  const moveColumn = (idx, dir) => {
    const j = idx + dir;
    if (j < 0 || j >= columns.length) return;
    const c = [...columns]; [c[idx], c[j]] = [c[j], c[idx]]; updateCols(c);
  };
  const patchCol = (idx, field, value) => {
    const c = [...columns]; c[idx] = { ...c[idx], [field]: value }; updateCols(c);
  };
  const patchColTitle = (idx, value) => {
    const c = [...columns];
    c[idx] = { ...c[idx], title: { ...(c[idx].title || {}), [localeTab]: value } };
    updateCols(c);
  };
  const addLink = (colIdx) => {
    const c = [...columns];
    c[colIdx] = { ...c[colIdx], links: [...(c[colIdx].links || []), { href: '/', label: { [localeTab]: 'Nuovo link' }, visible: true }] };
    updateCols(c);
  };
  const patchLink = (colIdx, linkIdx, field, value) => {
    ...
    c[colIdx] = { ...c[colIdx], links };
    updateCols(c);
  };
  const patchLinkLabel = (colIdx, linkIdx, value) => {
    const c = [...columns];
    const links = [...(c[colIdx].links || [])];
    links[linkIdx] = { ...links[linkIdx], label: { ...(links[linkIdx].label || {}), [localeTab]: value } };
    c[colIdx] = { ...c[colIdx], links };
    updateCols(c);
  };
  const removeLink = (colIdx, linkIdx) => {
    const c = [...columns];
    c[colIdx] = { ...c[colIdx], links: (c[colIdx].links || []).filter((_, i) => i !== linkIdx) };
    updateCols(c);
  };

  const addSocial = () => updateSocials([...socials, { id: 'instagram', href: 'https://instagram.com/' }]);
  const patchSocial = (idx, field, value) => {
    const s = [...socials]; s[idx] = { ...s[idx], [field]: value }; updateSocials(s);
  };
  ...
  return (
    <div className="ss-table" data-testid="ss-footer-cols-editor">
      ...
      {columns.map((col, idx) => (
        <div className="ss-footer-col-card" key={col.id || idx} data-testid={`ss-footer-col-${idx}`}>
          <div className="ss-footer-col-card__head">
            <input className="ss-input ss-input--sm" style={{ flex: '0 0 120px' }}
              value={col.id || ''} onChange={(e) => patchCol(idx, 'id', e.target.value)} placeholder="col-id" />
            <input className="ss-input ss-input--display ss-input--sm" style={{ flex: 1 }}
              value={(col.title || {})[localeTab] || ''}
              onChange={(e) => patchColTitle(idx, e.target.value)}
              placeholder={`Titolo colonna (${localeTab})`} />
            <button type="button" className="ss-chip ss-chip--inline"
              data-active={col.visible !== false}
              onClick={() => patchCol(idx, 'visible', col.visible === false)}>
          ...
          <div className="ss-footer-col-card__links">
            {(col.links || []).map((lk, j) => (
              <div className="ss-table__row ss-table__row--inset" key={j}>
                <input className="ss-input ss-input--sm" style={{ flex: 1 }}
                  value={(lk.label || {})[localeTab] || ''}
                  onChange={(e) => patchLinkLabel(idx, j, e.target.value)} placeholder="Label" />
                <input className="ss-input ss-input--sm" style={{ flex: '0 0 200px' }}
                  value={lk.href || ''}
                  onChange={(e) => patchLink(idx, j, 'href', e.target.value)} placeholder="/path or #anchor" />
      ...
      <button type="button" className="ss-btn ss-btn--ghost" onClick={addColumn} data-testid="ss-footer-col-add">
        <Plus size={12} /> Aggiungi colonna
      </button>

      <div className="ss-subhead" style={{ marginTop: 32 }}>Socials</div>
      {socials.map((s, idx) => (
        <div className="ss-table__row" key={idx}>
          <select className="ss-select ss-input--sm" style={{ flex: '0 0 140px' }}
...
// ───────────────────────────────────────────────────────────────────────
export const StatsBandEditor = ({ section, localeTab, onPatchLocale, onPatchSetting }) => {
  ...
  const addStat = () => updateStats([...items, { value: '100+', label_i18n: { [localeTab]: 'Nuovo KPI' } }]);
  const patchStat = (idx, field, value) => {
    const c = [...items]; c[idx] = { ...c[idx], [field]: value }; updateStats(c);
  };
  const patchLabel = (idx, value) => {
    const c = [...items];
    c[idx] = { ...c[idx], label_i18n: { ...(c[idx].label_i18n || {}), [localeTab]: value } };
    updateStats(c);
  };
  ...
  return (
    <>
      <TextField label="Eyebrow editoriale" value={localeData.eyebrow} testid="ss-field-eyebrow"
        onChange={(v) => onPatchLocale('eyebrow', v)} placeholder="Sopra-titolo" />
      <TextField label="Title cinematico" value={localeData.title} testid="ss-field-title" display
        onChange={(v) => onPatchLocale('title', v)} placeholder="L'esperienza in numeri" />
      <AreaField label="Body" value={localeData.body} testid="ss-field-body" rows={2}
        onChange={(v) => onPatchLocale('body', v)} />

      <div className="ss-section">
        <p className="ss-section__label">KPI Items ({items.length})</p>
        {items.map((it, idx) => (
          <div className="ss-table__row" key={idx}>
            <input className="ss-input ss-input--sm ss-input--display" style={{ flex: '0 0 140px' }}
              data-testid={`ss-stat-value-${idx}`}
              value={it.value || ''} onChange={(e) => patchStat(idx, 'value', e.target.value)} placeholder="100+" />
            <input className="ss-input ss-input--sm" style={{ flex: 1 }}
              data-testid={`ss-stat-label-${idx}`}
              value={(it.label_i18n || {})[localeTab] || ''}
              onChange={(e) => patchLabel(idx, e.target.value)} placeholder={`Etichetta (${localeTab})`} />
...
// ───────────────────────────────────────────────────────────────────────
export const BrandLogosEditor = ({ section, localeTab, onPatchLocale, onPatchSetting }) => {
  const localeData = (section.locale_content || {})[localeTab] || {};
  const items = Array.isArray(section.settings?.logos) ? section.settings.logos : [];

  const updateLogos = (next) => onPatchSetting('logos', next);
  const addLogo = () => updateLogos([...items, { name: 'Brand', logo_url: '', href: '' }]);
  const patch = (idx, field, value) => {
    const c = [...items]; c[idx] = { ...c[idx], [field]: value }; updateLogos(c);
  };
  const remove = (idx) => updateLogos(items.filter((_, i) => i !== idx));

  return (
    <>
      <TextField label="Eyebrow" value={localeData.eyebrow} testid="ss-field-eyebrow"
        onChange={(v) => onPatchLocale('eyebrow', v)} placeholder="Trusted by" />
      <TextField label="Title" value={localeData.title} testid="ss-field-title" display
        onChange={(v) => onPatchLocale('title', v)} placeholder="Le case che ci scelgono" />
...
// ───────────────────────────────────────────────────────────────────────
export const MagazineGridEditor = ({ section, localeTab, onPatchLocale, onPatchSetting }) => {
  const localeData = (section.locale_content || {})[localeTab] || {};
  ...
  return (
    <>
      <TextField label="Eyebrow" value={localeData.eyebrow} testid="ss-field-eyebrow"
        onChange={(v) => onPatchLocale('eyebrow', v)} placeholder="Journal" />
      <TextField label="Title cinematico" value={localeData.title} testid="ss-field-title" display
        onChange={(v) => onPatchLocale('title', v)} placeholder="Stories from the studio" />
      <AreaField label="Intro body" value={localeData.body} testid="ss-field-body" rows={2}
        onChange={(v) => onPatchLocale('body', v)} />
      ...
      <div className="ss-section">
        ...
        <input className="ss-input" type="number" min={1} max={12}
          data-testid="ss-mag-limit"
          value={limit} onChange={(e) => onPatchSetting('limit', parseInt(e.target.value || '3', 10))} />
      </div>

      {mode === 'manual' && (
        <div className="ss-section">
          <p className="ss-section__label">Slug articoli (uno per riga)</p>
          <textarea className="ss-textarea" rows={4}
            data-testid="ss-mag-slugs"
            value={(section.settings?.slugs || []).join('\n')}
            onChange={(e) => onPatchSetting('slugs', e.target.value.split('\n').map((s) => s.trim()).filter(Boolean))} />
        </div>
      )}

      <TextField label="CTA · label" value={localeData.cta_label} testid="ss-field-cta-label"
        onChange={(v) => onPatchLocale('cta_label', v)} placeholder="Leggi tutti gli articoli" />
      <TextField label="CTA · destination" value={localeData.cta_href} testid="ss-field-cta-href"
        onChange={(v) => onPatchLocale('cta_href', v)} placeholder="/magazine" />
    </>
  );
};
...
// ───────────────────────────────────────────────────────────────────────
export const NewsletterEditor = ({ section, localeTab, onPatchLocale }) => {
  const localeData = (section.locale_content || {})[localeTab] || {};
  return (
    <>
      <TextField label="Title" value={localeData.title} testid="ss-field-title" display
        onChange={(v) => onPatchLocale('title', v)} placeholder="Ispirazione e novità" />
      <AreaField label="Body editoriale" value={localeData.body} testid="ss-field-body" rows={2}
        onChange={(v) => onPatchLocale('body', v)} />
      <TextField label="Placeholder input" value={localeData.placeholder} testid="ss-field-placeholder"
        onChange={(v) => onPatchLocale('placeholder', v)} placeholder="La tua email" />
      <TextField label="CTA label" value={localeData.cta} testid="ss-field-cta-label"
        onChange={(v) => onPatchLocale('cta', v)} placeholder="Iscriviti" />
      <TextField label="Success message" value={localeData.success} testid="ss-field-success"
        onChange={(v) => onPatchLocale('success', v)} placeholder="Grazie per esserti iscritto." />
    </>
  );
};
...
// ───────────────────────────────────────────────────────────────────────
export const DualCtaEditor = ({ section, localeTab, onPatchLocale }) => {
  const localeData = (section.locale_content || {})[localeTab] || {};

  const card = (kind, label) => (
    <div className="ss-dual-card-block" data-testid={`ss-dualcta-${kind}`}>
      <p className="ss-subhead">{label}</p>
      <TextField label="Eyebrow" value={localeData[`${kind}_eyebrow`]}
        onChange={(v) => onPatchLocale(`${kind}_eyebrow`, v)}
        testid={`ss-dual-${kind}-eyebrow`}
        placeholder={kind === 'private' ? 'Sei un privato?' : 'Sei un professionista?'} />
      <TextField label="Title" value={localeData[`${kind}_title`]} display
        onChange={(v) => onPatchLocale(`${kind}_title`, v)}
        testid={`ss-dual-${kind}-title`}
        placeholder={kind === 'private' ? 'Inizia il tuo progetto' : 'Collabora con noi'} />
      <AreaField label="Body" value={localeData[`${kind}_body`]} rows={3}
        onChange={(v) => onPatchLocale(`${kind}_body`, v)}
        testid={`ss-dual-${kind}-body`} />
      <TextField label="CTA label" value={localeData[`${kind}_cta`]}
        onChange={(v) => onPatchLocale(`${kind}_cta`, v)}
        testid={`ss-dual-${kind}-cta`} />
      <TextField label="CTA destination" value={localeData[`${kind}_href`]}
...
export const renderBandEditor = (props) => {
  ...
  switch (section.section_type) {
    case 'footer_columns':  return <FooterColumnsEditor {...props} />;
    case 'stats_band':      return <StatsBandEditor {...props} />;
    case 'brand_logos':     return <BrandLogosEditor {...props} />;
    case 'magazine_grid':
    case 'journal_intro':   return <MagazineGridEditor {...props} />;
    case 'newsletter':      return <NewsletterEditor {...props} />;
    case 'dual_cta':        return <DualCtaEditor {...props} />;
    default:                return null; // fallback to generic cinematic editor
  }
};
