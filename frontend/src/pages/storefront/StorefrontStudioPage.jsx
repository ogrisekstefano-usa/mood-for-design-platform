...
const BAND_META = {
  store_hero:           { title: 'Hero',                  intro: 'L\'apertura cinematica del sito pubblico.' },
  hero:                 { title: 'Hero',                  intro: 'L\'apertura cinematica del sito pubblico.' },
  ...
  magazine_grid:        { title: 'Journal · Magazine',    intro: 'L\'introduzione editoriale al magazine.' },
  journal_intro:        { title: 'Journal · Magazine',    intro: 'L\'introduzione editoriale al magazine.' },
...
const POSITIONING_MODES = [
  { v: 'domestic_authority',         label: 'Domestic Luxury Authority' },
  { v: 'international_editorial',    label: 'International Editorial Positioning' },
  { v: 'hospitality_specialist',     label: 'Hospitality Contract Specialist' },
  { v: 'residential_advisory',       label: 'Luxury Residential Advisory' },
  { v: 'material_consultancy',       label: 'Material Consultancy' },
  { v: 'ad_specification_partner',   label: 'A&D Specification Partner' },
  { v: 'collectible_bespoke',        label: 'Collectible Design & Bespoke' },
];

const HERO_VARIANTS = [
  { v: 'cinematic',         label: 'Cinematic' },
  { v: 'editorial_static',  label: 'Editorial Static' },
  { v: 'slow_reveal',       label: 'Slow Reveal' },
  { v: 'immersive',         label: 'Immersive Fullscreen' },
  { v: 'split_narrative',   label: 'Split Narrative' },
  { v: 'material_focused',  label: 'Material Focused' },
];
...
const StorefrontStudioPage = () => {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      setLoading(true); setSelected(null); setDirty(false);
      try {
        const [pageR, mkR] = await Promise.all([
          api.get(`/api/storefront/admin/pages/${activePage}`),
  ...
  const onDrop      = (targetId) => {
    ...
    const renumbered = list.map((s, i) => ({ ...s, sort_order: (i + 1) * 10 }));
  ...
  const toggleVisible = async (s) => {
    try {
      const r = await api.put(`/api/storefront/admin/sections/${s.id}`, { visible: !s.visible });
      setSections((list) => list.map((x) => x.id === s.id ? { ...x, ...r.data } : x));
      if (selected?.id === s.id) setSelected({ ...selected, ...r.data });
  ...
  const saveSelected = async () => {
    ...
    try {
      ...
      setSections((list) => list.map((x) => x.id === selected.id ? { ...x, ...r.data } : x));
    ...
    <div className="ss-root" data-no-edit={!selected} data-testid="ss-root">
          ...
          {sections.sort((a, b) => a.sort_order - b.sort_order).map((s, i) => {
            ...
            return (
              <article
                key={s.id}
                className="ss-band"
                data-testid={`ss-band-${s.section_type}`}
              ...
              >
                  ...
                  <div className="ss-band__meta">
                    {pm && (
                      <span className="ss-meta-chip">
                        <span className="ss-meta-chip__dot" style={{ backgroundColor: 'var(--bp-primary)' }} />
                        {POSITIONING_MODES.find((p) => p.v === pm)?.label || pm}
                      </span>
                    )}
                    {hv && (
                      <span className="ss-meta-chip">
                        <span className="ss-meta-chip__dot" style={{ backgroundColor: '#c8a572' }} />
                ...
                <div className="ss-band__actions">
                  <button type="button" className="ss-eye-btn"
                    data-testid={`ss-toggle-${s.section_type}`}
      ...
      {selected && (
        <BandEditor
          ...
          dirty={dirty}
          saving={saving}
          onPatch={onSectionPatch}
          onSave={saveSelected}
          onClose={() => { setSelected(null); setDirty(false); }}
        />
      )}
    </div>
  );
...
const BandEditor = ({ section, markets, activeLocales, dirty, saving, onPatch, onSave, onClose }) => {
  ...
  const patchLocale = (field, value) => {
    const next = { ...localeContent, [localeTab]: { ...localeData, [field]: value } };
    onPatch({ locale_content: next });
  };
  const patchSetting = (field, value) => {
    onPatch({ settings: { ...settings, [field]: value } });
  };
  const toggleMarket = (code) => {
    ...
    if (code === 'all') next = ['all'];
      ...
      if (cur.includes(code)) next = without.length === 0 ? ['all'] : without;
      else next = [...without, code];
    }
    patchSetting('market_visibility', next);
  };

  return (
    <aside className="ss-editor" data-testid="ss-editor">
      ...
      <div className="ss-locale-tabs" data-testid="ss-locale-tabs">
        {(activeLocales.length > 0 ? activeLocales : ['it-IT']).map((loc) => (
          <button key={loc} type="button"
            className="ss-locale-tab"
            data-active={localeTab === loc}
            data-testid={`ss-locale-tab-${loc}`}
      ...
      <div className="ss-section">
        ...
        <div className="ss-multi-chips" data-testid="ss-market-chips">
          <button type="button" className="ss-chip"
            data-active={(settings.market_visibility || ['all']).includes('all')}
            data-testid="ss-market-all"
            onClick={() => toggleMarket('all')}>Global · all markets</button>
          {markets.map((m) => (
            <button key={m.id} type="button" className="ss-chip"
              data-active={(settings.market_visibility || ['all']).includes(m.code)}
              data-testid={`ss-market-${m.code}`}
              onClick={() => toggleMarket(m.code)}>{m.code}</button>
          ))}
        </div>
      </div>
      ...
      <div className="ss-section">
        ...
        <select className="ss-select"
          data-testid="ss-positioning-mode"
          value={settings.positioning_mode || ''}
          onChange={(e) => patchSetting('positioning_mode', e.target.value || null)}>
      ...
      {isHero && (
        <div className="ss-section">
          ...
          <select className="ss-select"
            data-testid="ss-hero-variant"
            value={settings.hero_variant || ''}
            onChange={(e) => patchSetting('hero_variant', e.target.value || null)}>
