...
const TABS = [{
  ...
  key: 'market',
  label: 'Market Editions'
}];
const slugify = s => (s || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');

// Editorial loading lines — never "AI generated".
const COMPOSING_LINES = ['Reinterpreting the project for this market…', 'Calibrating material vocabulary…', 'Reframing hospitality tone…', 'Composing market-native cadence…'];
const ProjectsStudioPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const [pR, mR] = await Promise.all([api.get('/api/portfolio/admin/projects'), api.get('/api/tenants/me/markets')]);
        if (!alive) return;
  ...
  const onVariantChange = patch => {
    setDetail(d => {
      const list = d.variants.map(v => v.id === activeVariant.id ? {
        ...v,
        ...patch
      } : v);
      return {
  ...
  const onPaletteAdd = value => {
    ...
    if (!v) return;
    onMasterChange({
      material_palette: [...(master.material_palette || []), v]
    });
  };
  const onPaletteRemove = idx => {
    ...
    list.splice(idx, 1);
    onMasterChange({
      material_palette: list
    });
  };

  // ── Render ─────────────────────────────────────────────────
  return <div className="ps-root" data-no-master={!master} data-testid="ps-root">
      {/* LEFT — MASTER ARCHIVE */}
      <aside className="ps-rail" data-testid="ps-rail">
        <header className="ps-rail__head">
          ...
          <button type="button" className="ps-rail__new" data-testid="ps-new-master" onClick={() => setShowNewModal(true)}>{t('projects.projects_studio.apri_un_nuovo_progetto')}</button>
        </header>
        ...
        <div className="ps-masters" data-testid="ps-masters">
          ...
          fontStyle: 'italic'
        }}>{t('projects.projects_studio.caricamento_archivio')}</p>}
          {!loading && masters.length === 0 && <p style={{
          color: 'var(--bp-text-muted)',
          fontStyle: 'italic',
          ...
          return <article key={m.id} className="ps-card" data-selected={selectedId === m.id} data-testid={`ps-card-${m.slug}`} onClick={() => setSelectedId(m.id)}>
                <div className={m.cover_image_url ? 'ps-card__cover' : 'ps-card__cover ps-card__cover--empty'}>
                  {m.cover_image_url ? <img src={m.cover_image_url} alt={m.title} /> : <span>{t('projects.projects_studio.nessuna_immagine')}</span>}
                </div>
                <div className="ps-card__body">
                  ...
                  <div className="ps-card__meta">
                    <span className="ps-card__meta-dot" data-status={m.is_published ? 'published' : 'draft'}>
                      {m.is_published ? t('projects.studio.status_live', null, 'Live') : t('taxonomy.editorial_status.draft', null, 'Draft')}
                    </span>
      ...
      <main className="ps-stage" data-testid="ps-stage">
        ...
        {master && <>
            <header className="ps-stage__head">
              ...
              <div className="ps-stage__actions">
                <span className="ps-status-pill" data-testid="ps-master-status">
                  ...
                  {master.is_published ? t('projects.studio.status_live', null, 'Live') : t('taxonomy.editorial_status.draft', null, 'Draft')}
                </span>
                <button type="button" className="ps-btn" data-testid="ps-publish-master" disabled={master.is_published} onClick={publishMaster}>{t('projects.projects_studio.pubblica_progetto')}</button>
              </div>
            </header>

            <nav className="ps-tabs" data-testid="ps-tabs">
              {TABS.map(t => <button key={t.key} type="button" className="ps-tab" data-active={tab === t.key} data-testid={`ps-tab-${t.key}`} onClick={() => setTab(t.key)}>{t.label}</button>)}
            </nav>

            {tab === 'master' && <MasterStoryEditor master={master} onChange={onMasterChange} onPaletteAdd={onPaletteAdd} onPaletteRemove={onPaletteRemove} onSave={saveMaster} dirty={dirty} saving={saving} />}
            ...
            {tab === 'market' && <>
              ...
              return <button key={m.id} type="button" className="ps-market-tab" data-active={activeMarketId === m.id} data-status={status} data-testid={`ps-market-${m.code}`} onClick={() => setActiveMarketId(m.id)}>
                ...
                {activeMarket && <MarketEditionEditor master={master} market={activeMarket} variant={activeVariant} onChange={onVariantChange} onSave={saveVariant} onPublish={publishVariant} onCompose={composeMarketEdition} dirty={dirty} saving={saving} />}
              </>}
          </>}
      </main>

      {composing && <div className="ps-composing-overlay" data-testid="ps-composing">
          <p className="ps-composing-overlay__eyebrow">Compose Market Edition</p>
          <p className="ps-composing-overlay__line">{composingLine}</p>
        </div>}

      {showNewModal && <div className="ps-modal-bg" onClick={() => setShowNewModal(false)}>
          <div className="ps-modal" onClick={e => e.stopPropagation()} data-testid="ps-new-modal">
            ...
            <div className="ps-section">
              ...
              <input className="ps-input ps-input--display" data-testid="ps-new-title" value={draft.title} onChange={e => setDraft({
            ...draft,
            ...
            <div className="ps-grid-2">
              <div className="ps-section">
                ...
                <select className="ps-select" data-testid="ps-new-category" value={draft.category} onChange={e => setDraft({
              ...draft,
              ...
              <div className="ps-section">
                ...
                <input className="ps-input" value={draft.location} onChange={e => setDraft({
              ...draft,
            ...
            <div className="ps-section">
              ...
              <input className="ps-input" value={draft.client} onChange={e => setDraft({
            ...draft,
...
const MasterStoryEditor = ({
  ...
  return <section data-testid="ps-master-tab">
      <div className="ps-grid-2">
        <div className="ps-section">
          <p className="ps-section__label">Titolo</p>
          <input className="ps-input ps-input--display" data-testid="ps-master-title" value={master.title || ''} onChange={e => onChange({
          title: e.target.value
        })} />
        </div>
        <div className="ps-section">
          <p className="ps-section__label">{t('projects.projects_studio.sottotitolo_editoriale')}</p>
          <input className="ps-input" value={master.subtitle || ''} onChange={e => onChange({
      ...
      <div className="ps-grid-3">
        <div className="ps-section">
          <p className="ps-section__label">Categoria</p>
          <select className="ps-select" value={master.category || ''} onChange={e => onChange({
          category: e.target.value
        })}>
            <option value="">—</option>
            <option value="residential">Residenziale</option>
        ...
        <div className="ps-section">
          <p className="ps-section__label">Cliente</p>
          <input className="ps-input" value={master.client || ''} onChange={e => onChange({
          client: e.target.value
        })} />
        </div>
        <div className="ps-section">
          <p className="ps-section__label">Località</p>
          <input className="ps-input" value={master.location || ''} onChange={e => onChange({
          location: e.target.value
        })} />
        </div>
      </div>
      ...
      <div className="ps-grid-2">
        <div className="ps-section">
          ...
          <input type="number" className="ps-input" value={master.year || ''} onChange={e => onChange({
          year: e.target.value
        })} />
        </div>
        <div className="ps-section">
          <p className="ps-section__label">Locale canonico</p>
          <select className="ps-select" value={master.default_locale || 'it-IT'} onChange={e => onChange({
      ...
      })} preset="hero" entityType="portfolio_project" entityId={master.id} role="cover" testId="ps-cover-media" helperText="L'immagine che apre la case history e diventa la cover di archive." />
      </div>

      <ProjectGalleryEditor gallery={master.gallery || []} coverUrl={master.cover_image_url || ''} onChange={next => onChange({
      gallery: next
    })} onSetCover={url => onChange({
      cover_image_url: url
    })} entityType="portfolio_project" entityId={master.id} testId="ps-project-gallery" />
      ...
      <div className="ps-section">
        ...
        <div className="ps-chips" data-testid="ps-material-palette">
          {(master.material_palette || []).map((p, i) => <span key={i} className="ps-chip">
              {p}
              <button type="button" onClick={() => onPaletteRemove(i)} aria-label="rimuovi">×</button>
            </span>)}
          <input className="ps-input" style={{
          maxWidth: 220,
          padding: '6px 12px',
          fontSize: 12
        }} value={paletteInput} onChange={e => setPaletteInput(e.target.value)} onKeyDown={e => {
          if (e.key === 'Enter') {
            e.preventDefault();
            onPaletteAdd(paletteInput);
            setPaletteInput('');
          }
      ...
      <StorySectionsEditor blocks={master.story_body || []} onChange={next => onChange({
      story_body: next
    })} hotspotMode="memory" entityType="portfolio_project" entityId={master.id} testId="ps-master-story" />
      ...
      gap: 10
        ...
        <button type="button" className="ps-btn ps-btn--primary" data-testid="ps-save-master" disabled={!dirty || saving} onClick={onSave}>{saving ? 'Salvataggio…' : 'Salva master story'}</button>
      </div>
    </section>;
};

// ── Market Edition Editor (subcomponent) ──────────────────────
const MarketEditionEditor = ({
  master,
  market,
  variant,
  onChange,
  ...
  if (!variant) {
    return <section data-testid="ps-market-empty">
        ...
        <button type="button" className="ps-btn ps-btn--primary" data-testid="ps-compose" onClick={onCompose}>Compose Market Edition</button>
      </section>;
  }
  return <section data-testid="ps-market-tab">
      <div style={{
      display: 'flex',
      ...
      marginBottom: 24
        ...
        gap: 10
          ...
          <button type="button" className="ps-btn ps-btn--primary" data-testid="ps-publish-variant" disabled={v.is_published || !master.is_published} onClick={onPublish}>Publish International Edition</button>
        </div>
      </div>

      <div className="ps-section">
        <p className="ps-section__label">{t('projects.projects_studio.titolo_per_il_mercato')}</p>
        <input className="ps-input ps-input--display" data-testid="ps-variant-title" value={v.variant_title || ''} onChange={e => onChange({
        variant_title: e.target.value
      })} />
      </div>

      <div className="ps-section">
        <p className="ps-section__label">Cultural angle · l'apertura emotiva</p>
        <textarea className="ps-textarea" data-testid="ps-cultural-angle" value={v.cultural_angle || ''} onChange={e => onChange({
        cultural_angle: e.target.value
      })} />
      </div>
      ...
      <div className="ps-grid-2">
        <div className="ps-section">
          ...
          <textarea className="ps-textarea" value={v.hospitality_tone || ''} onChange={e => onChange({
          hospitality_tone: e.target.value
        })} />
        </div>
        <div className="ps-section">
          <p className="ps-section__label">Luxury perception</p>
          <textarea className="ps-textarea" value={v.luxury_perception || ''} onChange={e => onChange({
          luxury_perception: e.target.value
        })} />
        </div>
      </div>
      ...
      <div className="ps-section">
        ...
        <textarea className="ps-textarea" value={v.aspirational_narrative || ''} onChange={e => onChange({
        aspirational_narrative: e.target.value
      })} />
      </div>
      ...
      <div className="ps-section">
        ...
        <div className="ps-grid-3">
          <input className="ps-input" data-testid="ps-material-primary" value={ml.primary || ''} placeholder="Materia primaria" onChange={e => onChange({
          material_language: {
            ...ml,
            primary: e.target.value
          }
        })} />
          <input className="ps-input" value={ml.secondary || ''} placeholder="Materia secondaria" onChange={e => onChange({
          material_language: {
            ...ml,
            secondary: e.target.value
          }
        })} />
          <input className="ps-input" value={ml.tactile || ''} placeholder="Tattilità" onChange={e => onChange({
          material_language: {
            ...ml,
      ...
      <div className="ps-section">
        ...
        <StorySectionsEditor blocks={v.story_body || []} onChange={next => onChange({
        story_body: next
      })} hotspotMode="memory" entityType="portfolio_variant" entityId={v.id} testId="ps-variant-story" />
      </div>

      <div className="ps-section">
        <p className="ps-section__label">CTA market-native</p>
        {ctaSet.map((c, i) => <div key={i} className="ps-grid-3" style={{
        marginBottom: 10
      }}>
            <select className="ps-select" value={c.tier || 'soft'} onChange={e => {
          const next = [...ctaSet];
          next[i] = {
            ...c,
            tier: e.target.value
          ...
          onChange({
            ...
            <input className="ps-input" value={c.label || ''} placeholder="Etichetta CTA market-native" onChange={e => {
          const next = [...ctaSet];
          next[i] = {
            ...c,
          ...
          onChange({
            ...
            <select className="ps-select" value={c.action || 'book_consultation'} onChange={e => {
          const next = [...ctaSet];
          next[i] = {
            ...c,
        ...
        <button type="button" className="ps-btn ps-btn--ghost" onClick={() => onChange({
        cta_set: [...ctaSet, {
          tier: 'soft',
          label: '',
      ...
      <div className="ps-grid-2">
        <div className="ps-section">
          ...
          <input className="ps-input" value={seo.seo_title || ''} onChange={e => onChange({
          seo: {
            ...seo,
        ...
        <div className="ps-section">
          ...
          <input className="ps-input" value={seo.focus_intent || ''} onChange={e => onChange({
          seo: {
            ...seo,
            focus_intent: e.target.value
          }
        })} />
        </div>
      </div>
      ...
      <div className="ps-section">
        ...
        <textarea className="ps-textarea" value={seo.meta_description || ''} onChange={e => onChange({
        seo: {
          ...seo,
      ...
      gap: 10
        ...
        <button type="button" className="ps-btn ps-btn--primary" data-testid="ps-save-variant" disabled={!dirty || saving} onClick={onSave}>{saving ? 'Salvataggio…' : 'Salva market edition'}</button>
      </div>
    </section>;
};
