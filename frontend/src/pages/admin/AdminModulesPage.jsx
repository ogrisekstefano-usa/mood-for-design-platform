  ...
  return (
    <div className="p-10 max-w-5xl mx-auto" data-testid="admin-modules-page">
      <div className="mb-10">
        ...
        <h1 className="font-heading text-5xl font-light text-[#EFEBE4]">{t('admin.modules.title', null, 'Module Registry')}</h1>
        <p className="text-[#6B6863] text-sm font-body mt-2">{t('admin.modules.sub', null, 'The platform-wide Blueprint module catalog. Activate per tenant from the tenant detail.')}</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {modules.map((m) => {
          const Icon = Icons[m.icon] || Icons.Box;
          return (
            <div key={m.id} className="bg-[#0F0F11] border border-white/[0.06] rounded-md p-5">
              <div className="flex items-start gap-4 mb-3">
                ...
                <div className="flex-1">
                  <h3 className="font-heading text-xl text-[#EFEBE4] mb-1 capitalize">{m.id}</h3>
                  <p className="text-[#6B6863] text-xs font-body">{m.description}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 text-[10px] text-[#4A4845] font-body uppercase tracking-wider">
