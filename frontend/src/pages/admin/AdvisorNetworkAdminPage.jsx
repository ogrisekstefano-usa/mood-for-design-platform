...
import {
  Users, Plus, ArrowUpRight, AlertTriangle, Award, MapPin, Copy, X,
} from 'lucide-react';
...
const AdvisorNetworkAdminPage = () => {
  ...
  const load = async () => {
    ...
    try {
      const [ov, adv] = await Promise.all([
        api.get('/api/advisor/admin/overview'),
        api.get('/api/advisor/admin/advisors'),
      ]);
      setOverview(ov.data);
  ...
  return (
    <div className="adv-page" data-testid="advisor-network-admin">
      ...
      {overview && (
        <div className="adv-pulse" data-testid="adv-pulse">
      ...
      <div className="adv-toolbar">
        ...
        <button type="button" className="adv-btn adv-btn--primary"
                onClick={() => setShowNew(true)}
                data-testid="adv-new-btn">
      ...
      {!loading && advisors.length === 0 && (
        <div className="adv-empty" data-testid="adv-empty">
      ...
      {!loading && advisors.length > 0 && (
        <div className="adv-cards" data-testid="adv-cards">
          {advisors.map((a) => {
            const pal = avatarPalette(a.name);
            return (
              <button key={a.id} type="button"
                      className="adv-card"
                      data-testid={`adv-card-${a.id}`}
                      onClick={() => nav(`/admin/advisors/${a.id}`)}>
                ...
                <div className="adv-card__stats">
                  ...
                  {a._at_risk > 0 && (
                    <span className="adv-card__warn"><AlertTriangle size={10} /> {a._at_risk} da supportare</span>
                  )}
      ...
      {showNew && <NewAdvisorDrawer onClose={() => { setShowNew(false); load(); }} />}
    </div>
  );
};

const PulseCell = ({ num, total, lbl, accent, warn }) => (
...
const NewAdvisorDrawer = ({ onClose }) => {
  const [form, setForm] = useState({
    name: '', email: '', phone: '', territory: '',
    commission_percentage: 15, default_discount_percentage: 10,
  });
    ...
    <div className="adv-drawer-bg" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <aside className="adv-drawer" data-testid="adv-new-drawer">
        ...
        <div className="adv-drawer__body">
          <Field label="Nome completo" value={form.name} onChange={(v) => setForm({ ...form, name: v })} testid="adv-new-name" />
          <Field label="Email" value={form.email} onChange={(v) => setForm({ ...form, email: v })} testid="adv-new-email" type="email" />
          <Field label="Telefono" value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} testid="adv-new-phone" />
          <Field label="Territorio" value={form.territory} onChange={(v) => setForm({ ...form, territory: v })} placeholder="Lombardia · IT" testid="adv-new-terr" />
          <div className="adv-grid-2">
            <Field label="Commissione %" value={form.commission_percentage}
                   onChange={(v) => setForm({ ...form, commission_percentage: parseFloat(v) || 0 })}
                   testid="adv-new-comm" type="number" />
            <Field label="Sconto default %" value={form.default_discount_percentage}
                   onChange={(v) => setForm({ ...form, default_discount_percentage: parseFloat(v) || 0 })}
...
const Field = ({ label, value, onChange, type = 'text', placeholder, testid }) => (
  <label className="adv-field">
    ...
    <input type={type} className="adv-field__input"
