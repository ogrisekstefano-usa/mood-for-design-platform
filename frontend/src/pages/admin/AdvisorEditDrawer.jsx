...
const ChipInput = ({ label, value, onChange, placeholder, testid }) => {
  ...
  const add = () => {
    ...
    onChange([...arr, v]); setDraft('');
  };
  return (
    <div className="aed-chip-field">
      <label className="aed-label">{label}</label>
      <div className="aed-chips" data-testid={testid}>
        {arr.map((t) => (
          <span key={t} className="aed-chip">
            {t}
            <button type="button" onClick={() => onChange(arr.filter((x) => x !== t))}
        ...
        <input type="text" value={draft}
               ...
               className="aed-chip-input" />
      </div>
    </div>
  );
};

const AdvisorEditDrawer = ({ open, advisor, onClose, onSaved }) => {
  ...
  return (
    <div className="aed-backdrop" data-testid="advisor-edit-drawer-backdrop"
      ...
      <aside className="aed-drawer" data-testid="advisor-edit-drawer">
        <header className="aed-head">
          ...
          <button type="button" className="aed-close"
                  onClick={onClose} aria-label="Chiudi"
                  data-testid="advisor-edit-drawer-close">
            <X size={18} />
          </button>
        </header>
        ...
        <div className="aed-body">
            ...
            <div className="aed-grid">
              <div className="aed-field">
                ...
                <input type="number" min="0" max="50" step="0.5"
                       ...
                       data-testid="aed-commission" className="aed-input" />
              </div>
              <div className="aed-field">
                <label className="aed-label">Sconto default %</label>
                <input type="number" min="0" max="50" step="0.5"
                       value={form.default_discount_percentage ?? ''}
                       onChange={(e) => set('default_discount_percentage', e.target.value)}
                       data-testid="aed-discount" className="aed-input" />
              </div>
              <div className="aed-field">
                <label className="aed-label">Ciclo payout (mesi)</label>
                <input type="number" min="1" max="24" step="1"
                       value={form.payout_cycle_months ?? ''}
                       onChange={(e) => set('payout_cycle_months', e.target.value)}
                       data-testid="aed-payout" className="aed-input" />
              </div>
              <div className="aed-field">
                <label className="aed-label">Mesi qualificati minimi</label>
                <input type="number" min="1" max="24" step="1"
                       value={form.minimum_qualified_months ?? ''}
                       onChange={(e) => set('minimum_qualified_months', e.target.value)}
                       data-testid="aed-qualified" className="aed-input" />
              </div>
            </div>
          </section>
          ...
          <section className="aed-section">
            ...
            <ChipInput label="Specializzazione di mercato"
                       ...
                       placeholder="es. Hospitality · Yacht client · Milano"
                       testid="aed-specialization" />
            <ChipInput label="Tag relazionali"
                       value={form.relationship_tags}
                       onChange={(v) => set('relationship_tags', v)}
                       placeholder="es. consigliere · architettura · partner editoriale"
                       testid="aed-tags" />
            <div className="aed-field">
              <label className="aed-label">Note interne</label>
              <textarea value={form.notes || ''}
                        onChange={(e) => set('notes', e.target.value)}
                        placeholder="Contesto, storia, sensibilità della relazione…"
                        rows={4}
                        data-testid="aed-notes" className="aed-input aed-textarea" />
            </div>
          </section>
          ...
          <section className="aed-section">
            ...
            <p className="aed-section__caption">
              Aree di rappresentanza editoriale. Cambi sono salvati istantaneamente.
            </p>
