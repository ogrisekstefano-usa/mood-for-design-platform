...
const STATUS_TONES = {
  ...
  archived: 'text-[#6B6863] bg-white/5',
};
...
const NewTenantModal = ({ onClose, onSaved }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#0F0F11] border border-white/[0.1] rounded-md w-full max-w-lg">
        <div className="flex items-center justify-between p-6 border-b border-white/[0.06]">
          <h3 className="font-heading text-2xl text-[#EFEBE4]">{t('admin.tenants.new', null, 'Create tenant')}</h3>
          <button onClick={onClose} className="text-[#4A4845] hover:text-[#A19D98] text-lg">×</button>
        </div>
        <form onSubmit={submit} className="p-6 space-y-4">
          <div>
            <label className="block text-[11px] font-semibold uppercase tracking-[0.1em] text-[#6B6863] font-body mb-1.5">Name</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold uppercase tracking-[0.1em] text-[#6B6863] font-body mb-1.5">Default locale</label>
            ...
            <div>
              <label className="block text-[11px] font-semibold uppercase tracking-[0.1em] text-[#6B6863] font-body mb-1.5">Plan</label>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button data-testid="save-tenant-btn" type="submit" disabled={saving}
              className="flex-1 px-4 py-2.5 bg-amber-400 hover:bg-amber-300 text-[#0A0A0B] font-semibold text-sm font-body rounded-[3px] disabled:opacity-50">
    ...
    <div className="p-10 max-w-7xl mx-auto" data-testid="admin-tenants-page">
      ...
      <div className="flex items-start justify-between mb-10">
        <div>
          ...
          <h1 className="font-heading text-5xl font-light text-[#EFEBE4]">{t('admin.tenants.title', null, 'Tenants')}</h1>
          <p className="text-[#6B6863] text-sm font-body mt-2">{tenants.length} {t('admin.tenants.totalSub', null, 'across the platform')}</p>
        </div>
        <button data-testid="new-tenant-btn" onClick={() => setShowModal(true)}
          className="flex items-center gap-2 px-4 py-2.5 bg-amber-400 hover:bg-amber-300 text-[#0A0A0B] font-semibold text-xs font-body rounded-[3px]">
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <Building2 size={36} className="text-[#3A3835] mx-auto mb-4" strokeWidth={1} />
          <p className="text-[#6B6863] font-body">{t('admin.tenants.empty', null, 'No tenants yet')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((tenant, i) => (
            <div
              ...
              className="bg-[#0F0F11] border border-white/[0.06] rounded-md p-5 cursor-pointer hover:border-amber-500/20 transition-colors group"
            >
              ...
              <h3 className="font-heading text-xl text-[#EFEBE4] leading-tight mb-1">{tenant.name}</h3>
              <p className="text-[#6B6863] text-xs font-body mb-3">/{tenant.slug}</p>
