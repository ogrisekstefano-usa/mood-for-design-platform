...
const Toggle = ({ checked, onChange, testid }) => (
  <button data-testid={testid} onClick={onChange}
    className={`w-9 h-5 rounded-full p-0.5 transition-colors flex items-center ${checked ? 'bg-amber-400' : 'bg-white/[0.1]'}`}>
...
const AdminTenantDetailPage = () => {
  ...
  return (
    <div className="p-10 max-w-6xl mx-auto" data-testid="admin-tenant-detail">
      <button onClick={() => navigate('/admin/tenants')} className="text-[#4A4845] hover:text-[#A19D98] text-xs font-body flex items-center gap-1.5 mb-6">
        <ArrowLeft size={12} /> {t('admin.nav.tenants')}
      </button>

      <div className="flex items-start justify-between mb-10">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-md flex items-center justify-center border border-white/[0.08]"
            style={{ backgroundColor: tenant.primary_color || '#1C1C1F' }}>
          ...
          <div>
            <h1 className="font-heading text-4xl font-light text-[#EFEBE4]">{tenant.name}</h1>
            <p className="text-[#6B6863] text-sm font-body">/{tenant.slug}</p>
          </div>
        </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
        {Object.entries(usage).map(([k, v]) => (
          <div key={k} className="bg-[#0F0F11] border border-white/[0.06] rounded-md p-4">
            <p className="text-[#4A4845] text-[10px] font-body uppercase tracking-[0.15em] mb-1">{k}</p>
            <p className="font-heading text-2xl text-[#EFEBE4] font-light">{v}</p>
          </div>
        ))}
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Status + Plan */}
        <section className="bg-[#0F0F11] border border-white/[0.06] rounded-md p-5">
          <div className="flex items-center gap-2 mb-4">
            <Settings size={13} className="text-amber-400" />
            <h2 className="text-[#EFEBE4] text-sm font-body font-semibold">{t('admin.detail.statusPlan', null, 'Status & Plan')}</h2>
          </div>

          <p className="text-[10px] uppercase tracking-wider text-[#6B6863] font-body mb-1.5">Status</p>
          <div className="grid grid-cols-3 gap-1 mb-4">
            {['active', 'suspended', 'archived'].map((s) => (
              <button key={s} data-testid={`status-${s}`} onClick={() => updateStatus(s)}
                className={`px-2 py-1.5 text-[11px] font-body rounded-[3px] capitalize ${tenant.status === s ? 'bg-white/[0.08] text-[#EFEBE4]' : 'text-[#6B6863] hover:bg-white/[0.03]'}`}>
                {s}
              </button>
            ))}
          </div>

          <p className="text-[10px] uppercase tracking-wider text-[#6B6863] font-body mb-1.5">Plan</p>
          <div className="grid grid-cols-2 gap-1">
            {['trial', 'starter', 'pro', 'enterprise'].map((p) => (
              <button key={p} data-testid={`plan-${p}`} onClick={() => updatePlan(p)}
                className={`px-2 py-1.5 text-[11px] font-body rounded-[3px] capitalize ${planName === p ? 'bg-amber-400/20 text-amber-300' : 'text-[#6B6863] hover:bg-white/[0.03]'}`}>
                {p}
              </button>
            ))}
          </div>
        ...
        <section className="bg-[#0F0F11] border border-white/[0.06] rounded-md p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-[#EFEBE4] text-sm font-body font-semibold">{t('admin.detail.modules', null, 'Modules')}</h2>
          </div>
              ...
              return (
                <div key={m.id} className="flex items-center justify-between">
                  <div className="min-w-0">
                    <p className="text-[#EFEBE4] text-sm font-body capitalize">{m.id}</p>
                    <p className="text-[10px] text-[#4A4845] font-body truncate">{m.description}</p>
                  </div>
        ...
        <section className="bg-[#0F0F11] border border-white/[0.06] rounded-md p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-[#EFEBE4] text-sm font-body font-semibold">{t('admin.detail.flags', null, 'Feature Flags')}</h2>
          </div>
            ...
            {flagCatalog.map((f) => {
              ...
              return (
                <div key={f.id} className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-[#EFEBE4] text-sm font-body">{f.id}</p>
                    <p className="text-[10px] text-[#4A4845] font-body">
      ...
      {/* Members */}
      <section className="mt-6 bg-[#0F0F11] border border-white/[0.06] rounded-md p-5">
        <div className="flex items-center gap-2 mb-4">
          <UserCog size={13} className="text-amber-400" />
          <h2 className="text-[#EFEBE4] text-sm font-body font-semibold">{t('admin.detail.members', null, 'Members')}</h2>
        </div>
        {detail.members.length === 0 ? (
          <p className="text-[#4A4845] text-sm font-body">{t('common.noResults')}</p>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.05]">
                {['Email', 'Name', 'Role', 'Status'].map((h) => (
                  <th key={h} className="text-left py-2 text-[10px] font-body font-bold uppercase tracking-[0.12em] text-[#4A4845]">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {detail.members.map((m) => (
                <tr key={m.id} className="border-b border-white/[0.03]" data-testid={`member-row-${m.id}`}>
                  <td className="py-2.5 text-[#EFEBE4] text-sm font-body">{m.email}</td>
                  <td className="py-2.5 text-[#A19D98] text-xs font-body">{`${m.first_name || ''} ${m.last_name || ''}`.trim()}</td>
                  <td className="py-2.5 text-[#A19D98] text-xs font-body capitalize">{m.role?.replace(/_/g, ' ')}</td>
                  <td className="py-2.5 text-[#6B6863] text-xs font-body">{m.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
