...
const UsageMeter = ({ icon: Icon, label, current, limit, unit = '', testid }) => {
  const unlimited = limit == null;
  const pct = unlimited ? 0 : Math.min(100, Math.round((current / Math.max(limit, 1)) * 100));
  const tone = unlimited ? 'safe'
             : pct >= 100 ? 'danger'
             : pct >= 80  ? 'warn'
             : 'safe';
  const tones = {
    safe:   { bar: 'bg-[var(--bp-primary)]',          fg: 'text-[var(--bp-text-primary)]' },
    warn:   { bar: 'bg-amber-300',                    fg: 'text-amber-200' },
    danger: { bar: 'bg-rose-400',                     fg: 'text-rose-300' },
  };
  return (
    <div className="bg-[var(--bp-surface-1)] border border-[var(--bp-border)] rounded-[var(--bp-radius-md)] p-5"
         data-testid={testid}>
      <div className="flex items-center justify-between mb-3">
        ...
        {unlimited && (
          <span className="text-[var(--bp-primary)] text-[10px] font-body uppercase tracking-[0.18em]">Unlimited</span>
        )}
      </div>
      ...
      <div className="mt-3 h-1 w-full bg-[var(--bp-surface-2)] rounded-full overflow-hidden">
        <div className={`h-full ${tones[tone].bar} transition-[width] duration-700`}
             style={{ width: `${unlimited ? 12 : pct}%` }} />
      </div>
      {tone === 'warn' && (
        <p className="mt-2 text-amber-200 text-[10px] font-body flex items-center gap-1">
          <AlertCircle size={10} strokeWidth={1.5} /> Approaching limit
        </p>
      )}
      {tone === 'danger' && (
        <p className="mt-2 text-rose-300 text-[10px] font-body flex items-center gap-1">
...
const PlanCard = ({ plan, isCurrent, isSuper, onAssign }) => (
  <div className={`relative bg-[var(--bp-surface-1)] border rounded-[var(--bp-radius-md)] p-6 transition-colors
    ${isCurrent ? 'border-[var(--bp-primary)] bg-[var(--bp-primary)]/4' : 'border-[var(--bp-border)] hover:border-[var(--bp-border-strong)]'}`}
       data-testid={`plan-card-${plan.key}`}>
    {isCurrent && (
      <span className="absolute top-4 right-4 px-2 py-0.5 rounded-full bg-[var(--bp-primary)] text-black text-[9px] font-body uppercase tracking-[0.18em]">
        Current
      </span>
    )}
    ...
    {isSuper && !isCurrent && (
      <button onClick={() => onAssign(plan.key)}
              data-testid={`plan-assign-${plan.key}`}
              className="w-full px-4 py-2 rounded-[var(--bp-radius-xs)] bg-[var(--bp-primary)] text-black text-[11px] font-body uppercase tracking-[0.22em] hover:brightness-110 transition-all">
        Assign this plan
      </button>
    )}
    {!isSuper && !isCurrent && (
      <button disabled
              className="w-full px-4 py-2 rounded-[var(--bp-radius-xs)] border border-[var(--bp-border)] text-[var(--bp-text-muted)] text-[11px] font-body uppercase tracking-[0.22em] flex items-center justify-center gap-2 cursor-not-allowed">
...
const PlanPage = () => {
  ...
  if (loading) {
    return (
      <div className="p-10 flex items-center justify-center text-[var(--bp-text-muted)]">
  ...
  return (
    <div className="p-10 max-w-5xl mx-auto" data-testid="plan-page">
      ...
      <div className="flex flex-wrap gap-1.5 mb-10">
        {(license.enabled_modules || []).map((m) => (
          <span key={m} data-testid={`module-${m}`}
                className="px-3 py-1 rounded-full bg-[var(--bp-surface-2)] border border-[var(--bp-border)] text-[var(--bp-text-secondary)] text-[10px] font-body uppercase tracking-[0.18em]">
            {m}
          </span>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {plans.map((p) => (
          <PlanCard key={p.key} plan={p}
                    isCurrent={p.key === license.plan_key}
                    isSuper={isSuper}
                    onAssign={assign} />
        ))}
      </div>
