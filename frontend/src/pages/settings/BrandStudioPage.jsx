...
import { ArrowLeft, Palette, Type, Save, RotateCcw, Loader2, CheckCircle2, Image as ImageIcon, Layers, Sparkles, Square, Circle, Sun, Moon } from 'lucide-react';
...
import { LIGHT_PALETTES, DARK_PALETTES, applyPalette as applyCuratedRoot, storePalette, clearPalette as clearCuratedRoot } from '../../lib/curatedPalettes';
...
const EDITORIAL_VOICE_OPTIONS = {
  communication_personality: [{
    ...
    key: 'strategic',
    label: 'Strategica · orientata a decisioni progettuali'
  }, {
    key: 'technical',
    label: 'Tecnica · architettonica, zero metafore'
  }, {
    key: 'editorial',
    label: 'Editoriale · registro magazine'
  }, {
    key: 'hospitality',
    label: 'Ospitalità · calore esperienziale'
  }, {
    key: 'commercial_soft',
    label: 'Commerciale morbida · rassicurante'
  }, {
    key: 'luxury',
    label: 'Luxury · registro raffinato'
  }, {
    key: 'minimal_executive',
    label: 'Minimal executive · una frase essenziale'
  }, {
    key: 'cultural_consultant',
    label: 'Consulenza culturale internazionale'
  }],
  vocabulary_style: [{
    key: '',
    label: 'Da definire'
  }, {
    key: 'architecture_studio',
    ...
    label: 'Editoriale magazine'
  }, {
    key: 'retail_showroom',
    label: 'Retail showroom'
  }, {
    key: 'international_consultancy',
    label: 'Consulenza internazionale'
  }],
  narrative_intensity: [{
    key: '',
    label: 'Da definire'
    ...
    key: 'minimal',
    label: 'Minimal · essenziale (1 frase)'
  }, {
    key: 'balanced',
    label: 'Bilanciata · 1-2 frasi dense'
  }, {
    key: 'editorial',
    label: 'Editoriale · 2-3 frasi'
  }, {
    key: 'cinematic',
    label: 'Cinematica · 3-4 frasi narrative'
  }],
  interpretation_density: [{
    ...
    key: 'concise',
    label: 'Concisa · zero ridondanze'
  }, {
    key: 'standard',
    label: 'Standard · densa ma non lunga'
  }, {
    key: 'deep_analysis',
    label: 'Approfondita · analitica'
  }]
};
...
const sortPresetsByHue = presets => {
  ...
  return [...presets].sort((a, b) => {
    ...
    const ca = parseColor(a?.theme?.palette?.primary) || {
      h: 0,
      s: 0,
      l: 0
    };
    const cb = parseColor(b?.theme?.palette?.primary) || {
      h: 0,
      s: 0,
      l: 0
    };
...
const Section = ({
  ...
  trace
    ...
    {trace && <div className="mb-4 inline-flex items-center gap-2 px-2.5 py-1 bg-[var(--bp-surface-2)] border border-[var(--bp-border)] rounded-[var(--bp-radius-xs)]" data-testid={`${testid}-trace`}>
...
const TextInput = ({
  ...
  placeholder,
  testid
}) => <input value={value || ''} onChange={e => onChange(e.target.value)} placeholder={placeholder} data-testid={testid} className="w-full px-3 py-2 bg-[var(--bp-surface-2)] border border-[var(--bp-border)] rounded-[var(--bp-radius-xs)] text-[var(--bp-text-primary)] text-[12px] font-body outline-none focus:border-[var(--bp-primary)] transition-colors" />;
...
const IdentityI18nGroup = ({
  ...
  return <div data-testid="brand-i18n-group" className="mb-2">
      {/* Locale switcher */}
      <div className="flex flex-wrap gap-1 mb-3" role="tablist" aria-label="Edit per locale">
        {I18N_LOCALES.map(l => <button key={l.code} type="button" role="tab" aria-selected={activeLocale === l.code} onClick={() => setActiveLocale(l.code)} data-testid={`brand-i18n-tab-${l.code}`} className={`px-2.5 py-1 text-[10px] uppercase tracking-[0.18em] font-body rounded-[3px] border transition-colors ${activeLocale === l.code ? 'bg-[var(--bp-primary)] text-[var(--bp-bg)] border-[var(--bp-primary)]' : 'bg-transparent text-[var(--bp-text-muted)] border-[var(--bp-border)] hover:text-[var(--bp-text-primary)]'}`}>
            <span style={{
          marginRight: 5
        }}>{l.flag}</span>{l.code}
          </button>)}
      </div>
      <p className="text-[10.5px] italic text-[var(--bp-text-muted)] font-body mb-3" style={{
      lineHeight: 1.5
    }}>
        {tr('brand.i18nHint', {
        locale: activeLocale
      }, `Stai modificando i valori per ${activeLocale}. Il sito pubblico mostra il valore del locale visitato — un visitatore tedesco vede la versione de-DE.`)}
      </p>

      <Field label={`${tr('brand.field.public_name', null, 'Nome pubblico del brand')} · ${activeLocale}`}>
        <TextInput value={getI18n('public_brand_name')} onChange={v => setI18n('public_brand_name', v)} placeholder="MOOD for DESIGN" testid="brand-public-name" />
      </Field>
      <Field label={`${tr('brand.field.tagline', null, 'Tagline')} · ${activeLocale}`}>
        <TextInput value={getI18n('tagline')} onChange={v => setI18n('tagline', v)} placeholder="Spaces that tell stories" testid="brand-tagline" />
      </Field>
      <Field label={`${tr('brand.field.short_desc', null, 'Descrizione breve')} · ${activeLocale}`}>
        <TextInput value={getI18n('short_description')} onChange={v => setI18n('short_description', v)} placeholder="A boutique studio crafting bespoke interiors." testid="brand-short-desc" />
      </Field>
    </div>;
};
const ColorPicker = ({
...
const SelectChips = ({
  ...
  renderAs
    ...
    return <button type="button" key={o} onClick={() => onChange(o)} data-testid={`${testid}-${o}`} style={fontStyle || undefined} className={`px-3 py-1.5 rounded-[var(--bp-radius-xs)] text-[10px] font-body uppercase tracking-[0.18em] border transition-colors
                  ${active ? 'border-[var(--bp-primary)] text-[var(--bp-primary)] bg-[var(--bp-primary)]/8' : 'border-[var(--bp-border)] text-[var(--bp-text-secondary)] hover:border-[var(--bp-border-strong)] hover:text-[var(--bp-text-primary)]'}`}>
          {o}
        </button>;
  })}
  </div>;
const PresetCard = ({
  ...
  return <button type="button" onClick={() => onPick(preset)} data-testid={testid} className={`text-left rounded-[var(--bp-radius-sm)] p-3 border transition-colors group
              ${current ? 'border-[var(--bp-primary)] bg-[var(--bp-primary)]/5' : 'border-[var(--bp-border)] hover:border-[var(--bp-border-strong)]'}`}>
      <div className="aspect-[5/2] rounded-[3px] mb-2 overflow-hidden border border-[var(--bp-border)]" style={{
      background: p.background || '#111'
    }}>
        <div className="h-full flex items-center px-3 gap-1.5">
          ...
          <span className="ml-auto font-heading text-[11px]" style={{
          color: p.text_primary,
          fontFamily: preset.theme?.typography?.display
        }}>
            {preset.label}
          </span>
        </div>
...
const CuratedPaletteCard = ({
  palette,
  current,
  onPick,
  testid
}) => <button type="button" onClick={() => onPick(palette)} data-testid={testid} className={`relative text-left rounded-[var(--bp-radius-sm)] p-3 border transition-all group
            ${current ? 'border-[var(--bp-primary)] bg-[var(--bp-primary)]/5 ring-1 ring-[var(--bp-primary)]' : 'border-[var(--bp-border)] hover:border-[var(--bp-border-strong)] hover:-translate-y-px'}`}>
    ...
    {current && <span className="absolute top-2 right-2 inline-flex w-4 h-4 rounded-full bg-[var(--bp-primary)] text-white items-center justify-center">
...
const LivePreview = ({
  ...
  return <div className="rounded-[var(--bp-radius-md)] border border-[var(--bp-border)] overflow-hidden sticky top-6" data-testid="brand-preview" style={{
    ...
    fontFamily: typo.body ? `${typo.body}, sans-serif` : 'inherit'
      ...
      borderColor: palette.border
        ...
        <div className="font-heading text-lg" style={{
        fontFamily: typo.display ? `${typo.display}, serif` : 'inherit'
      }}>
          {previewName}
        </div>
        <button className="px-3 py-1.5 text-[10px] uppercase tracking-[0.18em]" style={{
        background: palette.primary,
      ...
      background: palette.surface
        ...
        <p className="text-[10px] uppercase tracking-[0.22em] mb-2" style={{
        color: palette.primary
      }}>
          {previewTagline}
        </p>
        <h1 className="font-heading leading-tight mb-2 text-3xl" style={{
        fontFamily: typo.display ? `${typo.display}, serif` : 'inherit'
      }}>
          Spaces that tell stories.
        </h1>
        <p className="text-[12px] mb-4 max-w-md" style={{
        color: palette.text_secondary
      }}>
          {previewDesc}
        </p>
        <div className="flex gap-2">
          <button className="px-4 py-2 text-[10px] uppercase tracking-[0.2em]" style={{
          background: palette.primary,
          color: palette.background,
          borderRadius: radius
        }}>
          ...
          <button className="px-4 py-2 text-[10px] uppercase tracking-[0.2em] border" style={{
          borderColor: palette.border,
          color: palette.text_primary,
          borderRadius: radius
        }}>
      ...
      {/* Cards */}
      <div className="grid grid-cols-3 gap-2 p-4" style={{
      background: palette.background
    }}>
        {[1, 2, 3].map(i => <div key={i} className="p-3 border" style={{
        borderColor: palette.border,
        borderRadius: radius,
        background: palette.surface
      }}>
            <div className="aspect-square mb-2" style={{
          background: i === 2 ? palette.accent : palette.secondary,
          borderRadius: radius
        }} />
            <p className="text-[10px] uppercase tracking-[0.18em]" style={{
          color: palette.text_secondary
        }}>Project</p>
            <p className="font-heading text-[12px]" style={{
          fontFamily: typo.display ? `${typo.display}, serif` : 'inherit'
        }}>Villa #{i}</p>
          </div>)}
      </div>
      {/* Buttons strip */}
      <div className="flex gap-2 px-4 pb-4" style={{
      background: palette.background
    }}>
...
const BrandStudioPage = () => {
  ...
  // Load presets once
  useEffect(() => {
    api.get('/api/branding/presets').then(r => setPresets(r.data?.presets || [])).catch(() => {});
  }, []);
  ...
  const dirty = useMemo(() => JSON.stringify(branding) !== JSON.stringify(savedBranding || {}) || JSON.stringify(theme) !== JSON.stringify(savedTheme || {}), [branding, theme, savedBranding, savedTheme]);
  const setPalette = (k, v) => setTheme(prev => ({
    ...prev,
    palette: {
      ...(prev.palette || {}),
      [k]: v
  ...
  const setTypo = (k, v) => setTheme(prev => ({
    ...prev,
    typography: {
      ...(prev.typography || {}),
      [k]: v
  ...
  const applyCuratedPalette = palette => {
    ...
    const isDark = (palette.text || '').toLowerCase().startsWith('#e') || (palette.text || '').toLowerCase().startsWith('#f');
  ...
  if (!canManage) {
    return <div className="p-10 text-[var(--bp-text-muted)] text-center" data-testid="brand-page-denied">
  ...
  return <div className="p-8 pb-24 max-w-[1500px] mx-auto" data-testid="brand-page">
      ...
      <div className="flex items-start justify-between mb-7">
        <div>
          <button onClick={() => navigate('/settings')} data-testid="brand-back" className="flex items-center gap-1.5 text-[var(--bp-text-muted)] hover:text-[var(--bp-text-primary)] text-[10px] font-body uppercase tracking-[0.22em] mb-3">
            <ArrowLeft size={11} strokeWidth={1.5} /> {t('common.back', null, 'Back')}
          </button>
          <p className="text-[var(--bp-primary)] text-[10px] font-body uppercase tracking-[0.22em] font-semibold mb-1" data-testid="studio-identity-eyebrow">
            Workspace · Studio Identity™
          </p>
          <h1 className="font-heading text-4xl font-light text-[var(--bp-text-primary)] leading-none" data-testid="studio-identity-title">
          ...
          <div className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 bg-[var(--bp-surface-2)] border border-[var(--bp-border)] rounded-[var(--bp-radius-sm)]" data-testid="brand-scope-trace">
        ...
        <div className="flex items-center gap-2">
          {dirty && <button onClick={discard} data-testid="brand-discard" className="px-4 py-2.5 rounded-[var(--bp-radius-sm)] border border-[var(--bp-border)] text-[var(--bp-text-muted)] hover:text-[var(--bp-text-primary)] text-[10px] font-body uppercase tracking-[0.22em] flex items-center gap-1.5 transition-colors">
        ...
        <div>
          <Section kicker={t('brand.section.identityKicker', null, 'A · Identità')} title={t('brand.section.identity', null, 'Identità del brand')} testid="section-identity">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label={t('brand.field.support_email', null, 'Email di supporto')}>
                <TextInput value={branding.support_email} onChange={v => setBranding({
                ...branding,
                support_email: v
              })} placeholder="hello@studio.com" testid="brand-support-email" />
              </Field>
              <Field label={t('brand.field.phone', null, 'Telefono')}>
                <TextInput value={branding.phone} onChange={v => setBranding({
                ...branding,
                phone: v
            ...
            <Field label={t('brand.field.website', null, 'Sito web')}>
              <TextInput value={branding.website_url} onChange={v => setBranding({
              ...branding,
              website_url: v
            })} placeholder="https://studio.com" testid="brand-website" />
            </Field>
            <Field label={t('brand.field.primary_logo', null, 'Logo principale')}>
              <EditorialMediaField value={branding.primary_logo_url} onChange={v => setBranding({
              ...branding,
              primary_logo_url: v
            })} preset="logo" bucket="tenant-assets" folder="brand/logo" entityType="branding_asset" entityId={user?.tenant_id} role="primary_logo" helperText={t('brand.field.primary_logo_helper', null, 'Mostrato in header pubblico, email transazionali, footer. Sfondo trasparente raccomandato (PNG/SVG).')} testId="brand-logo-field" />
            </Field>
          </Section>

          <Section kicker={t('brand.section.paletteKicker', null, 'B · Palette')} title={t('brand.section.paletteTitle', null, 'Colori')} testid="section-palette" trace={t('brand.paletteTrace', null, 'Storefront pubblico (palette completa) · Blueprint editor (solo accent + font come identità sottile)')}>
            <div className="mb-4 flex items-center gap-2" data-testid="brand-mode-toggle">
              ...
              {['dark', 'light'].map(m => <button key={m} type="button" data-testid={`brand-mode-${m}`} onClick={() => setTheme({
              ...theme,
              mode: m
            })} className={`px-3 py-1.5 text-[10px] uppercase tracking-[0.18em] border rounded-[var(--bp-radius-xs)] transition-colors
                          ${(theme.mode || 'dark') === m ? 'bg-[var(--bp-primary-soft)] text-[var(--bp-primary)] border-[var(--bp-border-active)]' : 'bg-transparent text-[var(--bp-text-muted)] border-[var(--bp-border)] hover:text-[var(--bp-text-primary)]'}`}>
                  {m === 'dark' ? 'Dark base' : 'Light base'}
                </button>)}
              <span className="ml-2 text-[10px] italic text-[var(--bp-text-faint)]">
                {t("settings.brand_studio.il_preset_sceglie_la_modalita_automaticamente_puoi")}
              </span>
            </div>
            <div className="grid grid-cols-2 gap-x-5 gap-y-1">
              {[['primary', 'Primary'], ['secondary', 'Secondary'], ['accent', 'Accent'], ['background', 'Background'], ['surface', 'Surface'], ['text_primary', 'Text primary'], ['text_secondary', 'Text secondary'], ['border', 'Border'], ['success', 'Success'], ['warning', 'Warning'], ['danger', 'Danger']].map(([k, label]) => <ColorPicker key={k} label={label} value={palette[k] || ''} onChange={v => setPalette(k, v)} testid={`palette-${k}`} />)}
            </div>
          </Section>
          ...
          <Section kicker="D · Surface" title={t('brand.section.surfaceTitle', null, 'Raggio · Densità · Ombre')} testid="section-surface">
            <Field label={t('brand.field.radius', null, 'Border radius')}>
              <SelectChips options={RADIUS_OPTS} value={theme.radius} onChange={v => setTheme({
              ...theme,
              radius: v
            })} testid="radius" />
            </Field>
            <Field label={t('brand.field.density', null, 'Densità')}>
              <SelectChips options={DENSITY_OPTS} value={theme.density} onChange={v => setTheme({
              ...theme,
              density: v
            })} testid="density" />
            </Field>
            <Field label={t('brand.field.shadow', null, 'Morbidezza ombre')}>
              <SelectChips options={SHADOW_OPTS} value={theme.shadow} onChange={v => setTheme({
              ...theme,
              shadow: v
            })} testid="shadow" />
            </Field>
          ...
          <Section kicker="E · Preset editoriali" title={t('brand.section.presetsTitle', null, 'Preset editoriali')} testid="section-presets" trace={t('brand.presetsTrace', null, `${presets.length} identità complete · raggruppate per atmosfera · sovrascrivono palette + tipografia + radius`)}>
            {(() => {
            ...
            return <>
                  {lights.length > 0 && <>
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-7" data-testid="brand-editorial-presets-light-grid">
                        {lights.map(p => <PresetCard key={p.key} preset={p} current={theme.preset_key === p.key} onPick={applyPreset} testid={`preset-${p.key}`} />)}
                      </div>
                    </>}
                  {darks.length > 0 && <>
                      <div className="mb-3 text-[10px] uppercase tracking-[0.22em] text-[var(--bp-text-muted)] font-body flex items-center gap-2">
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3" data-testid="brand-editorial-presets-dark-grid">
                        {darks.map(p => <PresetCard key={p.key} preset={p} current={theme.preset_key === p.key} onPick={applyPreset} testid={`preset-${p.key}`} />)}
                      </div>
                    </>}
                </>;
          })()}
          ...
          <Section kicker="F · Voce editoriale" title={t("settings.brand_studio.voce_editoriale_dello_studio")} testid="section-editorial-voice" trace="Modula tutte le interpretazioni culturali e le Cultural Edition™ generate per questo studio · sovrascrivibile contestualmente per ogni singolo riferimento">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[['communication_personality', 'Personalità comunicativa'], ['vocabulary_style', 'Lessico dello studio'], ['narrative_intensity', 'Intensità narrativa abituale'], ['interpretation_density', 'Densità interpretativa']].map(([key, label]) => {
              const current = (branding.editorial_voice || {})[key] || '';
              return <Field key={key} label={label}>
                    <select value={current} data-testid={`editorial-voice-${key}`} onChange={e => setBranding({
                  ...branding,
                  editorial_voice: {
                    ...
                    [key]: e.target.value || undefined
                                 ...
                                 transition-colors">
                      {EDITORIAL_VOICE_OPTIONS[key].map(opt => <option key={opt.key || 'default'} value={opt.key}>{opt.label}</option>)}
                    </select>
                  </Field>;
            })}
            </div>
            <div className="mt-5 inline-flex items-start gap-2 px-3 py-2 bg-[var(--bp-primary-soft)] border border-[var(--bp-border-active)] rounded-[var(--bp-radius-xs)]">
              ...
              <span className="text-[10.5px] text-[var(--bp-text-muted)] font-body leading-relaxed italic max-w-md">
                {t("settings.brand_studio.la_voce_dello_studio_resta_persistente_la_direzion")}
              </span>
            </div>
          </Section>
        </div>
