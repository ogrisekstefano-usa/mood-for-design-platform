...
const StatusPill = ({
  ...
  const cls = status === 'published' ? 'border-emerald-400/40 text-emerald-300 bg-emerald-400/10' : status === 'archived' ? 'border-zinc-500/40 text-zinc-400 bg-zinc-500/10' : 'border-amber-400/40 text-amber-300 bg-amber-400/10';
  return <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-[2px] border text-[9px] uppercase tracking-[0.22em] ${cls}`} data-testid={`status-${status}`}>
...
const MagazineAdminPage = () => {
    ...
    try {
      const r = await api.post('/api/magazine/admin/articles', {
        ...
        body_blocks: [{
          ...
          locale_content: {
            it: {
              text: 'Scrivi qui l\'introduzione editoriale del progetto.'
            }
          }
        }],
  ...
  return <div className="px-8 py-8 max-w-7xl mx-auto" data-testid="magazine-admin-page">
      <header className="flex items-end justify-between mb-10 pb-6 border-b border-[var(--bp-border)]">
        ...
        <button type="button" onClick={() => setShowCreate(true)} data-testid="magazine-create-btn" className="inline-flex items-center gap-2 px-4 py-2.5 bg-[var(--bp-primary)] text-white text-[10px] font-body uppercase tracking-[0.22em] hover:bg-[var(--bp-primary-hover)] transition-colors">
      ...
      {!state.loading && state.articles.length === 0 && <div className="text-center py-20 max-w-md mx-auto" data-testid="magazine-empty">
      ...
      {state.articles.length > 0 && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" data-testid="magazine-list">
          {state.articles.map(a => {
        const lc = a.locale_content?.it || a.locale_content?.['en-US'] || Object.values(a.locale_content || {})[0] || {};
        return <article key={a.id} className="group bg-[var(--bp-surface)] border border-[var(--bp-border)] hover:border-[var(--bp-primary)]/40 transition-colors" data-testid={`magazine-card-${a.id}`}>
                <Link to={`/settings/magazine/${a.id}`} className="block">
                  <div className="aspect-[16/10] bg-[var(--bp-surface-2)] overflow-hidden">
                    {a.cover_url || a.hero_url ? <img src={a.cover_url || a.hero_url} alt="" className="w-full h-full object-cover group-hover:scale-[1.02] transition-transform duration-500" /> : <div className="w-full h-full flex items-center justify-center">
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {a.category_slug && <span className="text-[9px] text-[var(--bp-text-muted)] uppercase tracking-[0.22em]">· {a.category_slug}</span>}
                    </div>
                ...
                <footer className="flex items-center justify-between px-4 pb-4 pt-1 border-t border-[var(--bp-border)]/40 mt-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    {a.status === 'published' && <a href={`/magazine/${a.slug}`} target="_blank" rel="noreferrer" className="p-1.5 text-[var(--bp-text-muted)] hover:text-[var(--bp-primary)]" title="Open public page" data-testid={`magazine-view-${a.id}`}>
                        <ExternalLink size={12} strokeWidth={1.6} />
                      </a>}
                    <button type="button" onClick={() => handleDelete(a.id)} className="p-1.5 text-[var(--bp-text-muted)] hover:text-rose-400" title="Delete" data-testid={`magazine-delete-${a.id}`}>
      ...
      {/* Create modal */}
      {showCreate && <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-6" data-testid="magazine-create-modal">
          <form onSubmit={handleCreate} className="bg-[var(--bp-surface)] border border-[var(--bp-border)] w-full max-w-md p-7 shadow-2xl">
            ...
            })} required autoFocus data-testid="magazine-create-title" className="w-full px-3 py-2 bg-[var(--bp-bg)] border border-[var(--bp-border)] text-[var(--bp-text-primary)] outline-none focus:border-[var(--bp-primary)] text-sm" />
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-[0.22em] text-[var(--bp-text-muted)] block mb-1.5">Slug URL</label>
                <input type="text" value={form.slug} onChange={e => setForm({
              ...form,
              slug: slugify(e.target.value)
            })} placeholder="es. casa-vista-mare-ligure" data-testid="magazine-create-slug" className="w-full px-3 py-2 bg-[var(--bp-bg)] border border-[var(--bp-border)] text-[var(--bp-text-primary)] outline-none focus:border-[var(--bp-primary)] text-sm font-mono" />
              </div>
              <div>
            ...
            })} data-testid="magazine-create-category" className="w-full px-3 py-2 bg-[var(--bp-bg)] border border-[var(--bp-border)] text-[var(--bp-text-primary)] outline-none focus:border-[var(--bp-primary)] text-sm">
            ...
            <div className="flex items-center justify-end gap-2 mt-6">
              <button type="button" onClick={() => setShowCreate(false)} className="px-3 py-2 text-[10px] uppercase tracking-[0.22em] text-[var(--bp-text-muted)] hover:text-[var(--bp-text-primary)]" data-testid="magazine-create-cancel">
                Annulla
              </button>
              <button type="submit" disabled={busy || !form.title} className="inline-flex items-center gap-1.5 px-4 py-2 bg-[var(--bp-primary)] text-white text-[10px] uppercase tracking-[0.22em] hover:bg-[var(--bp-primary-hover)] disabled:opacity-50" data-testid="magazine-create-confirm">
