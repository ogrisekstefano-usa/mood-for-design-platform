...
const StudioLogo = ({
  ...
  const initials = (name || 'MOOD').split(' ').map(p => p[0]).filter(Boolean).slice(0, 2).join('').toUpperCase();
  if (logoUrl && !errored) {
    return <span className="cck-hero__studio" data-testid="cockpit-hero-studio-logo">
        <img src={logoUrl} alt={name || 'studio'} loading="lazy" onError={() => setErrored(true)} />
      </span>;
  }
  return <span className="cck-hero__studio cck-hero__studio--text" data-testid="cockpit-hero-studio-initials">
      <span className="cck-hero__studio-initials">{initials}</span>
      {name && <span className="cck-hero__studio-name">{name}</span>}
    </span>;
};
const DailyStudioStatus = ({
  firstName,
  summary,
  suggested,
  attentionCount,
  ...
  if (Array.isArray(summary)) {
    summary.slice(0, 3).forEach(s => sentences.push({
      text: s.text,
      to: s.to,
      icon: s.icon
    }));
  ...
  return <section className="cck-hero" data-testid="cockpit-hero">
      ...
      {loading && sentences.length === 0 ? <ul className="cck-hero__sentences" aria-busy="true">
        ...
        </ul> : sentences.length > 0 ? <ul className="cck-hero__sentences" data-testid="cockpit-hero-sentences">
        ...
        </ul> : <p className="cck-hero__quiet" data-testid="cockpit-hero-quiet">
          {runtime.copy('dashboard.hero.empty') || 'Studio in quiete editoriale. È il momento ideale per iniziare un nuovo progetto o riaprire una relazione.'}
        </p>}
...
const SuggestedNextActions = ({
  ...
  return <section className="cck-block" data-testid="cockpit-suggested-actions">
    ...
    {loading && (!suggestions || suggestions.length === 0) ? <div className="cck-grid-3" data-testid="cockpit-suggested-skeleton">
        {[0, 1, 2].map(i => <div key={i} className="cck-suggest cck-suggest--skel">
...
const QUICK_CLUSTERS = [{
  ...
  items: [{
    label: 'Nuovo Account',
    to: '/crm/accounts?new=1',
    icon: 'UserPlus'
  }, {
    label: 'Nuovo Contatto',
    to: '/crm/contacts?new=1',
    icon: 'User'
  }, {
    label: 'Registra visita',
    to: '/crm/accounts?action=visit',
    icon: 'MapPin'
  }, {
    label: 'Registra chiamata',
    to: '/crm/accounts?action=call',
    icon: 'Phone'
  }, {
    label: 'Nuovo follow-up',
    to: '/crm/accounts?action=note',
    icon: 'MessageCircle'
  }]
}, {
  id: 'projects',
  ...
  items: [{
    ...
    label: 'Nuova moodboard',
    to: '/moodboards?new=1',
    icon: 'Layers'
  }, {
    label: 'Apri progetti in corso',
    to: '/workspace/projects',
    icon: 'FolderOpen'
  }]
}, {
  ...
  glyph: 'Globe',
  items: [{
    label: 'Crea Cultural Edition™',
    to: '/workspace/cultural-editions?new=1',
    icon: 'Globe2'
  }, {
    label: 'Market Resonance™',
    to: '/admin/market-matrix',
    icon: 'Compass'
  }]
}, {
  id: 'editorial',
...
const QuickActions = ({
  onOpenCulturalWizard
    ...
    <div className="cck-quick-grid">
      ...
      return <div key={c.id} className="cck-quick-cluster" data-testid={`cockpit-quick-cluster-${c.id}`}>
            ...
            if (isCultural) {
              return <button key={i} type="button" onClick={onOpenCulturalWizard} className="cck-quick-item" data-testid={`cockpit-quick-${c.id}-${i}`}>
            ...
            return <Link key={i} to={it.to} className="cck-quick-item" data-testid={`cockpit-quick-${c.id}-${i}`}>
                    <span className="cck-quick-item__icon"><Icon size={12} strokeWidth={1.6} /></span>
                    <span className="cck-quick-item__label">{it.label}</span>
                    <Icons.ArrowUpRight size={10} className="cck-quick-item__arrow" />
                  </Link>;
...
const StudioAttention = ({
  ...
  return <section className="cck-block" data-testid="cockpit-studio-attention">
      ...
      {loading && (!projects || projects.length === 0) ? <div className="cck-grid-4">
          {[0, 1, 2, 3].map(i => <div key={i} className="cck-attn cck-attn--skel">
        ...
        </div> : projects && projects.length > 0 ? <div className="cck-grid-4">
          {projects.slice(0, 4).map(p => {
        const stale = staleSet.has(p.id);
        return <Link key={p.id} to={`/workspace/projects/${p.id}`} className={`cck-attn ${stale ? 'cck-attn--stale' : ''}`} data-testid={`cockpit-attention-project-${p.id}`}>
                ...
                <div className="cck-attn__bar">
                  <div className="cck-attn__bar-fill" style={{
              width: `${p.progress || 0}%`
            }} />
                </div>
              </Link>;
  ...
  return <section className="cck-block" data-testid="cockpit-relationship-engine">
      <header className="cck-block__head">
        ...
        <Link to="/crm/accounts" className="cck-block__link" data-testid="cockpit-rel-all">
          {t("dashboard.dashboard.tutti_gli_account")} <Icons.ArrowUpRight size={11} />
        </Link>
      </header>
      {loading && (!rows || rows.length === 0) ? <div className="cck-rel-list">
          {[0, 1, 2, 3].map(i => <div key={i} className="cck-rel cck-rel--skel">
        ...
        </div> : rows && rows.length > 0 ? <div className="cck-rel-list">
          {rows.slice(0, 6).map(r => {
        const cold = (r.days_since || 0) >= 14;
        return <div key={r.account_id} className="cck-rel" data-testid={`cockpit-rel-${r.account_id}`}>
                ...
                <div className="cck-rel__body">
                  <div className="cck-rel__row1">
                    ...
                    {r.market_code && <span className="cck-rel__market">
                ...
                <div className="cck-rel__actions">
                  <button type="button" title="Registra nota" onClick={() => navigate(`/crm/accounts/${r.account_id}?action=note`)} className="cck-rel__act" data-testid={`cockpit-rel-act-note-${r.account_id}`}>
                    <Icons.MessageCircle size={11} />
                  </button>
                  <button type="button" title={t("dashboard.dashboard.apri_timeline")} onClick={() => navigate(`/crm/accounts/${r.account_id}`)} className="cck-rel__act cck-rel__act--primary" data-testid={`cockpit-rel-act-open-${r.account_id}`}>
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    api.get('/api/dashboard/summary', {
      ...
      if (!data) {
        setError({
          status,
          message: status === 403 ? 'Questa dashboard è riservata ai membri dello studio.' : 'Impossibile caricare il cockpit in questo momento.'
        });
      }
  ...
  if (error && !data) {
    return <div data-testid="dashboard-error" className="cck-error">
        ...
        {error.status !== 403 && <button type="button" onClick={() => {
        reloadRef.current += 1;
        setError(null);
      }} data-testid="dashboard-retry-btn" className="cck-error__btn">
            <Icons.RotateCcw size={12} /> {t("dashboard.dashboard.riprova")}
          </button>}
      </div>;
  }
  const d = data || {};
  return <div data-testid="dashboard-page" className="cck-page">
      {/* 1 · Daily Studio Status™ — render immediato anche senza fetch */}
      <DailyStudioStatus firstName={firstName} summary={d.operational_summary} suggested={d.suggested_actions} attentionCount={(d.stale_project_ids || []).length} relCount={(d.relationship_engine || []).length} loading={loading} logoUrl={studioLogoUrl} studioName={studioName} onOpenCulturalWizard={() => setWizardOpen(true)} />
      ...
      {/* 4 · Studio Attention™ */}
      <StudioAttention projects={d.featured_projects} staleIds={d.stale_project_ids} loading={loading} />
      ...
      <CulturalEditionWizard open={wizardOpen} onClose={() => setWizardOpen(false)} onCreated={draft => {
      setWizardOpen(false);
