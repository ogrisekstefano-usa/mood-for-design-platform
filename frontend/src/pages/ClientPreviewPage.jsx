...
export default function ClientPreviewPage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`${BACKEND}/api/inspirations/public/preview/${encodeURIComponent(token)}`).then(r => {
  ...
  const ambient = useMemo(() => MODE_AMBIENT[data?.preview_mode] || MODE_AMBIENT.editorial, [data?.preview_mode]);
  ...
  const onApprove = async () => {
    await submitFeedback('approve_direction', {
  ...
  const onAlternatives = async () => {
    await submitFeedback('request_alternatives', {
      client_identifier: noteClientId || undefined
    });
    setThanks({
      title: 'Ricevuto',
  ...
  // ── States ──────────────────────────────────────────────────────
  if (loading) {
    return <div className="cp-shell cp-shell--loading" data-testid="cp-loading">
        <div className="cp-skel cp-skel--hero" />
        <p className="cp-loading-text">{t('clientpreview.client_preview.sto_preparando_la_stanza')}</p>
      </div>;
  }
  if (error === 'expired') {
    return <div className="cp-shell cp-shell--gone" data-testid="cp-expired">
        ...
        <p className="cp-sub">
          {t("ClientPreviewPage.jsx.client_preview.lo_studio_potrebbe_averla_aggiornata_chiedi_un_nuo")}
        </p>
      </div>;
  }
  if (error || !data) {
    return <div className="cp-shell cp-shell--gone" data-testid="cp-notfound">
  ...
  return <div className="cp-shell" data-testid="cp-shell" style={{
    '--cp-accent': accent
  }}>
      {/* ─── HEADER minimale ─── */}
      <header className="cp-header" data-testid="cp-header">
        <div className="cp-studio">
          <span className="cp-studio__dot" style={{
          background: accent
        }} />
      ...
      <section className="cp-hero" data-testid="cp-hero">
        ...
        {data.direction?.description && <p className="cp-sub" data-testid="cp-direction-desc">
      ...
      <main className="cp-sequence" data-testid="cp-sequence">
        {data.assets.length === 0 && <div className="cp-empty">
            <p>{t('clientpreview.client_preview.questa_direzione_e_ancora_in_fase_di_curatela')}</p>
          </div>}
        {data.assets.map((a, idx) => <article key={a.id} className="cp-frame" data-testid={`cp-frame-${a.id}`} onMouseEnter={() => recordView(a.id)}>
            <div className="cp-frame__num">
              {String(idx + 1).padStart(2, '0')}
            </div>
            <button type="button" className="cp-frame__media" onClick={() => {
          setActiveAsset(a);
          recordView(a.id);
        }} aria-label={`Apri ${a.product_name || 'asset'}`}>
              {a.file_url ? <img src={a.file_url} alt={a.alt_text || ''} loading="lazy" /> : <div className="cp-frame__placeholder"><Icons.Image size={20} /></div>}
            </button>
            <div className="cp-frame__caption">
              ...
              {a.client_note && <p className="cp-frame__note">
                  &mdash; {a.client_note}
                </p>}
              <button type="button" className="cp-frame__note-cta" onClick={() => {
            setTargetAssetForNote(a);
            setNoteOpen(true);
      ...
      <footer className="cp-footer" data-testid="cp-footer">
        ...
        <div className="cp-actions">
          <button type="button" className="cp-btn cp-btn--approve" onClick={onApprove} data-testid="cp-approve">
            <Icons.Check size={14} />
            <span>{t('clientpreview.client_preview.approvo_questa_direzione')}</span>
          </button>
          <button type="button" className="cp-btn cp-btn--alt" onClick={onAlternatives} data-testid="cp-alternatives">
            <Icons.Compass size={14} />
            <span>Vorrei esplorare alternative</span>
          </button>
          <button type="button" className="cp-btn cp-btn--note" onClick={() => {
          setTargetAssetForNote(null);
          setNoteOpen(true);
        ...
        <p className="cp-footer__sub">
          {t("ClientPreviewPage.jsx.client_preview.il_tuo_studio")} <strong>{data.studio?.name}</strong>
        </p>
      </footer>

      {/* ─── Zoom modal ─── */}
      {activeAsset && <div className="cp-zoom" role="dialog" aria-modal="true" data-testid="cp-zoom" onClick={() => setActiveAsset(null)}>
          <button type="button" className="cp-zoom__close" aria-label={t("ClientPreviewPage.jsx.client_preview.chiudi")} onClick={() => setActiveAsset(null)} data-testid="cp-zoom-close">
            <Icons.X size={16} />
          </button>
          <img src={activeAsset.file_url} alt={activeAsset.alt_text || ''} onClick={e => e.stopPropagation()} />
      ...
      {/* ─── Note modal ─── */}
      {noteOpen && <div className="cp-note-modal" role="dialog" aria-modal="true" data-testid="cp-note-modal" onClick={e => {
      if (e.target === e.currentTarget) setNoteOpen(false);
    }}>
          <div className="cp-note-card">
            <header className="cp-note-card__head">
              <p className="cp-eyebrow">
                {targetAssetForNote ? `Una nota su "${targetAssetForNote.product_name || 'questo asset'}"` : 'Una nota progettuale'}
              </p>
              <h3 className="cp-note-card__title">
                <em>{t('clientpreview.client_preview.lasciaci_sentire_la_tua_direzione')}</em>
              </h3>
            </header>
            <input type="text" placeholder={t("ClientPreviewPage.jsx.client_preview.il_tuo_nome_opzionale")} value={noteClientId} onChange={e => setNoteClientId(e.target.value)} className="cp-note-card__input" data-testid="cp-note-name" maxLength={120} />
            <textarea placeholder={t("ClientPreviewPage.jsx.client_preview.cosa_ti_emoziona_di_questa_direzione_cosa_vorresti")} value={noteText} onChange={e => setNoteText(e.target.value)} rows={5} autoFocus className="cp-note-card__textarea" data-testid="cp-note-text" maxLength={2000} />
            <div className="cp-note-card__actions">
              <button type="button" className="cp-btn cp-btn--ghost" onClick={() => {
            setNoteOpen(false);
            setNoteText('');
          }}>
                {t("ClientPreviewPage.jsx.client_preview.annulla")}
              </button>
              <button type="button" className="cp-btn cp-btn--approve" onClick={submitNote} disabled={!noteText.trim()} data-testid="cp-note-submit">
                <Icons.Send size={13} />
      ...
      {/* ─── Thanks modal ─── */}
      {thanks && <div className="cp-thanks" data-testid="cp-thanks" onClick={() => setThanks(null)}>
          <div className="cp-thanks__card">
            ...
            <button type="button" className="cp-btn cp-btn--ghost" onClick={() => setThanks(null)}>
              {t("ClientPreviewPage.jsx.client_preview.chiudi")}
            </button>
          </div>
        </div>}
    </div>;
