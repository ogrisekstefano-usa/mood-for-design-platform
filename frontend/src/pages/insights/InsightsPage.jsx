...
import { Compass, Layers, Bookmark, Users, Activity, Sparkles, TrendingUp, CircleDot } from 'lucide-react';
...
const Sparkline = ({
  ...
  return <div className="ins-spark" data-testid={`spark-${(label || '').toLowerCase().replace(/\s+/g, '-')}`}>
...
const ActivityHeatmap = ({
  ...
  return <div className="ins-heatmap" data-testid="ins-activity-heatmap">
      {surface.days.map((d, i) => {
      const v = surface.events[i];
      const intensity = v ? Math.max(0.18, v / max) : 0.04;
      return <span key={d} title={`${d} · ${v} ${v === 1 ? 'evento' : 'eventi'}`} className="ins-heatmap__cell" style={{
        backgroundColor: `rgba(127, 223, 255, ${intensity})`
      }} />;
    })}
    </div>;
};
...
const PulseRow = ({
  ...
  return <div className="ins-pulse__row" data-testid={`ins-pulse-${entry.milestone_type}`}>
      ...
      <div className="ins-pulse__bar">
        ...
        return <span key={status} className="ins-pulse__seg" title={`${status} · ${count}`} style={{
          flexBasis: `${w}%`,
          backgroundColor: STATUS_TONE[status] || '#444'
        }} />;
      })}
      </div>
    </div>;
...
const InsightsPage = () => {
  ...
  useEffect(() => {
    api.get('/api/insights/studio-overview').then(r => setData(r.data)).catch(() => setError('Impossibile caricare le pulsazioni dello studio'));
  }, []);
  const headlineCards = useMemo(() => {
    ...
    return [{
      ...
      label: 'Progetti vivi',
      value: h.projects,
      sublabel: `${h.projects_won || 0} chiusi · ${h.projects_in_progress || 0} in lavorazione`,
      tone: 'gold',
      ...
      label: 'Moodboard',
      value: h.moodboards,
      sublabel: 'composizioni curatoriali',
      tone: 'cyan',
      testid: 'stat-moodboards'
    }, {
      icon: Bookmark,
      label: 'Ispirazioni',
      value: h.inspirations,
      sublabel: 'memoria visuale',
      tone: 'pearl',
      testid: 'stat-inspirations'
    }, {
      icon: Sparkles,
      label: 'Design Journey',
      value: h.design_journeys,
      sublabel: `${h.milestones_approved || 0} pietre miliari approvate`,
      tone: 'gold',
      testid: 'stat-journeys'
    }, {
      icon: Users,
      label: 'Accounts',
      value: h.accounts,
      sublabel: `${h.leads || 0} nuovi contatti`,
      tone: 'cyan',
  ...
  if (error) {
    return <div className="ins-page ins-page--error" data-testid="ins-error">
        <p>{error}</p>
      </div>;
  }
  if (!data) {
    return <div className="ins-page ins-page--loading" data-testid="ins-loading">
        <p>{t('insights.insights.sto_leggendo_le_pulsazioni_dello_studio')}</p>
      </div>;
  }
  return <div className="ins-page" data-testid="insights-page">
      ...
      <section className="ins-grid">
        ...
        <article className="ins-card ins-card--pulse" data-testid="ins-milestone-pulse">
          ...
          <div className="ins-pulse">
            ...
            {data.milestone_pulse.length === 0 && <p className="ins-empty">{t('insights.insights.nessuna_pietra_miliare_ancora_attiva')}</p>}
          </div>
        </article>
          ...
          <div className="ins-sig">
            <div className="ins-sig__col">
              ...
              <ul className="ins-sig__tags">
                {data.signature.tags.slice(0, 8).map(t => <li key={t.label} className="ins-sig__chip">
                    <span>{t.label}</span>
                    <em>{t.count}</em>
                  </li>)}
            ...
            <div className="ins-sig__col">
              ...
              <ul className="ins-sig__tags">
                {data.signature.colors.slice(0, 6).map(t => <li key={t.label} className="ins-sig__chip ins-sig__chip--color">
                    <span>{t.label}</span>
                    <em>{t.count}</em>
                  </li>)}
      ...
      <footer className="ins-foot">
        <p>Aggiornato · {new Date(data.generated_at).toLocaleString('it-IT', {
          dateStyle: 'medium',
          timeStyle: 'short'
        })}</p>
      </footer>
