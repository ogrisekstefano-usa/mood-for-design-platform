...
const ClientOverviewPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const {
          data: d
        } = await api.get('/api/client/overview');
        if (alive) setData(d);
      } catch (e) {
        if (alive) {
  ...
  if (loading) {
    return <div data-testid="client-overview-loading" className="py-20 flex items-center justify-center">
...
const HasDataExperience = ({
  ...
  return <div data-testid="client-overview-data" className="max-w-[1280px] space-y-8">
      {/* HERO PROJECT CARD */}
      <section data-testid="client-project-hero" className="cp-card relative overflow-hidden grid grid-cols-12 gap-0 min-h-[400px]">
        <div className="col-span-12 lg:col-span-7 relative z-10 px-10 lg:px-12 py-10 lg:py-12 flex flex-col justify-center">
          ...
          {project.project_type && <p className="mt-5 text-[14px] text-[var(--cp-text-secondary)] font-body">
              {project.project_type}
            </p>}
          {project.location && <p className="mt-1 text-[14px] text-[var(--cp-text-muted)] font-body">
              {project.location}
            </p>}
          <div className="mt-8">
            <a href="/client/project" data-testid="client-project-cta" className="cp-cta-gold inline-flex items-center gap-2 px-6 py-3 text-[12px] uppercase tracking-[0.18em]">
              Vai al progetto
        ...
        <div className="col-span-12 lg:col-span-5 relative min-h-[260px]">
          {project.cover_url ? <img src={project.cover_url} alt="" loading="eager" className="absolute inset-0 w-full h-full object-cover" /> : <div className="absolute inset-0 bg-gradient-to-br from-[var(--cp-surface-3)] to-[var(--cp-surface-2)]" />}
          <div aria-hidden className="absolute inset-0 pointer-events-none" style={{
          background: 'linear-gradient(90deg, var(--cp-surface-1) 0%, transparent 36%, transparent 100%)'
        }} />
        </div>

        {/* Inset side card "Stato attuale" — desktop only */}
        <aside data-testid="client-status-card" className="hidden lg:flex absolute right-8 top-1/2 -translate-y-1/2 z-20
      ...
      <section data-testid="client-timeline-card" className="cp-card px-10 py-10">
        <header className="mb-9 flex items-end justify-between">
          ...
          <a href="/client/timeline" data-testid="client-timeline-full-link" className="text-[11px] uppercase tracking-[0.18em] text-[var(--cp-text-muted)]
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <article data-testid="client-moodboards-card" className="cp-card p-8 lg:col-span-7">
          ...
          {moodboards.length === 0 ? <EmptyHint icon={Sparkles} copy="Il nostro team sta preparando le tue moodboard." /> : <div className="grid grid-cols-3 gap-4">
              {moodboards.slice(0, 3).map(mb => <a key={mb.id} href={`/client/moodboards`} className="block group">
                  <div className="aspect-[4/3] rounded-[var(--cp-radius-sm)] overflow-hidden border border-[var(--cp-border)] bg-[var(--cp-surface-3)]">
                    {mb.cover_url ? <img src={mb.cover_url} alt="" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.02]" /> : <div className="w-full h-full grid place-items-center text-[var(--cp-text-subtle)]">
                        <Sparkles size={20} strokeWidth={1.2} />
                      </div>}
                  </div>
                  <p className="mt-3 text-[13px] text-[var(--cp-text-primary)] font-body leading-tight">{mb.title}</p>
                  {mb.status && <p className="text-[11px] text-[var(--cp-text-muted)] mt-0.5 capitalize">{mb.status}</p>}
                </a>)}
            </div>}
        </article>
        ...
        <article data-testid="client-approvals-card" className="cp-card p-8 lg:col-span-5">
          ...
          {approvals_pending.length === 0 ? <EmptyHint icon={Sparkles} copy="Le prime proposte del tuo progetto stanno arrivando." /> : <ul className="space-y-4">
              {approvals_pending.slice(0, 3).map(p => <li key={p.id} className="flex items-start justify-between gap-3 pb-4 border-b border-[var(--cp-border)] last:border-b-0 last:pb-0">
                  <div>
...
const EmptyHint = ({
  ...
  copy
    ...
    <span aria-hidden className="w-9 h-9 rounded-full flex items-center justify-center
