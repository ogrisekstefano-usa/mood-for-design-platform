...
import { Plus, Copy, CalendarClock, Calendar, Compass, FolderOpen, ChevronRight, Loader2, X, BookOpen } from 'lucide-react';
...
const FLOW_STAGES = [{
  id: 'master',
  label: 'Master',
  desc: 'Direzione editoriale globale'
}, {
  id: 'editions',
  label: 'Market Editions',
  desc: 'Versioni culturalmente native'
}, {
  id: 'review',
  label: 'Review',
  desc: 'Approvazione publication'
}, {
  id: 'schedule',
  label: 'Schedule',
  desc: 'Programmazione internazionale'
}, {
  id: 'publish',
  label: 'Publish',
  desc: 'Live sulle superfici pubbliche'
}];
...
const NewMasterModal = ({
  ...
  return <div data-testid="new-master-modal" className="fixed inset-0 z-[55] bg-black/72 backdrop-blur-sm flex items-center justify-center p-6" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-lg bg-[var(--bp-surface-elevated)] border border-[var(--bp-border)] rounded-[14px] shadow-2xl">
        <header className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-[var(--bp-border)]">
          ...
          <button type="button" onClick={onClose} data-testid="new-master-close" className="text-[var(--bp-text-muted)] hover:text-[var(--bp-text-primary)]">
            <X size={16} />
          </button>
        </header>
        <div className="px-6 py-5 space-y-4">
          <div>
            ...
            <input value={code} onChange={e => setCode(e.target.value)} placeholder="es. mediterranean-spring-2026" data-testid="new-master-code" className="w-full bg-[var(--bp-surface-1)] border border-[var(--bp-border)] rounded-[7px] py-2 px-3 text-[13.5px] text-[var(--bp-text-primary)] font-mono focus:outline-none focus:border-[var(--bp-border-hover)]" />
          </div>
          <div>
            <label className="block text-[10px] uppercase tracking-[0.22em] text-[var(--bp-text-faint)] mb-1.5">Titolo</label>
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="es. Atmosfere mediterranee 2026" data-testid="new-master-title" className="w-full bg-[var(--bp-surface-1)] border border-[var(--bp-border)] rounded-[7px] py-2 px-3 text-[13.5px] text-[var(--bp-text-primary)] focus:outline-none focus:border-[var(--bp-border-hover)]" />
          </div>
          <div>
            <label className="block text-[10px] uppercase tracking-[0.22em] text-[var(--bp-text-faint)] mb-1.5">Locale canonico</label>
            <select value={locale} onChange={e => setLocale(e.target.value)} data-testid="new-master-locale" className="w-full bg-[var(--bp-surface-1)] border border-[var(--bp-border)] rounded-[7px] py-2 px-3 text-[13.5px] text-[var(--bp-text-primary)] focus:outline-none focus:border-[var(--bp-border-hover)] cursor-pointer">
          ...
          <div>
            ...
            <textarea value={direction} onChange={e => setDirection(e.target.value)} rows={3} placeholder={t("editorial.market_editions_toolbar.una_frase_che_cattura_l_atmosfera_non_un_brief_tec")} data-testid="new-master-direction" className="w-full bg-[var(--bp-surface-1)] border border-[var(--bp-border)] rounded-[7px] py-2 px-3 text-[13px] text-[var(--bp-text-primary)] focus:outline-none focus:border-[var(--bp-border-hover)] resize-none" />
          </div>
        ...
        <footer className="flex items-center justify-end gap-2 px-6 py-4 border-t border-[var(--bp-border)] bg-[var(--bp-surface-1)] rounded-b-[14px]">
          ...
          <button type="button" onClick={submit} disabled={saving} data-testid="new-master-submit" className="inline-flex items-center gap-2 px-4 py-2 rounded-[8px] bg-[var(--bp-primary)] text-black hover:opacity-90 text-[12.5px] font-medium disabled:opacity-50">
...
const NewMarketEditionModal = ({
  ...
  const submit = async () => {
    if (!master?.id) {
      setError('Seleziona prima un Editorial Master');
      return;
    }
  ...
  return <div data-testid="new-edition-modal" className="fixed inset-0 z-[55] bg-black/72 backdrop-blur-sm flex items-center justify-center p-6" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-xl bg-[var(--bp-surface-elevated)] border border-[var(--bp-border)] rounded-[14px] shadow-2xl">
        <header className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-[var(--bp-border)]">
          <div>
        ...
        <div className="px-6 py-5 space-y-4">
          {!master?.id && <p className="text-[12px] text-amber-400 font-body italic">Seleziona prima un Editorial Master dalla colonna a sinistra.</p>}
          <div>
            <label className="block text-[10px] uppercase tracking-[0.22em] text-[var(--bp-text-faint)] mb-2">Mercato</label>
            <div className="grid grid-cols-2 gap-2">
              {(markets || []).map(m => <button key={m.id} type="button" onClick={() => pickMarket(m)} data-testid={`new-edition-market-${m.code}`} className={`text-left p-3 rounded-[8px] border transition-colors ${marketId === m.id ? 'border-[var(--bp-primary)] bg-[var(--bp-primary-soft)]' : 'border-[var(--bp-border)] bg-[var(--bp-surface-1)] hover:border-[var(--bp-border-hover)]'}`}>
              ...
              {(markets || []).length === 0 && <p className="text-[12px] text-[var(--bp-text-muted)] font-body italic col-span-2 py-3 text-center">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={targetLocale} onChange={e => setTargetLocale(e.target.value)} placeholder="es. en-AE" data-testid="new-edition-locale" className="w-full bg-[var(--bp-surface-1)] border border-[var(--bp-border)] rounded-[7px] py-2 px-3 text-[13px] font-mono text-[var(--bp-text-primary)] focus:outline-none focus:border-[var(--bp-border-hover)]" />
            </div>
            <div>
              <label className="block text-[10px] uppercase tracking-[0.22em] text-[var(--bp-text-faint)] mb-1.5">Variant slug</label>
              <input value={variantSlug} onChange={e => setVariantSlug(e.target.value)} placeholder="es. spring-2026-ae" data-testid="new-edition-slug" className="w-full bg-[var(--bp-surface-1)] border border-[var(--bp-border)] rounded-[7px] py-2 px-3 text-[13px] font-mono text-[var(--bp-text-primary)] focus:outline-none focus:border-[var(--bp-border-hover)]" />
            </div>
          </div>
          {error && <p className="text-[12px] text-red-400 font-body">{error}</p>}
        </div>
        <footer className="flex items-center justify-end gap-2 px-6 py-4 border-t border-[var(--bp-border)] bg-[var(--bp-surface-1)] rounded-b-[14px]">
          ...
          <button type="button" onClick={submit} disabled={saving || !master?.id} data-testid="new-edition-submit" className="inline-flex items-center gap-2 px-4 py-2 rounded-[8px] bg-[var(--bp-primary)] text-black hover:opacity-90 text-[12.5px] font-medium disabled:opacity-50">
            {saving ? <Loader2 size={12} className="animate-spin" /> : <Plus size={12} />}
...
const ScheduleModal = ({
  ...
  return <div data-testid="schedule-modal" className="fixed inset-0 z-[55] bg-black/72 backdrop-blur-sm flex items-center justify-center p-6" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-md bg-[var(--bp-surface-elevated)] border border-[var(--bp-border)] rounded-[14px] shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-3">
          ...
          <input type="datetime-local" value={when} onChange={e => setWhen(e.target.value)} data-testid="schedule-datetime" className="w-full bg-[var(--bp-surface-1)] border border-[var(--bp-border)] rounded-[7px] py-2 px-3 text-[13px] text-[var(--bp-text-primary)] focus:outline-none focus:border-[var(--bp-border-hover)]" />
        </div>
        <footer className="flex items-center justify-end gap-2 px-6 py-4 border-t border-[var(--bp-border)] bg-[var(--bp-surface-1)] rounded-b-[14px]">
          <button type="button" onClick={onClose} className="px-3 py-2 text-[12px] text-[var(--bp-text-muted)]">{t('editorial.market_editions_toolbar.annulla')}</button>
          <button type="button" onClick={submit} disabled={saving} data-testid="schedule-submit" className="inline-flex items-center gap-2 px-4 py-2 rounded-[8px] bg-[var(--bp-primary)] text-black hover:opacity-90 text-[12.5px] font-medium disabled:opacity-50">
...
const FlowStrip = ({
  activeStage
    ...
    return <React.Fragment key={stage.id}>
          <div className={`me-flow-step ${active ? 'is-active' : ''} ${passed ? 'is-passed' : ''}`} data-testid={`me-flow-step-${stage.id}`}>
  ...
  return <>
      <div className="me-toolbar" data-testid="me-toolbar">
        <div className="me-toolbar__primary">
          ...
          <button type="button" className="me-btn" data-testid="me-cta-schedule" onClick={() => setShowSchedule(true)} disabled={!selectedVariant?.id || !['approved', 'scheduled'].includes(selectedVariant?.status)} title={!selectedVariant?.id ? 'Seleziona una Market Edition' : !['approved', 'scheduled'].includes(selectedVariant?.status) ? 'La Market Edition deve essere approvata prima della programmazione' : ''}>
            <CalendarClock size={12} strokeWidth={1.6} />
            <span>Programma</span>
          </button>
        </div>
        <div className="me-toolbar__secondary">
          <button type="button" className="me-btn me-btn--ghost" data-testid="me-cta-open-calendar" onClick={() => navigate('/blueprint/editorial-calendar')}>
            <Calendar size={12} strokeWidth={1.6} />
            <span>{t('editorial.market_editions_toolbar.apri_calendario')}</span>
          </button>
          <button type="button" className="me-btn me-btn--ghost" data-testid="me-cta-from-pinterest" onClick={() => navigate('/workspace/references?openAdd=1')}>
            <Compass size={12} strokeWidth={1.6} />
            <span>Da Pinterest</span>
          </button>
          <button type="button" className="me-btn me-btn--ghost" data-testid="me-cta-from-project" onClick={() => navigate('/blueprint/projects-studio')}>
      ...
      <NewMasterModal open={showNewMaster} onClose={() => setShowNewMaster(false)} onCreated={onReload} />
      <NewMarketEditionModal open={showNewEdition} master={selectedMaster} markets={markets} onClose={() => setShowNewEdition(false)} onCreated={onReload} />
      <ScheduleModal open={showSchedule} variant={selectedVariant} onClose={() => setShowSchedule(false)} onScheduled={onReload} />
    </>;
};
