...
export default function MoodboardPickerModal({
  asset,
  onClose,
  onAdded
}) {
  ...
  useEffect(() => {
    api.get('/api/moodboards?limit=40').then(r => {
      const payload = r.data;
      let items = [];
      if (Array.isArray(payload)) items = payload;else if (Array.isArray(payload?.items)) items = payload.items;else if (Array.isArray(payload?.data)) items = payload.data;else if (Array.isArray(payload?.moodboards)) items = payload.moodboards;
      setMoodboards(items);
    }).catch(() => setMoodboards([])).finally(() => setLoading(false));
  }, []);
  ...
  const createNewMoodboard = async () => {
    ...
    if (!title) {
      toast.error('Indica un titolo');
      return;
    }
    try {
      const r = await api.post('/api/moodboards', {
        title
      });
      // POST /api/moodboards returns the moodboard object flat (id at root)
      const mb = r.data?.item || r.data?.data || r.data;
      if (!mb?.id) {
        toast.error('Risposta server non valida');
        return;
      }
  ...
  return <div className="mp-modal" role="dialog" aria-modal="true" data-testid="mp-modal" onClick={e => {
    if (e.target === e.currentTarget) onClose();
      ...
      <div className="mp-modal__card" onClick={e => e.stopPropagation()}>
        <header className="mp-modal__head">
          ...
          <button type="button" className="mp-modal__close" onClick={onClose} aria-label={t("inspirations.moodboard_picker.chiudi")} data-testid="mp-close">
            <Icons.X size={14} />
          </button>
        </header>
        ...
        <div className="mp-modal__body">
          <div className="mp-modal__search">
            ...
            <input type="text" value={search} placeholder={t("inspirations.moodboard_picker.cerca_tra_i_tuoi_moodboard")} onChange={e => setSearch(e.target.value)} data-testid="mp-search" />
          </div>
          ...
          <div className="mp-modal__list" data-testid="mp-list">
            ...
            {!loading && filtered.length === 0 && <p className="mp-modal__empty">
                {t("inspirations.moodboard_picker.nessun_moodboard")} {search ? 'trovato' : 'ancora'}{t("inspirations.moodboard_picker.crea_il_primo")}
              </p>}
            {filtered.map(mb => <button key={mb.id} type="button" className={`mp-row ${busy === mb.id ? 'is-busy' : ''}`} onClick={() => addToMoodboard(mb)} disabled={busy === mb.id} data-testid={`mp-row-${mb.id}`}>
                <div className="mp-row__cover">
                  {mb.cover_url ? <img src={mb.cover_url} alt="" loading="lazy" /> : <Icons.LayoutGrid size={14} />}
                </div>
          ...
          {creating ? <div className="mp-modal__create" data-testid="mp-create-panel">
              <input type="text" placeholder={t("inspirations.moodboard_picker.titolo_nuovo_moodboard")} value={newTitle} onChange={e => setNewTitle(e.target.value)} autoFocus data-testid="mp-create-title" />
              <button type="button" className="mp-btn mp-btn--primary" onClick={createNewMoodboard} data-testid="mp-create-confirm">
                {t("inspirations.moodboard_picker.crea_e_aggiungi")}
              </button>
              <button type="button" className="mp-btn" onClick={() => {
            setCreating(false);
            setNewTitle('');
          }}>
                {t("inspirations.moodboard_picker.annulla")}
              </button>
            </div> : <button type="button" className="mp-modal__create-trigger" onClick={() => setCreating(true)} data-testid="mp-create-trigger">
              <Icons.Plus size={13} />
