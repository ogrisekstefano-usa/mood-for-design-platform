...
const BUCKET_ORDER = ['lifestyle', 'still_life', 'cutouts', 'details', 'textures', 'material_samples', 'variants', 'campaigns', 'renderings', 'technicals'];
...
const COMPOSITION_MODES = [{
  ...
  bucketOrder: ['lifestyle', 'campaigns', 'still_life', 'details', 'cutouts', 'textures', 'material_samples', 'variants', 'renderings', 'technicals'],
  score: c => (c.asset_type === 'lifestyle' || c.asset_type === 'campaign' ? 4 : 0) + (c.editorial_score || 0) * 3 + (c.visual_weight || 0) * 1.5
}, {
  key: 'composition',
  label: 'Composition',
  sublabel: 'atelier creativo',
  bucketOrder: ['still_life', 'cutouts', 'details', 'material_samples', 'textures', 'lifestyle', 'variants', 'renderings', 'campaigns', 'technicals'],
  score: c => (c.asset_type === 'still_life' || c.asset_type === 'cutout' || c.asset_type === 'detail' ? 4 : 0) + (c.composition_friendly || 0) * 4 + (c.moodboard_priority || 0) * 0.5
}, {
  key: 'material',
  label: 'Material',
  sublabel: 'materioteca contemporanea',
  bucketOrder: ['textures', 'material_samples', 'details', 'cutouts', 'still_life', 'lifestyle', 'variants', 'renderings', 'campaigns', 'technicals'],
  score: c => (c.asset_type === 'texture' || c.asset_type === 'material_sample' ? 4 : 0) + (c.asset_type === 'detail' ? 2 : 0) + (c.texture_repetition_score || 0) * 2
}, {
  key: 'storytelling',
  label: 'Storytelling',
  sublabel: 'narrativa cinematica',
  bucketOrder: ['lifestyle', 'campaigns', 'renderings', 'still_life', 'details', 'cutouts', 'textures', 'material_samples', 'variants', 'technicals'],
  score: c => (c.editorial_score || 0) * 4 + (c.asset_type === 'lifestyle' || c.asset_type === 'campaign' || c.asset_type === 'rendering' ? 3 : 0) + (c.visual_weight || 0) * 1.2
}];

// Quick filters — pure UI predicates on atlas cards
const QUICK_FILTERS = [{
  key: 'moodboard_ready',
  label: 'Moodboard ready',
  ...
  key: 'composition',
  label: 'Composition friendly',
  test: c => (c.composition_friendly || 0) >= 0.75
}, {
  key: 'high_weight',
  label: 'High visual weight',
...
const AssetBadges = ({
  ...
  const b = [];
  if ((asset.moodboard_priority || 0) >= 4) b.push({
    k: 'mb',
    label: 'Moodboard ready',
    intent: 'cyan'
  });
  if ((asset.editorial_score || 0) >= 0.78) b.push({
    k: 'ed',
    label: 'Editorial',
    intent: 'warm'
  });
  if ((asset.composition_friendly || 0) >= 0.80) b.push({
    k: 'comp',
    label: 'Composition',
    intent: 'cyan'
  });
  if (asset.asset_type === 'texture') b.push({
    k: 'tx',
    label: 'Texture',
    intent: 'plain'
  });
  if (asset.asset_type === 'material_sample') b.push({
    k: 'ms',
    label: 'Materia',
    intent: 'plain'
  });
  if (asset.asset_type === 'detail') b.push({
    k: 'dt',
    label: 'Dettaglio',
    intent: 'plain'
  });
  if (asset.asset_type === 'still_life') b.push({
    k: 'sl',
    label: 'Still life',
    intent: 'plain'
  });
  if ((asset.visual_weight || 0) >= 0.78) b.push({
    k: 'vw',
    label: 'High visual weight',
    intent: 'warm'
  });
  return <div className="pg-asset-badges">
...
const AssetTile = ({
  ...
  isHero = false,
  isSaved = false
}) => <button type="button" className={`pg-tile ${isHero ? 'pg-tile--hero' : ''}`} data-testid={`pg-tile-${asset.id}`} onClick={() => onPick(asset)}>
    <div className="pg-tile__media">
      {asset.file_url ? <img src={asset.file_url} alt={asset.alt_text || ''} loading="lazy" /> : <div className="pg-tile__placeholder"><Icons.Image size={18} /></div>}
      <div className="pg-tile__overlay">
        <AssetBadges asset={asset} />
        <span className={`pg-tile__save ${isSaved ? 'is-saved' : ''}`} role="button" tabIndex={-1} onClick={e => {
        e.stopPropagation();
        onSave(asset);
...
const Bucket = ({
  ...
  return <section className="pg-bucket" data-testid={`pg-bucket-${name}`}>
      <header className="pg-bucket__head">
        <button type="button" className="pg-bucket__toggle" onClick={() => setOpen(o => !o)} data-testid={`pg-bucket-toggle-${name}`}>
      ...
      {open && <div className="pg-bucket__grid">
          {items.map(it => <AssetTile key={it.id} asset={it} onPick={onPick} onSave={onSave} isSaved={savedIds.has(it.id)} />)}
        </div>}
    </section>;
};
...
const AssetInfoTab = ({
  ...
  return <div className="pg-info" data-testid="pg-tab-asset-info">
      ...
      {(asset.mood_tags?.length || 0) > 0 && <div className="pg-info__chips">
      ...
      {(asset.dominant_color_palette?.length || 0) > 0 && <div className="pg-info__palette" data-testid="pg-palette">
          <p className="pg-info__chips-label">Palette dominante</p>
          <div className="pg-info__palette-row">
            {asset.dominant_color_palette.slice(0, 5).map((p, i) => <span key={i} className="pg-swatch" style={{
          background: p.hex
        }} title={p.hex} />)}
          </div>
        </div>}
...
const StudioLanguageWidget = () => {
  ...
  useEffect(() => {
    api.get('/api/inspirations/usage-memory/studio-language?days=90').then(r => setData(r.data)).catch(() => setData(null)).finally(() => setLoading(false));
  }, []);
  if (loading) return null;
  if (!data) return null;
  return <div className="pg-studio-lang" data-testid="pg-studio-language">
      <p className="pg-info__chips-label">{t('inspirations.product_gallery.linguaggio_progettuale_dello_studio')}</p>
      {(data.atmospheres || []).length > 0 && <div className="pg-studio-lang__row">
          <span className="pg-studio-lang__lbl">Atmosfere</span>
          <div className="pg-studio-lang__chips">
            {data.atmospheres.slice(0, 5).map(a => <span key={a.label} className={`pg-chip pg-chip--plain pg-studio-lang__chip is-${a.presence}`}>
                {a.label}
              </span>)}
          </div>
        </div>}
      {(data.materialities || []).length > 0 && <div className="pg-studio-lang__row">
          <span className="pg-studio-lang__lbl">Materialità</span>
          <div className="pg-studio-lang__chips">
            {data.materialities.slice(0, 5).map(m => <span key={m.label} className={`pg-chip pg-chip--plain pg-studio-lang__chip is-${m.presence}`}>
                {m.label}
              </span>)}
          </div>
        </div>}
      {(data.narrative_threads || []).length > 0 && <p className="pg-studio-lang__narrative">
...
const ClientPreviewLinkRow = ({
  ...
  const reload = useCallback(() => {
    api.get(`/api/inspirations/references/collections/${collection.id}/preview-links`).then(r => setLinks(r.data?.items || [])).catch(() => setLinks([]));
  }, [collection.id]);
  useEffect(() => {
    reload();
  }, [reload]);
  const onGenerate = async () => {
    setGenerating(true);
    try {
      const r = await api.post(`/api/inspirations/references/collections/${collection.id}/preview-links`, {
        preview_mode: 'editorial',
  ...
  const onCopy = token => {
    ...
    navigator.clipboard.writeText(url).then(() => toast.success('Link copiato'), () => toast.error('Copia non riuscita'));
  };
  ...
  return <div className="pg-preview-link" data-testid={`pg-preview-link-${collection.id}`}>
      {active.length === 0 ? <button type="button" className="pg-preview-link__cta" onClick={onGenerate} disabled={generating} data-testid={`pg-preview-generate-${collection.id}`}>
          <Icons.Send size={11} />
          <span>{generating ? 'Sto preparando…' : 'Genera link cliente'}</span>
        </button> : active.map(link => <div key={link.id} className="pg-preview-link__row">
            <button type="button" className="pg-preview-link__copy" onClick={() => onCopy(link.token)} data-testid={`pg-preview-copy-${link.id}`} title="Copia link">
              <Icons.Link size={11} />
            ...
            <button type="button" className="pg-preview-link__action" onClick={() => setFeedbackOpenId(feedbackOpenId === link.id ? null : link.id)} title={t('inspirations.product_gallery.feedback_title')} data-testid={`pg-preview-feedback-${link.id}`}>
              <Icons.MessageCircle size={11} />
            </button>
            <button type="button" className="pg-preview-link__action pg-preview-link__action--danger" onClick={() => onRevoke(link.id)} title="Revoca" data-testid={`pg-preview-revoke-${link.id}`}>
              <Icons.X size={11} />
            </button>
            {feedbackOpenId === link.id && <ClientFeedbackPanel linkId={link.id} />}
          </div>)}
    </div>;
};
const ClientFeedbackPanel = ({
  linkId
}) => {
  const {
    t
  ...
  const [data, setData] = useState(null);
  useEffect(() => {
    api.get(`/api/inspirations/references/preview-links/${linkId}/feedback`).then(r => setData(r.data)).catch(() => setData(null));
  }, [linkId]);
  if (!data) return null;
  return <div className="pg-preview-feedback" data-testid={`pg-preview-feedback-panel-${linkId}`}>
      ...
      {(data.feedback || []).slice(0, 5).map(f => <div key={f.id} className="pg-preview-feedback__item">
      ...
      {data.feedback.length === 0 && <p className="pg-preview-feedback__empty">{t('inspirations.product_gallery.in_attesa_di_riscontro_dal_cliente')}</p>}
    </div>;
};
const ReferencesTab = ({
  collections,
  ...
  savedInstances
    ...
    {savedInstances && savedInstances.length > 0 && <div className="pg-refs__saved-banner" data-testid="pg-refs-saved-banner">
        Già salvato in {savedInstances.length} {savedInstances.length === 1 ? 'collezione' : 'collezioni'}
      </div>}
    <ul className="pg-refs__list">
      {(collections || []).map(c => <li key={c.id} className="pg-refs__item" data-testid={`pg-refs-item-${c.id}`}>
          <button type="button" className="pg-refs__item-btn" onClick={() => onAddToCollection(c)}>
            <div className="pg-refs__cover">
              {c.cover_asset?.file_url ? <img src={c.cover_asset.file_url} alt="" loading="lazy" /> : <Icons.Layers size={14} />}
            </div>
          ...
          {c.visibility === 'client_visible' && <ClientPreviewLinkRow collection={c} />}
        </li>)}
      {(!collections || collections.length === 0) && <li className="pg-refs__empty">
...
const RelatedTab = ({
  ...
  return <div className="pg-related" data-testid="pg-tab-related">
    ...
    {!loading && (items || []).length === 0 && <p className="pg-related__empty">{t('inspirations.product_gallery.ancora_nessuna_affinita_curatoriale_rilevata')}</p>}
    <ul className="pg-related__list">
      {(items || []).slice(0, 8).map(it => <li key={it.id} className="pg-related__item">
          <button type="button" className="pg-related__item-btn" onClick={() => onPick(it)} data-testid={`pg-related-${it.id}`}>
            <div className="pg-related__media">
              {it.file_url ? <img src={it.file_url} alt="" loading="lazy" /> : <Icons.Image size={14} />}
            </div>
...
const FloatingTray = ({
  ...
  return <div className="pg-tray" data-testid="pg-tray" role="status">
      <Icons.Check size={14} className="pg-tray__check" />
      <span className="pg-tray__msg">{message}</span>
      <div className="pg-tray__actions">
        {(actions || []).map(a => <button key={a.key} type="button" className="pg-tray__act" onClick={() => {
        a.onClick();
        onClose();
      }} data-testid={`pg-tray-${a.key}`}>
            {a.label}
          </button>)}
        <button type="button" className="pg-tray__close" onClick={onClose} aria-label={t('common.close')}>
          <Icons.X size={12} />
        </button>
      </div>
    </div>;
...
export default function ProductGalleryPage() {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    api.get(`/api/inspirations/registry/products/${productId}/visual-assets`).then(r => {
      setAtlas(r.data);
      const initialHero = r.data.hero || r.data.gallery && r.data.gallery[0] || null;
      setHero(initialHero);
      setActiveAsset(initialHero);
    }).catch(e => {
      if (e?.response?.status === 404) setError('Product non trovato');else setError('Errore nel caricamento del Product Visual Atlas™');
    }).finally(() => setLoading(false));
  }, [productId]);

  // ── Load collections ────────────────────────────────────────────
  const reloadCollections = useCallback(() => {
    api.get('/api/inspirations/references/collections').then(r => setCollections(r.data?.items || [])).catch(() => setCollections([]));
  }, []);
  useEffect(() => {
    reloadCollections();
  }, [reloadCollections]);
  ...
  useEffect(() => {
    ...
    api.get(`/api/inspirations/references/by-asset/${activeAsset.id}`).then(r => {
  ...
  useEffect(() => {
    ...
    api.get(`/api/inspirations/registry/products/${activeAsset.id}/related?limit=12`).then(r => setRelated({
      items: r.data?.items || [],
      loading: false
    })).catch(() => setRelated({
      items: [],
  ...
  const activeModeDef = useMemo(() => COMPOSITION_MODES.find(m => m.key === mode) || COMPOSITION_MODES[1], [mode]);
  ...
  useEffect(() => {
    ...
    const scored = [...atlas.gallery].sort((a, b) => activeModeDef.score(b) - activeModeDef.score(a));
    const top = scored[0];
  ...
  const onSaveAsset = useCallback((asset, collectionId = null) => {
    api.post('/api/inspirations/references/save', {
      ...
      const collTitle = collectionId ? collections.find(c => c.id === collectionId)?.title || 'collezione' : 'Curated References™';
      setTray({
        message: r.data.created ? `Salvato in ${collTitle}` : `Già presente in ${collTitle}`,
        actions: [...(collectionId ? [{
          key: 'open',
          label: 'Apri collezione',
          onClick: () => navigate(`/inspirations/references/${collectionId}`)
        }] : []), {
          key: 'moodboard',
          label: 'Aggiungi al moodboard',
          onClick: () => setMoodboardPicker({
            asset
          })
        }]
      });
      ...
      if (asset.id === activeAsset?.id) {
        api.get(`/api/inspirations/references/by-asset/${asset.id}`).then(rr => setSavedInstancesForActive(rr.data?.instances || []));
      }
    }).catch(() => toast.error('Salvataggio non riuscito'));
  }, [collections, activeAsset?.id, navigate, reloadCollections]);
  const onCreateCollection = () => setCollectionDrawer({
    open: true
  });
  ...
  // ── Render ──────────────────────────────────────────────────────
  if (loading) {
    return <div className="pg-shell pg-shell--loading" data-testid="pg-loading">
  ...
  if (error) {
    return <div className="pg-shell pg-shell--error" data-testid="pg-error">
  ...
  return <div className="pg-shell" data-testid="pg-shell">
      ...
      <header className="pg-header">
        ...
        <nav className="pg-modes" role="tablist" aria-label="Composition Modes" data-testid="pg-modes">
          {COMPOSITION_MODES.map(m => <button key={m.key} type="button" role="tab" aria-selected={mode === m.key} className={`pg-mode ${mode === m.key ? 'is-on' : ''}`} onClick={() => setMode(m.key)} data-testid={`pg-mode-${m.key}`} title={m.sublabel}>
      ...
      <div className="pg-body">
        ...
        <aside className="pg-aside pg-aside--left" data-testid="pg-aside-left">
          <section className="pg-aside__group">
            ...
            <div className="pg-quick">
              {QUICK_FILTERS.map(f => <button key={f.key} type="button" className={`pg-pill ${quickFilters.has(f.key) ? 'is-on' : ''}`} onClick={() => setQuickFilters(s => {
              const n = new Set(s);
          ...
          <section className="pg-aside__group">
            ...
            <div className="pg-meta-list">
              {(atlas.metadata?.mood_tags || []).slice(0, 10).map(t => <span key={t} className="pg-chip pg-chip--plain">{t}</span>)}
              {(atlas.metadata?.mood_tags || []).length === 0 && <p className="pg-aside__empty">{t('inspirations.product_gallery.atmosfera_in_lettura_curatoriale')}</p>}
            </div>
          </section>
          ...
          <section className="pg-aside__group">
            ...
            <div className="pg-aside__palette">
              {(atlas.metadata?.palette_aggregate || []).slice(0, 6).map((p, i) => <span key={i} className="pg-swatch" style={{
              background: p.hex
            }} title={p.hex} />)}
            </div>
          </section>
        ...
        <main className="pg-main" data-testid="pg-main">
          {/* HERO */}
          <section className={`pg-hero ${fullscreen ? 'pg-hero--fullscreen' : ''}`} data-testid="pg-hero">
            <div className="pg-hero__media">
              {hero?.file_url ? <img src={hero.file_url} alt={hero.alt_text || ''} /> : <div className="pg-hero__placeholder"><Icons.Image size={24} /></div>}
            </div>
            <div className="pg-hero__actions">
              <button type="button" className="pg-btn pg-btn--ghost" onClick={() => setFullscreen(f => !f)} data-testid="pg-hero-fullscreen">
                <Icons.Maximize2 size={13} />
                <span>{fullscreen ? 'Riduci' : 'Fullscreen'}</span>
              </button>
              <button type="button" className="pg-btn pg-btn--primary" onClick={() => activeAsset && onSaveAsset(activeAsset)} data-testid="pg-hero-save">
                <Icons.Bookmark size={13} />
                <span>{t('inspirations.product_gallery.salva_in_references')}</span>
              </button>
              <button type="button" className="pg-btn pg-btn--accent" onClick={onAddActiveToMoodboard} data-testid="pg-hero-moodboard">
          ...
          <section className="pg-stream" data-testid="pg-stream">
            {renderOrder.map(name => <Bucket key={name} name={name} items={filteredBuckets[name]} onPick={onPickAsset} onSave={onSaveAsset} savedIds={savedIds} />)}
            {(atlas.gallery?.length || 0) === 0 && <div className="pg-empty">
        ...
        <aside className="pg-aside pg-aside--right" data-testid="pg-aside-right">
          <nav className="pg-tabs" role="tablist">
            {TABS.map(t => <button key={t} type="button" role="tab" aria-selected={activeTab === t} className={`pg-tab ${activeTab === t ? 'is-on' : ''}`} onClick={() => setActiveTab(t)} data-testid={`pg-tab-${t}`}>
                {t === 'asset_info' && 'Asset'}
          ...
          <div className="pg-tab-body">
            ...
            {activeTab === 'references' && <ReferencesTab collections={collections} onCreate={onCreateCollection} onAddToCollection={coll => activeAsset && onSaveAsset(activeAsset, coll.id)} currentAsset={activeAsset} savedInstances={savedInstancesForActive} />}
            {activeTab === 'related' && <RelatedTab items={related.items} loading={related.loading} onPick={onPickAsset} />}
          </div>
        </aside>
      </div>

      {/* Floating tray */}
      {tray && <FloatingTray message={tray.message} actions={tray.actions} onClose={() => setTray(null)} />}

      {/* Modals */}
      {moodboardPicker && <MoodboardPickerModal asset={moodboardPicker.asset} onClose={() => setMoodboardPicker(null)} onAdded={mbId => {
      setMoodboardPicker(null);
      setTray({
        message: 'Asset aggiunto al moodboard',
        actions: [{
          key: 'open-mb',
          label: 'Apri moodboard',
          onClick: () => navigate(`/moodboards/${mbId}`)
        }]
      });
    }} />}
      {collectionDrawer?.open && <CuratedCollectionDrawer onClose={() => setCollectionDrawer(null)} onCreated={onCollectionCreated} />}
    </div>;
}