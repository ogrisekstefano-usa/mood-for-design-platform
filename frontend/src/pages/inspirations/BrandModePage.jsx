...
const BrandCard = ({ b }) => {
  ...
  return (
    <Link to={`/inspirations/brands/${b.id}`} className="bm-card" data-testid={`bm-card-${b.id}`}>
      ...
      {atmo.length > 0 && (
        <div className="bm-card__chips" data-testid={`bm-atmo-${b.id}`}>
          {atmo.slice(0, 3).map((a) => (
            <span key={a} className="bm-chip">{a}</span>
          ))}
        </div>
      )}

      {mat.length > 0 && (
        <p className="bm-card__materials" data-testid={`bm-materials-${b.id}`}>
...
const BrandCardSkeleton = () => (
  <div className="bm-card bm-card--skeleton">
    <div className="bm-card__head">
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((b) => {
      ...
      return (b.name || '').toLowerCase().includes(ql)
          || (b.positioning || '').toLowerCase().includes(ql)
          || (b.dominant_atmospheres || []).some(a => a.toLowerCase().includes(ql))
          || (b.dominant_materials   || []).some(m => m.toLowerCase().includes(ql));
    });
      ...
      <div className="bm-controls">
        <div className="bm-search">
          ...
          <input
            ...
            placeholder="Cerca per nome, atmosfera, materia…"
            data-testid="brand-mode-search"
          />
        </div>
        {luxuryFacets.length > 0 && (
          <div className="bm-facets" data-testid="brand-mode-facets">
            <button type="button"
                    className={`bm-facet ${!luxuryFilter ? 'is-on' : ''}`}
                    onClick={() => setLuxuryFilter(null)}>
              tutti i posizionamenti
            </button>
            {luxuryFacets.map(([k]) => (
              <button key={k}
                      type="button"
                      data-testid={`brand-mode-facet-${k}`}
                      className={`bm-facet ${luxuryFilter === k ? 'is-on' : ''}`}
                      onClick={() => setLuxuryFilter(luxuryFilter === k ? null : k)}>
                {LUXURY_LABEL[k] || k}
              </button>
            ))}
          </div>
