...
const SEASONS = [
  ...
  { value: 'icon', label: 'Icon · senza stagione' },
];

const CATEGORIES = [
  '', 'Arredi', 'Illuminazione', 'Outdoor', 'Cucine', 'Bagni',
  'Rivestimenti', 'Pietra naturale', 'Decor', 'Hospitality',
];

const CollectionFormModal = ({ open, mode = 'create', brand, collection, onClose, onSaved }) => {
  const isEdit = mode === 'edit' && collection;
  const [form, setForm] = useState({
    name: '', year: '', season: '', category: '', description: '',
  });
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      setForm({ name: '', year: new Date().getFullYear(), season: '', category: '', description: '' });
    }
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        toast.success(r.data?.created === false
          ? `Collezione "${item.name}" già presente`
          : `Collezione "${item.name}" aggiunta al brand`);
      ...
      <form className="bf-modal" data-testid="collection-form-modal" onSubmit={submit}>
        <header className="bf-head">
          <div>
            ...
            <h2 className="bf-title">
              {isEdit ? <em>{collection.name}</em> :
                brand ? <>Capitolo editoriale · <em>{brand.name}</em></> : 'Aggiungi collezione'}
            </h2>
          </div>
          <button type="button" className="bf-close" onClick={onClose} data-testid="collection-form-close" aria-label="Chiudi">
            <Icons.X size={13} />
          </button>
        </header>
        ...
        <div className="bf-body">
          <label className="bf-field">
            ...
            <input
              ...
              placeholder="es. Outdoor Living 2026"
          ...
          <div className="bf-row">
            <label className="bf-field">
              ...
              <input
                ...
                placeholder="2026"
                data-testid="collection-form-year"
              />
            </label>
            <label className="bf-field">
          ...
          <label className="bf-field">
            ...
            <textarea
              ...
              placeholder="Il linguaggio progettuale di questo capitolo, le materialità prevalenti, l'atmosfera."
        ...
        <footer className="bf-foot">
          ...
          <button type="submit" className="bf-btn"
            ...
            {saving
              ? <><Icons.Loader2 size={11} className="animate-spin" /> Salvataggio</>
              : isEdit
                ? <><Icons.Save size={11} /> Salva modifiche</>
                : <><Icons.Plus size={11} /> Aggiungi collezione</>}
          </button>
        </footer>
      </form>
    </div>,
