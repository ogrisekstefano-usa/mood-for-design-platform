...
const StudioCollectionsPage = () => {
  ...
  const reload = () => {
    setLoading(true);
    Promise.all([api.get('/api/inspirations/catalogs'), api.get('/api/inspirations/registry/brands?limit=50')]).then(([rc, rb]) => {
  ...
  return <div className="sc-page" data-testid="studio-collections-page">
      <header className="sc-page__head">
        <div>
          ...
          <p className="sc-page__subtitle">
            {t("inspirations.studio_collections.tutti_i_produttori_e_le_collezioni_che_lo_studio_h")} {totalImported > 0 && <> Hai già <strong>{totalImported}</strong> Product Inspirations™ archiviate.</>}
          </p>
        </div>
        <div style={{
        display: 'flex',
        ...
        flexWrap: 'wrap'
          ...
          <Link to="/inspirations" className="ins-cta-secondary" data-testid="sc-import-catalog">
            <Icons.Plus size={12} /> Importa catalogo fornitore
          </Link>
          <Link to="/inspirations" className="ins-cta-secondary" data-testid="sc-back-inspirations">
      ...
      {!loading && grouped.length === 0 && <div className="sc-empty">
          ...
          <Link to="/inspirations" className="ins-cta-primary"><Icons.Plus size={13} /> {t("inspirations.studio_collections.importa_il_primo_catalogo")}</Link>
        </div>}

      <div className="sc-grid">
        {grouped.map(g => <BrandGroup key={g.brand_name} group={g} onAskDelete={setConfirmDel} />)}
      </div>

      <ConfirmCinematicDialog open={!!confirmDel} title={`Rimuovere il catalogo "${confirmDel?.collection || 'senza titolo'}"?`} body="Il catalogo verrà rimosso dall'archivio dello studio. I prodotti già importati nei Product Inspirations™ restano archiviati." confirmLabel="Rimuovi catalogo" tone="destructive" onConfirm={handleDelete} onClose={() => setConfirmDel(null)} testid="sc-delete-confirm" />
    </div>;
};
const BrandGroup = ({
  group,
  onAskDelete
    ...
    <header className="sc-brand__head">
      <div>
        ...
        {group.brand_meta && <p className="sc-brand__meta">
    ...
    <div className="sc-catalogs">
      {group.catalogs.map(c => <article key={c.id} className="sc-catalog" data-testid={`sc-catalog-${c.id}`} style={{
      position: 'relative'
    }}>
          <header>
          ...
          <div className="bm-actions" style={{
        position: 'absolute',
        top: 10,
        right: 10,
        opacity: 0.55,
        ...
        e.currentTarget.style.opacity = 0.55;
            ...
            <button type="button" className="bm-action-btn bm-action-btn--danger" onClick={() => onAskDelete?.(c)} data-testid={`sc-delete-${c.id}`} title="Rimuovi catalogo" style={{
          width: 26,
          height: 26
        }}>
