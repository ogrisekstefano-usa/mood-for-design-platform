...
const InsightSkeleton = () => <li className="bd-insight bd-insight--skeleton">
...
const Tile = ({
  ...
  const inner = <div className="bd-tile" data-testid={testid}>
...
const BrandDetailPage = () => {
  ...
  const reload = () => {
    setProfile(null);
    setErr(null);
    api.get(`/api/inspirations/registry/brands/${brandId}/curatorial-profile`).then(r => setProfile(r.data)).catch(e => setErr(e?.response?.data?.detail || 'Profilo non disponibile'));
  };
  ...
  if (err) {
    return <div className="bm-root bd-error" data-testid="bd-error">
  ...
  return <div className="bm-root bd-root" data-testid="brand-detail-page">
      ...
      <header className="bd-hero">
        ...
        <div className="bd-hero__body">
          ...
          {!loading && brand.positioning && <p className="bd-hero__lead">{brand.positioning}</p>}
          {!loading && <div className="bd-hero__meta">
          ...
          {!loading && brand.is_studio_private && <div className="bm-actions" data-testid="bd-actions" style={{
          marginTop: 8
        }}>
              <button type="button" className="bm-action-btn" onClick={() => setEditOpen(true)} data-testid="bd-edit-brand" title={t("inspirations.brand_detail.modifica_produttore")}>
                <Icons.Pencil size={12} strokeWidth={1.4} />
              </button>
              <button type="button" className="bm-action-btn bm-action-btn--danger" onClick={() => setConfirmDel(true)} data-testid="bd-delete-brand" title="Rimuovi produttore">
      ...
      <section className="bd-section" data-testid="bd-insights-section">
        ...
        <ol className="bd-insights">
          {loading ? Array.from({
          length: 3
        }).map((_, i) => <InsightSkeleton key={i} />) : (profile.curatorial_insights || []).map((t, i) => <Insight key={i} idx={i} text={t} />)}
        </ol>
      </section>

      {/* Atmosfere + Materialità prevalenti — chip eleganti */}
      {!loading && (profile.dominant_atmospheres.length > 0 || profile.dominant_materials.length > 0) && <section className="bd-section bd-section--cols" data-testid="bd-dominants">
          {profile.dominant_atmospheres.length > 0 && <div>
              <p className="bd-section__eyebrow">Atmosfere prevalenti</p>
              <div className="bd-chip-cloud">
                {profile.dominant_atmospheres.map(a => <span key={a.label} className="bm-chip">{a.label}</span>)}
              </div>
            </div>}
          {profile.dominant_materials.length > 0 && <div>
              <p className="bd-section__eyebrow">Materialità ricorrenti</p>
              <div className="bd-chip-cloud">
                {profile.dominant_materials.map(m => <span key={m.label} className="bm-chip bm-chip--material">{m.label}</span>)}
              </div>
            </div>}
          {profile.dominant_markets.length > 0 && <div>
              <p className="bd-section__eyebrow">Geografie narrative</p>
              <div className="bd-chip-cloud">
                {profile.dominant_markets.map(m => <span key={m.label} className="bm-chip bm-chip--market">{formatMarket(m.label)}</span>)}
              </div>
            </div>}
        </section>}

      {/* Collezioni */}
      {!loading && (profile.collections.length > 0 || collectionsAreEditable) && <section className="bd-section" data-testid="bd-collections-section">
          ...
          gap: 10
              ...
              {collectionsAreEditable && <button type="button" className="bm-action-btn" onClick={() => setCollForm({
            mode: 'create'
          })} data-testid="bd-add-collection" title={t("inspirations.brand_detail.aggiungi_capitolo_editoriale")}>
                  <Icons.Plus size={12} strokeWidth={1.4} />
                </button>}
            </div>
          </header>
          {profile.collections.length === 0 ? <p style={{
        fontSize: 11,
        color: 'var(--bp-text-muted)',
        fontStyle: 'italic',
        padding: '14px 0'
      }}>
              {t("inspirations.brand_detail.nessuna_collezione_registrata_il_primo_capitolo_ed")}
            </p> : <div className="bd-coll-grid">
              {profile.collections.slice(0, 24).map(c => <div key={c.id} className="bd-coll-card" data-testid={`bd-coll-${c.id}`} style={{
          position: 'relative'
        }}>
                  ...
                  {collectionsAreEditable && <div className="bm-actions" style={{
            position: 'absolute',
            top: 8,
            right: 8,
            opacity: 0.6,
            ...
            e.currentTarget.style.opacity = 0.6;
                      ...
                      <button type="button" className="bm-action-btn" onClick={() => setCollForm({
              mode: 'edit',
              collection: c
            })} data-testid={`bd-coll-edit-${c.id}`} title={t("inspirations.brand_detail.modifica_collezione")} style={{
              width: 24,
              height: 24
            }}>
                        <Icons.Pencil size={10} strokeWidth={1.4} />
                      </button>
                      <button type="button" className="bm-action-btn bm-action-btn--danger" onClick={() => setConfirmDelColl(c)} data-testid={`bd-coll-delete-${c.id}`} title="Rimuovi collezione" style={{
              width: 24,
              height: 24
            }}>
      ...
      {!loading && profile.products.length > 0 && <section className="bd-section" data-testid="bd-products-section">
          <header className="bd-section__head">
          ...
          <div className="bd-prod-grid">
            {profile.products.slice(0, 18).map(p => <Tile key={p.id} testid={`bd-product-${p.id}`} src={p.image_url} label={p.product_name || p.title} sub={p.collection} />)}
          </div>
        </section>}

      {/* Editorial References */}
      {!loading && profile.inspirations.length > 0 && <section className="bd-section" data-testid="bd-inspirations-section">
          <header className="bd-section__head">
          ...
          <div className="bd-prod-grid">
            {profile.inspirations.slice(0, 18).map(p => <Tile key={p.id} testid={`bd-insp-${p.id}`} src={p.image_url} label={p.title} />)}
          </div>
        </section>}

      {/* Moodboard correlate */}
      {!loading && profile.moodboards.length > 0 && <section className="bd-section" data-testid="bd-moodboards-section">
          <header className="bd-section__head">
          ...
          <div className="bd-mb-grid">
            {profile.moodboards.map(m => <Link key={m.id} to={`/moodboards/${m.id}`} className="bd-mb-card" data-testid={`bd-mb-${m.id}`}>
      ...
      {!loading && <section className="bd-section bd-jumps" data-testid="bd-quick-jump">
      ...
      <ConfirmCinematicDialog open={!!confirmDelColl} title={`Rimuovere la collezione "${confirmDelColl?.name || ''}"?`} body="La collezione verrà rimossa dal capitolo editoriale del brand. I prodotti già archiviati restano accessibili nell'archivio Inspirations™." confirmLabel="Rimuovi collezione" tone="destructive" onConfirm={handleDeleteCollection} onClose={() => setConfirmDelColl(null)} testid="bd-coll-delete-confirm" />
    </div>;
};
