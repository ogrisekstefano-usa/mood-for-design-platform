...
const CATEGORIES = [
  ...
  { value: 'cucine',        label: 'Cucine' },
  { value: 'bagni',         label: 'Bagni' },
  { value: 'rivestimenti',  label: 'Rivestimenti' },
...
const BrandFormModal = ({ open, mode = 'create', brand, onClose, onSaved }) => {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      setForm({ name: '', positioning: '', luxury_tier: '', primary_markets: [], country: '', website: '', category: '' });
    }
  }, [open, isEdit, brand]);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const payload = Object.fromEntries(
        Object.entries(form).filter(([, v]) => Array.isArray(v) ? v.length > 0 : v?.toString().trim().length > 0)
      );
      if (isEdit) {
    ...
    <div className="bf-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose?.(); }}>
      <form className="bf-modal" data-testid="brand-form-modal" onSubmit={submit}>
        <header className="bf-head">
          ...
          <button type="button" className="bf-close" onClick={onClose} data-testid="brand-form-close" aria-label="Chiudi">
            <Icons.X size={13} />
          </button>
        </header>
        ...
        <div className="bf-body">
          ...
          <Field label="Posizionamento curatoriale" testid="bf-field-positioning">
            <textarea
              ...
              placeholder="Una frase che identifica il linguaggio progettuale del brand."
          ...
          <button type="submit" className="bf-btn"
            ...
            {saving
              ...
              : isEdit
                ...
                : <><Icons.Plus size={11} /> Aggiungi all'atlante</>}
          </button>
        </footer>
      </form>
    </div>,
