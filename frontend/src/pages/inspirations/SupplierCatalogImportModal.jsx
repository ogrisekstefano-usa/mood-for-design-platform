...
const STEPS = [{
  ...
  key: 'tagging',
  label: 'Tagging e import'
}];
const SupplierCatalogImportModal = ({
  open,
  onClose,
  config,
  onImported
  ...
  useEffect(() => {
    if (!open) return;
    api.get('/api/inspirations/registry/taxonomy').then(r => setTaxonomy(r.data)).catch(() => {});
  }, [open]);
  useEffect(() => {
    if (!open) {
      // Reset on close
      ...
      setYear('');
      setCategory('arredi');
      setRightsStatus('studio_uploaded');
      setBatchAtmos([]);
      setBatchMat([]);
      setBatchMarkets([]);
      setBatchLuxury('');
  ...
  const createCatalog = async () => {
    if (!brandObj && !brand.trim()) {
      toast.error('Indica il produttore.');
      return;
    }
    try {
  ...
  const patchCandidate = (page, patch) => {
    setCandidates(prev => prev.map(c => c.page_number === page ? {
      ...c,
      ...patch
    } : c));
  };
  ...
  const selectedCount = candidates.filter(c => c.selected).length;
  return <div className="scim-backdrop" data-testid="catalog-import-modal" onMouseDown={e => {
    if (e.target === e.currentTarget) onClose?.();
      ...
      <div className="scim-modal">
        <header className="scim-head">
          ...
          <button type="button" className="scim-close" onClick={onClose} data-testid="catalog-import-close" aria-label={t('common.close')}>
            <Icons.X size={16} />
          </button>
        </header>

        <ol className="scim-steps" aria-label={t('inspirations.supplier_import.wizard_step_aria')}>
          {STEPS.map((s, i) => <li key={s.key} className={`scim-step ${i === step ? 'is-active' : ''} ${i < step ? 'is-done' : ''}`}>
        ...
        <div className="scim-body">
          {step === 0 && <IdentityStep taxonomy={taxonomy} brand={brand} setBrand={setBrand} brandObj={brandObj} setBrandObj={setBrandObj} collectionObj={collectionObj} setCollectionObj={setCollectionObj} supplierName={supplierName} setSupplierName={setSupplierName} collection={collection} setCollection={setCollection} year={year} setYear={setYear} category={category} setCategory={setCategory} rightsStatus={rightsStatus} setRightsStatus={setRightsStatus} onOpenAddBrand={() => setShowAddBrand(true)} onOpenAddCollection={() => setShowAddCollection(true)} />}

          {step === 1 && <UploadStep brand={brand} collection={collection} onPick={onFilePick} extracting={extracting} />}

          {step === 2 && <ReviewStep extraction={extraction} candidates={candidates} onPatch={patchCandidate} selectedCount={selectedCount} />}

          {step === 3 && <TaggingStep config={config} batchAtmos={batchAtmos} setBatchAtmos={setBatchAtmos} batchMat={batchMat} setBatchMat={setBatchMat} batchMarkets={batchMarkets} setBatchMarkets={setBatchMarkets} batchLuxury={batchLuxury} setBatchLuxury={setBatchLuxury} batchProfile={batchProfile} setBatchProfile={setBatchProfile} selectedCount={selectedCount} brand={brand} />}
        </div>

        <footer className="scim-foot">
          {step > 0 && step !== 1 && <button type="button" className="scim-btn-soft" onClick={goPrev} data-testid="catalog-import-back">
              <Icons.ChevronLeft size={12} /> Indietro
            </button>}
          <div className="scim-foot__spacer" />
          {step === 0 && <button type="button" className="scim-btn" onClick={createCatalog} disabled={!brandObj && !brand.trim()} data-testid="catalog-import-create">
              Continua <Icons.ChevronRight size={12} />
            </button>}
          {step === 1 && <p className="scim-hint">
              {extracting ? 'MOOD sta preparando le anteprime del catalogo…' : 'Carica un PDF per procedere.'}
            </p>}
          {step === 2 && <button type="button" className="scim-btn" onClick={async () => {
          await saveCandidates();
          goNext();
        }} disabled={selectedCount === 0} data-testid="catalog-import-confirm-review">
              Continua con {selectedCount} prodotti <Icons.ChevronRight size={12} />
            </button>}
          {step === 3 && <button type="button" className="scim-btn scim-btn--strong" onClick={finalize} disabled={importing} data-testid="catalog-import-finalize">
              {importing ? 'Importazione in corso…' : <>Importa {selectedCount} prodotti in Inspirations™ <Icons.ChevronRight size={12} /></>}
            </button>}
        </footer>
      </div>

      {/* Mini-drawer: Aggiungi produttore (Brand Registry™) */}
      {showAddBrand && <AddBrandDrawer taxonomy={taxonomy} markets={config?.markets || []} initialName={brand} onClose={() => setShowAddBrand(false)} onCreated={b => {
      setBrandObj(b);
      setBrand(b.name);
      ...
      {showAddCollection && brandObj && <AddCollectionDrawer brand={brandObj} onClose={() => setShowAddCollection(false)} onCreated={col => {
...
const IdentityStep = ({
  ...
  return <div className="scim-grid" data-testid="catalog-step-identity">
      ...
      <Field label="Produttore" required>
        <BrandPicker value={brandObj} freeText={brand} onSelect={b => {
        setBrandObj(b);
        setBrand(b?.name || '');
        setCollectionObj(null);
        if (b?.category) setCategory(b.category);
        ...
        {brandObj && <p className="scim-hint scim-hint--soft" data-testid="catalog-brand-meta">
      ...
      <Field label="Collezione">
        <CollectionPicker brand={brandObj} value={collectionObj} freeText={collection} onSelect={c => {
        setCollectionObj(c);
      ...
      <div className="scim-row">
        <Field label="Anno catalogo">
          <input type="number" className="scim-input" value={year} min="1990" max="2100" onChange={e => setYear(e.target.value)} placeholder="2026" data-testid="catalog-field-year" />
        </Field>
        <Field label="Categoria principale">
          <select className="scim-input" value={category} onChange={e => setCategory(e.target.value)} data-testid="catalog-field-category">
            {(taxonomy?.categories || []).map(c => <option key={c.key} value={c.key}>{c.label}</option>)}
          </select>
        </Field>
      </div>

      <Field label="Showroom / Studio (opzionale)">
        <input type="text" className="scim-input" value={supplierName} onChange={e => setSupplierName(e.target.value)} placeholder="Distributore / dealer / showroom" data-testid="catalog-field-supplier" />
      </Field>

      <Field label="Rights & Permissions™">
        <select className="scim-input" value={rightsStatus} onChange={e => setRightsStatus(e.target.value)} data-testid="catalog-field-rights">
          {(taxonomy?.rights_permissions || []).map(r => <option key={r.key} value={r.key}>{r.label}</option>)}
        </select>
        {rightsMeta && <div className="scim-rights-meta" data-testid="catalog-rights-meta">
...
const BrandPicker = ({
  ...
  useEffect(() => {
    setQ(value?.name || freeText || '');
  }, [value, freeText]);
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    const tmo = setTimeout(() => {
      api.get(`/api/inspirations/registry/brands?q=${encodeURIComponent(q)}&limit=20`).then(r => setItems(r.data?.items || [])).catch(() => setItems([])).finally(() => setLoading(false));
    }, 180);
    return () => clearTimeout(tmo);
  }, [q, open]);
  return <div className="scim-picker" data-testid="catalog-brand-picker">
      <input type="text" className="scim-input" value={q} onChange={e => {
      setQ(e.target.value);
      ...
      {open && <div className="scim-picker__panel" data-testid="catalog-brand-results">
        ...
        setOpen(false);
          ...
          <button type="button" className="scim-picker__add" onMouseDown={e => e.preventDefault()} onClick={() => {
        setOpen(false);
        onAddNew?.();
...
const CollectionPicker = ({
  ...
  useEffect(() => {
    ...
    api.get(`/api/inspirations/registry/brands/${brand.id}/collections`).then(r => setItems(r.data?.items || [])).catch(() => setItems([]));
  }, [open, brand?.id]);
  ...
  return <div className="scim-picker" data-testid="catalog-collection-picker">
      <input type="text" className="scim-input" value={q} onChange={e => {
      setQ(e.target.value);
      onFreeText?.(e.target.value);
      setOpen(true);
    }} onFocus={() => setOpen(true)} onBlur={() => setTimeout(() => setOpen(false), 180)} placeholder={brand ? `Cerca collezione di ${brand.name}…` : 'Seleziona prima un produttore'} disabled={!brand} data-testid="catalog-field-collection" />
      {open && brand && <div className="scim-picker__panel">
          {filtered.length === 0 && <div className="scim-picker__item is-muted">
              Nessuna collezione registrata per {brand.name}.
            </div>}
          {filtered.map(c => <button key={c.id} type="button" className={`scim-picker__item ${value?.id === c.id ? 'is-on' : ''}`} onMouseDown={e => e.preventDefault()} onClick={() => {
        onSelect(c);
        setOpen(false);
...
const AddBrandDrawer = ({
  ...
  return <div className="scim-drawer" data-testid="catalog-add-brand-drawer" onMouseDown={e => {
    if (e.target === e.currentTarget) onClose();
  }}>
      <div className="scim-drawer__panel">
        ...
        <div className="scim-drawer__body">
          <Field label="Nome" required>
            <input className="scim-input" value={name} onChange={e => setName(e.target.value)} placeholder="es. Walter Knoll" data-testid="add-brand-name" />
          </Field>
          <Field label="Website">
            <input className="scim-input" value={website} onChange={e => setWebsite(e.target.value)} placeholder="walterknoll.de" data-testid="add-brand-website" />
          </Field>
          <div className="scim-row">
            <Field label="Categoria">
              <select className="scim-input" value={category} onChange={e => setCategory(e.target.value)} data-testid="add-brand-category">
                {(taxonomy?.categories || []).map(c => <option key={c.key} value={c.key}>{c.label}</option>)}
              </select>
            </Field>
            <Field label="Paese">
          ...
          <Field label="Mercati principali">
            <div className="scim-chips" data-testid="add-brand-markets">
              {markets.map(m => <button key={m.code} type="button" className={`scim-chip ${primary.includes(m.code) ? 'is-on' : ''}`} onClick={() => toggle(m.code)}>
                  {m.label}
                </button>)}
            </div>
          </Field>
        ...
        <footer className="scim-drawer__foot">
          ...
          <button type="button" className="scim-btn" onClick={save} disabled={saving || !name.trim()} data-testid="add-brand-save">
...
const AddCollectionDrawer = ({
  ...
  return <div className="scim-drawer" data-testid="catalog-add-collection-drawer" onMouseDown={e => {
    if (e.target === e.currentTarget) onClose();
  }}>
      <div className="scim-drawer__panel">
        <header className="scim-drawer__head">
        ...
        <div className="scim-drawer__body">
          <Field label="Nome collezione" required>
            <input className="scim-input" value={name} onChange={e => setName(e.target.value)} placeholder="es. Heritage · 26 Collection" data-testid="add-collection-name" />
          </Field>
          <div className="scim-row">
            <Field label="Anno"><input type="number" className="scim-input" value={year} onChange={e => setYear(e.target.value)} placeholder="2026" data-testid="add-collection-year" /></Field>
            <Field label="Stagione"><input className="scim-input" value={season} onChange={e => setSeason(e.target.value)} placeholder="Outdoor · Permanent" data-testid="add-collection-season" /></Field>
          </div>
          <Field label="Descrizione">
            <textarea className="scim-input" rows={3} value={description} onChange={e => setDescription(e.target.value)} placeholder="Una riga descrittiva…" />
          </Field>
        </div>
        <footer className="scim-drawer__foot">
          <button type="button" className="scim-btn-soft" onClick={onClose}>{t('inspirations.supplier_catalog_import.annulla')}</button>
          <button type="button" className="scim-btn" onClick={save} disabled={saving || !name.trim()} data-testid="add-collection-save">
...
const UploadStep = ({
  ...
  return <div className="scim-upload" data-testid="catalog-step-upload">
      <p className="scim-lead">
      ...
      <label htmlFor="scim-file-input" className={`scim-dropzone ${dragging ? 'is-drag' : ''} ${extracting ? 'is-busy' : ''}`} onDragOver={e => {
      e.preventDefault();
      setDragging(true);
      ...
      if (f) onPick(f);
        ...
        <input id="scim-file-input" type="file" accept="application/pdf,.pdf" style={{
        display: 'none'
      }} disabled={extracting} onChange={e => onPick(e.target.files?.[0])} />
        {extracting ? <>
...
const ReviewStep = ({
  ...
  return <div className="scim-review" data-testid="catalog-step-review">
      ...
      {(extraction.warnings || []).length > 0 && <ul className="scim-review__warnings">
          {extraction.warnings.map((w, i) => <li key={i}><Icons.Info size={11} /> {w}</li>)}
        </ul>}

      <div className="scim-review__actions">
        <button type="button" className="scim-link" onClick={() => candidates.forEach(c => onPatch(c.page_number, {
        selected: true
      ...
      <div className="scim-grid-cards">
        {candidates.map(c => <CandidateCard key={c.page_number} c={c} editing={editing === c.page_number} onEditToggle={() => setEditing(editing === c.page_number ? null : c.page_number)} onPatch={patch => onPatch(c.page_number, patch)} />)}
      </div>
    </div>;
};
const CandidateCard = ({
  ...
  onEditToggle,
  onPatch
}) => <div className={`scim-cand ${c.selected ? 'is-on' : ''}`} data-testid={`catalog-cand-${c.page_number}`}>
    <button type="button" className="scim-cand__media" onClick={() => onPatch({
    selected: !c.selected
    ...
    <div className="scim-cand__body">
      {editing ? <>
          <input type="text" className="scim-input scim-input--inline" value={c.product_name || ''} onChange={e => onPatch({
        product_name: e.target.value
      })} placeholder="Nome prodotto" data-testid={`catalog-cand-name-${c.page_number}`} />
          <input type="text" className="scim-input scim-input--inline" value={c.category_hint || ''} onChange={e => onPatch({
        category_hint: e.target.value
      })} placeholder="Categoria" />
          <input type="text" className="scim-input scim-input--inline" value={c.designer || ''} onChange={e => onPatch({
        designer: e.target.value
          ...
          {(c.category_hint || c.designer) && <p className="scim-cand__meta">
              {c.category_hint}{c.category_hint && c.designer && ' · '}
              {c.designer && <em>design {c.designer}</em>}
            </p>}
          <button type="button" className="scim-link" onClick={onEditToggle} data-testid={`catalog-cand-edit-${c.page_number}`}>
...
const TaggingStep = ({
  ...
  const toggle = (arr, setArr, key) => {
    if (arr.includes(key)) setArr(arr.filter(k => k !== key));else setArr([...arr, key]);
  };
  return <div className="scim-tagging" data-testid="catalog-step-tagging">
      <p className="scim-lead">
      ...
      <TagGroup title={t('inspirations.supplier_import.atmosphere_title')} options={config?.atmosphere_tags || []} values={batchAtmos} onToggle={k => toggle(batchAtmos, setBatchAtmos, k)} testid="catalog-tag-atmos" />

      <TagGroup title="Materia" options={config?.material_tags || []} values={batchMat} onToggle={k => toggle(batchMat, setBatchMat, k)} testid="catalog-tag-mat" />

      <TagGroup title="Mercati di affinità" options={config?.markets || []} codeKey="code" values={batchMarkets} onToggle={k => toggle(batchMarkets, setBatchMarkets, k)} testid="catalog-tag-markets" />

      <div className="scim-row">
        <div>
          <p className="scim-label">Livello luxury</p>
          <select className="scim-input" value={batchLuxury} onChange={e => setBatchLuxury(e.target.value)} data-testid="catalog-tag-luxury">
            <option value="">Da definire</option>
            {(config?.luxury_levels || []).map(l => <option key={l.key} value={l.key}>{l.label}</option>)}
          </select>
        </div>
        <div>
          <p className="scim-label">Destinazione</p>
          <select className="scim-input" value={batchProfile} onChange={e => setBatchProfile(e.target.value)} data-testid="catalog-tag-profile">
            <option value="">Da definire</option>
            {(config?.hospitality_profiles || []).map(p => <option key={p.key} value={p.key}>{p.label}</option>)}
          </select>
        </div>
      </div>
    </div>;
...
const TagGroup = ({
  ...
  testid
    ...
    <div className="scim-chips">
      ...
      return <button key={k} type="button" className={`scim-chip ${active ? 'is-on' : ''}`} onClick={() => onToggle(k)}>
            {opt.label}
          </button>;
    })}
    </div>
