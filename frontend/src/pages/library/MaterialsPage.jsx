...
const NewMaterialModal = ({ onClose, onCreated }) => {
  const [form, setForm] = useState({
    name: '', category: '', subcategory: '', supplier: '', finish: '', thickness: '', origin: '',
    description: '',
  });
  ...
  return (
    ...
    >
      <div
        ...
        className="w-full max-w-2xl bg-[var(--bp-surface-1)] border border-[var(--bp-border)] rounded-md p-6 max-h-[90vh] overflow-y-auto"
      >
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="col-span-2">
            <F label="Editorial description" k="description" area
               placeholder="Quartzite with cream tone and gold + copper veins…" />
          </div>
        </div>
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button
            type="button"
            onClick={onClose}
            data-testid="new-material-cancel-btn"
          ...
          <button
            ...
            onClick={submit}
            data-testid="new-material-submit-btn"
...
const MaterialCard = ({ material }) => (
  <Link
    to={`/library/materials/${material.slug}`}
    data-testid={`material-card-${material.slug}`}
    className="group block rounded-md border border-[var(--bp-border)] bg-[var(--bp-surface-1)]
  ...
  >
    <div className="aspect-[4/5] bg-[var(--bp-surface-2)] relative overflow-hidden">
      {material.primary_asset?.file_url || material.primary_asset?.display_url ? (
        ...
        <div
          ...
          style={{ background: material.dominant_color || undefined }}
        >
          <Gem size={28} className="text-[var(--bp-text-muted)] opacity-60" strokeWidth={1.3} />
        </div>
      )}
      {material.asset_count > 0 && (
        <div className="absolute top-3 right-3 px-2 py-0.5 rounded
    ...
    <div className="p-4">
      {material.category && (
        <p className="text-[9px] uppercase tracking-[0.28em] text-[var(--bp-text-muted)] font-body mb-1.5">
      ...
      <div className="px-10 pt-10 pb-8 border-b border-[var(--bp-border)]">
        ...
        <div className="flex items-end justify-between gap-6 flex-wrap">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-[5px] bg-[var(--bp-primary)] text-black
      ...
      <div className="px-10 py-4 border-b border-[var(--bp-border)] flex items-center gap-4 flex-wrap">
        <div className="flex-1 max-w-md relative min-w-[240px]">
          ...
          <input
            ...
            placeholder="Search materials, suppliers, finishes…"
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <button
            ...
            className={`px-3 py-1 rounded-full text-[11px] font-body uppercase tracking-wider transition-colors ${
              !category
                ? 'bg-[var(--bp-primary)]/10 text-[var(--bp-primary)] border border-[var(--bp-primary)]/30'
                : 'text-[var(--bp-text-muted)] hover:text-[var(--bp-text-primary)] border border-[var(--bp-border)]'
            }`}
          >
            All
          </button>
        ...
        {loading && materials.length === 0 ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-center pt-12">
            ...
            <button
              type="button"
              onClick={() => setShowNew(true)}
              data-testid="empty-state-new-material-btn"
