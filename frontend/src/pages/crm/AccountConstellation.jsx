...
const STATE_TONE = {
  ...
  active:    { eyebrow: 'Attiva',          glow: 'teal' },
  trusted:   { eyebrow: 'Di fiducia',      glow: 'gold' },
  dormant:   { eyebrow: 'In silenzio',     glow: 'muted' },
  strategic: { eyebrow: 'Strategica',      glow: 'gold' },
  returning: { eyebrow: 'Tornata',         glow: 'teal' },
};
...
const AccountConstellation = ({ accountId }) => {
  ...
  useEffect(() => {
    if (!accountId) return;
    setLoading(true);
    api.get(`/api/relationships/accounts/${accountId}/constellation`)
  ...
  if (loading) {
    return (
      <div className="ac-loading" data-testid="constellation-loading">
        Sto leggendo la costellazione relazionale…
      </div>
    );
  }
  if (!data) return null;

  const { hero, active_journeys = [], people = [], memory = {},
          shared_artifacts = [], insights = [], lexicon = {} } = data;
  ...
  return (
    <div className="ac-shell" data-testid="account-constellation">
      ...
      <header className={`ac-hero ac-hero--${stateMeta.glow}`} data-testid="ac-hero">
        ...
        <div className="ac-hero__meta">
          <span data-testid="ac-hero-journeys">
            ...
            {hero.total_journeys > hero.active_journeys &&
              <em> · {hero.total_journeys} totali</em>}
          </span>
          {hero.primary_contact && (
            <span>Riferimento · {hero.primary_contact.first_name}
        ...
        {active_journeys.length === 0 ? (
          ...
          <div className="ac-journey-grid">
            {active_journeys.map((j) => {
              ...
              return (
                <article key={j.id}
                         className={`ac-journey-card ac-journey-card--${glow}`}
                         onClick={() => openJourney(j)}
                         data-testid={`ac-journey-${j.id}`}>
      ...
      {people.length > 0 && (
        <section className="ac-section" data-testid="ac-people-section">
          ...
          <div className="ac-people">
            {people.map((p) => (
              <div key={p.id} className="ac-person" data-testid={`ac-person-${p.id}`}>
      ...
      {/* ── PROJECT MEMORY ──────────────────────────────────── */}
      {(memory.preferred_materials?.length || memory.rationales?.length) > 0 && (
        <section className="ac-section ac-memory" data-testid="ac-memory-section">
          <header className="ac-section__head">
          ...
          {memory.preferred_materials?.length > 0 && (
            <div className="ac-memory__row">
              <div className="ac-memory__label">Materie che ama</div>
              <div className="ac-memory__chips">
                {memory.preferred_materials.map((m) => (
                  <span key={m} className="ac-chip">{m}</span>
                ))}
              </div>
            </div>
          )}

          {memory.saved_inspirations_count > 0 && (
            <div className="ac-memory__row">
          ...
          {memory.rationales?.length > 0 && (
            <div className="ac-memory__rationales">
              {memory.rationales.slice(0, 3).map((r, i) => (
                <blockquote key={i} className="ac-rationale" data-testid={`ac-rationale-${i}`}>
      ...
      {/* ── SHARED ARTIFACTS ────────────────────────────────── */}
      {shared_artifacts.length > 0 && (
        <section className="ac-section" data-testid="ac-artifacts-section">
          <header className="ac-section__head">
          ...
          <div className="ac-artifacts">
            {shared_artifacts.slice(0, 8).map((a) => (
              <div key={`${a.artifact_type}-${a.artifact_id}`}
                   className="ac-artifact"
                   data-testid={`ac-artifact-${a.artifact_id}`}>
                <div className="ac-artifact__kind">
                  {a.artifact_type === 'moodboard' ? 'Moodboard' :
                   a.artifact_type === 'proposal'  ? 'Proposta'  :
                   'Collezione curata'}
                </div>
      ...
      {insights.length > 0 && (
        <section className="ac-section ac-insights" data-testid="ac-insights-section">
          <header className="ac-section__head">
            <span className="ac-section__eyebrow"><Compass size={11} /> Segnali del viaggio</span>
            <h2 className="ac-section__title">Momenti in attesa</h2>
          </header>
          <div className="ac-insights__list">
            {insights.map((s, i) => (
              <div key={i} className={`ac-insight ac-insight--${s.severity || 'soft'}`}
                   data-testid={`ac-insight-${i}`}>
