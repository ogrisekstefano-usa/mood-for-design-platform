...
export const ActivityModal = ({ open, account, defaultType = 'internal_note',
                               onClose, onCreated }) => {
  ...
  React.useEffect(() => {
    if (!open) return;
    setType(defaultType);
    setTitle(''); setSummary(''); setNextStep(''); setNextDate('');
  }, [open, defaultType]);
    ...
    <div className="rl-modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose?.(); }}>
      <div className="rl-modal" data-testid="activity-modal">
        <div className="rl-modal__head">
          ...
          <button className="rl-modal__close" onClick={onClose} aria-label="Chiudi" data-testid="activity-modal-close">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="rl-field">
          ...
          <select className="rl-field__select" value={type}
        ...
        {isVoice ? (
          <VoiceRecorder accountId={accountId}
                         onSaved={(it) => { onCreated?.(it); onClose?.(); }}
                         onClose={onClose} />
        ) : (
          <>
            <div className="rl-field">
              <label className="rl-field__label">Titolo</label>
              <input className="rl-field__input" type="text" value={title}
                     onChange={(e) => setTitle(e.target.value)}
                     placeholder="Es. Visita showroom · Maya Aldhabi"
                     data-testid="activity-title-input" />
            </div>
            <div className="rl-field">
              <label className="rl-field__label">Note</label>
              <textarea className="rl-field__textarea" value={summary}
                        onChange={(e) => setSummary(e.target.value)}
                        placeholder="Racconta l'essenziale — pensieri, impressioni, prossimi step…"
                        data-testid="activity-summary-input" />
            </div>
            <div className="rl-field">
              <label className="rl-field__label">Prossimo passo (opzionale)</label>
              <input className="rl-field__input" type="text" value={nextStep}
                     onChange={(e) => setNextStep(e.target.value)}
                     placeholder="Es. Inviare proposta materiali"
                     data-testid="activity-nextstep-input" />
            </div>
            <div className="rl-field">
              <label className="rl-field__label">Data follow-up (opzionale)</label>
              <input className="rl-field__input" type="date" value={nextDate}
                     onChange={(e) => setNextDate(e.target.value)}
                     data-testid="activity-nextdate-input" />
            ...
            <div className="rl-modal__actions">
              ...
              <button className="rl-btn rl-btn--primary" onClick={save}
                      disabled={saving} data-testid="activity-save">
