...
import { X, UserCircle, FileText, ListChecks, Folder, Layers, Compass, BellRing, NotebookPen, Palette, Clock, Plus, Loader2 } from 'lucide-react';
...
const STAGE_LABEL = {
  ...
  editorial_engagement: 'Editorial',
  project_conversation: 'In conversazione',
  material_exploration: 'Materiali',
  strategic_direction: 'Direzione',
  specification: 'Specifica',
  proposal: 'Proposta',
  active_collaboration: 'Attivo',
...
const ContactsPane = ({
  ...
  return <div data-testid="adr-contacts-pane">
      <p className="adr-helper">
        I Contact sono <strong>persone esterne</strong> {t("crm.account_detail.collegate_a_questo_account_i_membri_del_team_appar")} <strong>Team</strong> e non vengono mischiati qui.
      </p>
      {loading && <p className="adr-empty">{t('crm.account_detail.caricamento')}</p>}
      {!loading && contacts.length === 0 && <p className="adr-empty">{t('crm.account_detail.nessun_contact_ancora_aggiungi_il_primo_qui_sotto')}</p>}
      <div className="adr-list">
        {contacts.map(c => <div key={c.id} className="adr-card" data-testid={`adr-contact-${c.id}`}>
      ...
      {!showAdd ? <button type="button" className="crm-btn crm-btn--ghost" onClick={() => setShowAdd(true)} data-testid="adr-contact-add">
...
const TimelinePane = ({
  ...
  return <div data-testid="adr-timeline-pane">
...
const AccountDetailDrawer = ({
  ...
  return <div className="adr-bg" onClick={e => {
      ...
      <aside className="adr" onClick={e => e.stopPropagation()}>
        <header className="adr__head">
          <div className="adr__head-main">
            <span className="adr__avatar" style={{
            background: pal.bg,
            color: pal.fg,
          ...
          <button type="button" onClick={onClose} className="adr__close" data-testid="adr-close" aria-label={t("crm.account_detail.chiudi")}>
            <X size={17} />
          </button>
        </header>
          ...
          <div className="adr__fact">
            ...
            <span className="adr__fact-val">
              ...
              borderColor: stageCol
                ...
                <span className="adr__stage-pill-dot" style={{
