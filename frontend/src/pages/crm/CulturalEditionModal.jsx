...
export const CulturalEditionModal = ({
  open,
  accountId,
  sourceType = 'project',
  sourceId = '',
  ...
  const selectedSub = useMemo(() => submarkets.find(x => x.code === selected), [submarkets, selected]);
  ...
  return <div className="rl-modal-backdrop" data-testid="cultural-edition-modal-backdrop" onClick={e => {
    if (e.target === e.currentTarget) onClose?.();
      ...
      <div className="rl-modal" data-testid="cultural-edition-modal">
        <div className="rl-modal__head">
          ...
          <button className="rl-modal__close" onClick={onClose} aria-label={t("crm.cultural_edition.chiudi")} data-testid="ce-close">
            <X size={18} />
          </button>
        </div>
        ...
        marginBottom: 18
          ...
          {t("crm.cultural_edition.mood_adatta_tono_ritmo_cta_e_narrativa_al_mercato")}
        </p>
        ...
        <div className="rl-field">
          ...
          gap: 8
            ...
            <select className="rl-field__select" value={srcType} onChange={e => setSrcType(e.target.value)} style={{
            maxWidth: 200
            ...
            <input className="rl-field__input" value={srcId} onChange={e => setSrcId(e.target.value)} placeholder="ID origine (opzionale per account)" data-testid="ce-source-id" />
          </div>
        </div>

        <div className="rl-field">
          <label className="rl-field__label">Macro mercato</label>
          <select className="rl-field__select" value={marketFilter} onChange={e => setMarketFilter(e.target.value)} data-testid="ce-market-select">
        ...
        <div className="rl-field">
          ...
          <select className="rl-field__select" value={selected} onChange={e => setSelected(e.target.value)} data-testid="ce-submarket-select">
            <option value="">Seleziona…</option>
            {filtered.map(s => <option key={s.code} value={s.code}>
                {subLabel(s)} · {s.macro_market_code}
              </option>)}
          </select>
        </div>

        {selectedSub && <div className="rl-insight" style={{
        marginBottom: 14
      }}>
        ...
        <div className="rl-field">
          ...
          <select className="rl-field__select" value={locale} onChange={e => setLocale(e.target.value)} data-testid="ce-locale-select">
        ...
        <div className="rl-field">
          ...
          <textarea className="rl-field__textarea" value={note} onChange={e => setNote(e.target.value)} placeholder={t("crm.cultural_edition.briefing_editoriale_angle_desiderato_vincoli")} data-testid="ce-note" />
        </div>

        <div className="rl-modal__actions">
          <button className="rl-btn rl-btn--ghost" onClick={onClose} data-testid="ce-cancel">
            {t("crm.cultural_edition.annulla")}
          </button>
          <button className="rl-btn rl-btn--primary" onClick={save} disabled={!selected || saving} data-testid="ce-create">
