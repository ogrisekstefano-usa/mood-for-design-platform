...
- **Sprint ITER138 · Blueprint Atelier™ Visual System — Phase 2 (Global Component Audit + Wave Propagation)** · ✅ CORE DELIVERED · 23 Feb 2026 · Atelier Nordic™ DNA propagato all'intera applicazione tramite **single global component override layer + module-specific editorial overlays**. **3 deliverables consegnati e validati live**:

  **(1) Global Atelier Component Layer** (`frontend/src/design-system/atelier/components.css` · NEW · ~1300 LoC). Single source of truth che allinea ogni primitivo shadcn/ui (Button, Input, Card, Dialog, Drawer, Dropdown, Popover, Tooltip, Table, Tabs, Toggle, Switch, Checkbox, Radio, Progress, Skeleton, Toast, Pagination, Select, Sheet, Separator, Badge) ai token Atelier Nordic™. Strategia: re-binding dei token HSL Tailwind/shadcn (`--background`, `--card`, `--primary`, `--border`, `--input`, `--ring`, `--muted`, `--accent`, `--destructive`) sotto `[data-atelier="nordic"]` → ogni `<Card>`, `<Button>`, `<Input>` eredita Nordic senza un solo edit JSX. **Legacy `--bp-*` bridge** mappa `--bp-font-heading → Cormorant Garamond`, `--bp-font-body → Inter`, `--bp-primary-soft → rgba(94,234,212,0.06)`, `--bp-border-hover/active → cyan-line`, `--bp-surface-elevated → bp-elevated` — così tutte le pagine legacy (CRM, Inspirations, Settings, Editorial, Governance) ereditano automaticamente Atelier Nordic senza toccare i loro CSS individuali. Polish layer cinematic: focus rings cyan globali, scrollbar atmospheric, selection cyan glow, tooltip whisper-style, table editorial register (no zebra SaaS), tab underline-style (no pill), Switch cyan glow su checked, Toaster glass-morphism. Re-usable primitives: `.atelier-modal` (luxury dialog shell con head/body/foot), `.atelier-field` (form field editorial), `.atelier-page` (canvas container con eyebrow/title/sub/actions).

  **(2) Wave 1 · Design Journey Atelier** (`/workspace/projects`) — `projects-page.css` riscritto da zero (300 LoC), rimossi tutti i `--mood-*` legacy + Playfair Display + JetBrains Mono. Ora usa esclusivamente token Atelier Nordic. Atmospheric cyan-radial background wash, header editorial (cyan eyebrow + Cormorant italic display title), tabs underline cinematic, ProjectCard editoriale (cyan glow status rail · italic serif title · cyan accent material chips · "Continua il viaggio" cyan-uppercase CTA on hover). NewProjectModal sostituito con `.atelier-modal` primitivo. Empty state silenzioso. **Validato live**: lista progetti `MISS 10 · LEAK 0` su locale es-ES, 3-col responsive grid 1920px, cyan signature ovunque.

  **(3) Wave 2 · CRM Relationship Intelligence Lounge** (`/crm/*`) — override mirati per `.crm-page`/`.crm-hero`/`.crm-tabs`/`.crm-pulse`/`.crm-toolbar`/`.crm-cards`/`.crm-card`/`.crm-table`/`.crm-modal`/`.crm-followups` + AccountDetailDrawer (`.adr-*`). Goal: trasformare il CRM da "sales pipeline tool" a "relationship intelligence lounge". Cyan eyebrow "CRM" · Cormorant italic display title · tabs underline editorial (Accounts/Follow-ups/Archived) · italic serif helper text · big KPI editorial numbers (109 · 0 · 0) · pill search bar · pill filter selects · AccountCard editoriale (italic serif name, eyebrow type, glass-morphism stage pill, italic family-name primary contact, cyan follow-up badge) · `.crm-btn--primary` = cyan pill con glow signature. **Validato live**: `/crm/accounts` rendering 109 account come gallery, sensazione "studio di conversazione" raggiunta.

  **(4) Wave 3 · Inspirations + Brand Atlas Curatorial Gallery** (`/inspirations`, `/inspirations/brands`) — override per `.ins-*` (page/header/title/lede/tabs/cards/chips/modal/drawer) + `.bm-*` (BrandMode) + `.bd-*` (BrandDetail). Goal: gallery system, material archive, collectible. Atmosphere radial cyan wash · pill secondary actions (Studio Collections™ / Brand Atlas™ / Material View™ / Importa catalogo fornitore) · editorial Cormorant italic title "Inspirations™" · masonry-ish 6-col grid con cinematic hover (image scale + brightness shift) · cyan PRODOTTO/EDITORIALE chips · BrandCard collectible (cyan-line PREMIUM tag, italic material chips "sobrio · architettonico · materialità · legno · metallo", editorial numeric counts "11 COLLEZIONI · 37 PRODOTTI"). Validato live entrambe le route.

  **(5) Wave 4 · Settings Operational Coherence** (`/settings`) — non richiede CSS override aggiuntivo: il `--bp-*` token bridge in `components.css` rende le cards/sezioni native Atelier Nordic. Cyan section eyebrows (ATELIER · WORKSPACE · VETRINA), card luxury con bordi sottili, icon accent cyan, badge SUPERADMIN cyan pill. Tono "cinematic silence luxury restraint" rispettato → no fallback admin UI generico.

  **Atelier Nordic™ DNA congelato — v1**: backgrounds layered blacks 5 depths · cyan glow singolo accent sacro · Cormorant Garamond editorial serif + Inter operational sans · luxury restraint radii (3-18px) · editorial spacing (4-96px scale) · cinematic motion ease (cubic-bezier 0.16,1,0.3,1) · architectural thin borders. Nessun viola/gradient SaaS, nessun neon. La Shell (Sidebar, Topbar, DashboardLayout) + Dashboard (Wave A+B) + 4 wave Phase 2 = **intera superficie OS coerente sotto un unico linguaggio visivo**.

  **NON consegnato in questa sessione (P1 backlog ITER138 Phase 3)**: ⚠️ **Responsive hardening completo** (iPad refinement, Ultrawide refinement, mobile graceful blockers eleganti per moduli desktop-first). ⚠️ **Deep refactor pagine annidate** (Editorial Studio, Editorial Calendar, Storefront Studio, Admin Tenants, Advisor Network — questi pagine usano CSS proprio e beneficiano dall'override globale ma non sono state visualmente verificate). ⚠️ **Audit overlay legacy** (alcuni `.bg-white` / `text-black` Tailwind hard-coded sopravvivono in pages annidate — il catch-all in `components.css` les neutralizza, ma un audit fine-grained richiede sprint dedicato).
